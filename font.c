...
//	glAlphaFunc(GL_GREATER, 0.1f);
	...
	while (1) {
		...
		if (cur_char == '\n' || cur_char == '\r') {
			...
			if (cur_y - y > height - displayed_font_y_size) {
				break;
			}
			continue;
		}
...
void draw_ortho_ingame_scaled(float x, float y, float z, const unsigned char *our_string, int max_lines, float displayed_font_x_size, float displayed_font_y_size) {
	unsigned char cur_char;
	float cur_x, cur_y;
	int current_lines = 1;
	int i = 0;
...
void draw_ortho_ingame_string_zoomed(float x, float y, float z, const unsigned char *our_string, int max_lines, float font_x_scale, float font_y_scale, float zoom) {
...
int pos_selected(int msg, int ichar, select_info *select) {
	...
	if (select->em > select->sm) {
		d = 1;
	} else if ((select->em == select->sm) && (select->ec >= select->sc)) {
		d = 1;
	} else {
		d = -1;
	}
	if (d * (msg - select->sm) > d * (select->em - select->sm)) {
		return 0;
	}
	if ((msg == select->em) && (d * ichar > d * select->ec)) {
		return 0;
	}
	if (d * msg < d * select->sm) {
		return 0;
	}
	if ((msg == select->sm) && (d * ichar < d * select->sc)) {
		return 0;
	}
	return 1;
}
...
void draw_messages(int x, int y, text_message *msgs, int msgs_size, Uint8 filter, int msg_start, int offset_start, int cursor, int width, int height, float text_zoom, select_info *select) {
	...
	imsg = msg_start;
	ichar = offset_start;
	if (msgs[imsg].data == NULL || msgs[imsg].deleted) {
		return;
	}
	if (width < displayed_font_x_size || height < displayed_font_y_size) {
		return;
	...
	for (node = widthsnode->children; node; node = node->next) {
		if (xmlStrEqual(node->name, (xmlChar *)"text")) {
			...
			while (*widths) {
				...
				if (n >= FONT_CHARS_PER_LINE * FONT_CHARS_MAX_LINE) {
					break;
				}
				widths++;
			}
...
void set_font_parameters(xmlNode *node) {
	int num;
	if (!xmlStrEqual(node->name, (xmlChar *)"font")) {
		return;
	}
