...
static int resize_handler(window_info *win, int width, int height) {
	...
	widget_move(server_popup_win, buttonId, (width - button_widget->len_x) / 2, height - sep - button_widget->len_y);
	...
	if (!actual_scroll_width && (text_widget_height < (2 * sep + num_text_lines * DEFAULT_FONT_Y_LEN * chat_zoom))) {
		actual_scroll_width = scroll_width;
		text_widget_width = width - (2 * sep + actual_scroll_width);
		widget_resize(server_popup_win, textId, text_widget_width, text_widget_height);
	...
	if (actual_scroll_width) {
		scroll_id = vscrollbar_add_extended(server_popup_win, scroll_id, NULL, width - (scroll_width + sep), sep, scroll_width, text_widget_height, 0, (text_widget_height - 2 * sep) / (DEFAULT_FONT_Y_LEN * chat_zoom), 0.77f, 0.57f, 0.39f, scroll_line, 1, num_text_lines);
...
void display_server_popup_win(const char *const message) {
	...
	if (server_popup_win < 0) {
		...
		server_popup_win = create_window("", -1, 0, server_popup_win_x, server_popup_win_y, winWidth, winHeight, win_property_flags);
		...
		buttonId = button_add_extended(server_popup_win, buttonId, NULL, 0, 0, 0, 0, 0, chat_zoom, 0.77f, 0.57f, 0.39f, "OK");
	...
	if ((!text_message_is_empty(&widget_text)) && (widget_find(server_popup_win, textId) == NULL)) {
		textId = text_field_add_extended(server_popup_win, textId, NULL, sep, sep, text_widget_width, text_widget_height, TEXT_FIELD_NO_KEYPRESS, chat_zoom, chat_font, 0.77f, 0.57f, 0.39f, &widget_text, 1, FILTER_NONE, sep, sep);
	}
