...
int get_console_text_width(void) { return console_text_width; }
...
static void update_total_nr_lines(void) {
	int n = 0;
	for (text_message *m = display_text_buffer, *e = m + last_message + 1; m < e; ++m) {
		if (m->chan_idx != CHAT_NONE && m->chan_idx != CHAT_COMBAT && !m->deleted && m->data && m->len) {
			n += rewrap_message(m, chat_text_size, chat_font, console_text_width, 0);
		}
	}
	total_nr_lines = n;
}
...
void update_console_win(text_message *msg) {
	if (!msg->deleted) {
		rewrap_message(msg, chat_text_size, chat_font, console_text_width, NULL);
		if (scroll_up_lines == 0) {
			console_text_changed = 1;
		} else {
	...
	scroll_up_lines = clampi(scroll_up_lines, 0, DISPLAY_TEXT_BUFFER_SIZE);
	update_total_nr_lines();
}
