...
void init_ipu(INPUT_POPUP *ipu, int parent, int x_len, int y_len, int maxlen, int rows, void cancel(void *), void input(const char *, void *)) {
	...
	ipu->popup_win = ipu->popup_field = ipu->popup_label = ipu->popup_ok = ipu->popup_no = ipu->x = ipu->y = -1;
	if (x_len == -1) {
...
static INPUT_POPUP *ipu_from_widget(widget_list *w) {
	if ((w == NULL) || (w->window_id < 0) || (windows_list.num_windows < w->window_id) || (windows_list.window[w->window_id].data == NULL)) {
...
static int popup_cancel_button_handler(widget_list *w, int UNUSED(mx), int UNUSED(my), Uint32 flags) {
...
static int popup_ok_button_handler(widget_list *w, int UNUSED(mx), int UNUSED(my), Uint32 flags) {
	INPUT_POPUP *ipu = ipu_from_widget(w);
	if (ipu == NULL) {
		return 0;
	}
...
static int popup_keypress_handler(window_info *win, int UNUSED(mx), int UNUSED(my), Uint32 key, Uint32 unikey) {
...
static int click_save_handler(widget_list *w, int UNUSED(mx), int UNUSED(my), Uint32 flags) {
	// only handle mouse button clicks, not scroll wheels moves
	// Update: don't check when saving on exit (w == NULL)
	if ((flags & ELW_MOUSE_BUTTON) == 0 && w != NULL) {
		return 0;
	}
...
static int notepad_remove_category(widget_list *UNUSED(w), int UNUSED(mx), int UNUSED(my), Uint32 flags) {
...
static int open_note_tab(widget_list *UNUSED(w), int UNUSED(mx), int UNUSED(my), Uint32 flags) {
	int i = 0;
	// only handle mouse button clicks, not scroll wheels moves
	if ((flags & ELW_MOUSE_BUTTON) == 0) {
		return -1;
...
static int notepad_add_category(widget_list *UNUSED(w), int UNUSED(mx), int UNUSED(my), Uint32 flags) {
	// only handle mouse button clicks, not scroll wheels moves
	if ((flags & ELW_MOUSE_BUTTON) == 0) {
		return 0;
	}
...
static int click_buttonwin_handler(window_info *UNUSED(win), int UNUSED(mx), int UNUSED(my), Uint32 flags) {
