...
void remove_tab(Uint8 channel, int niveau_tab) {
	...
	if (niveau_tab == 1) {
		if (tab_bar_win_1 != -1) {
			remove_tab_button(channel, 1);
		}
	} else if (niveau_tab == 2) {
		if (tab_bar_win_2 != -1) {
			remove_tab_button(channel, 2);
		}
	}
	if (chat_win != -1) {
...
void update_tab_idx(Uint8 old_idx, Uint8 new_idx, int niveau_tab) {
	...
	if (tab_bar_win_1 != -1 && niveau_tab == 1) {
		update_tab_button_idx(old_idx, new_idx, 1);
	}
	if (tab_bar_win_2 != -1 && niveau_tab == 2) {
		update_tab_button_idx(old_idx, new_idx, 2);
	}
	if (chat_win != -1) {
...
void set_channel_tabs(const Uint32 *chans) {
	...
	for (chan_nr = 0; chan_nr < nmax; chan_nr++) {
		...
		for (chan_nrp = 0; chan_nrp < nmax; chan_nrp++) {
			if (active_channels[chan_nrp] == chan) {
				break;
			}
		}
		if (chan_nrp >= nmax) {
...
int close_channel(window_info *win) {
	...
	for (ichan = 0; ichan < MAX_CHAT_TABS; ichan++) {
		if (channels[ichan].tab_id == id) {
			...
			// Safe to remove?
			if (tab_bar_win_1 != -1) {
				remove_tab_button(channels[ichan].chan_nr, 1);
			}
			return 1;
		}
	}
...
void add_chan_name(int no, char *name, char *desc) {
	chan_name *entry;
	int len;
	if (((entry = malloc(sizeof(*entry))) == NULL) || ((entry->description = malloc(strlen(desc) + 1)) == NULL) || ((entry->name = malloc(strlen(name) + 1)) == NULL)) {
...
int test_channel_race(int no_channel) {
	...
	if ((no_channel >= 11) && (no_channel <= 19)) {
		...
		if (((your_info.race == HAUT_ELFE) && (no_channel == 11)) || ((your_info.race == GALDUR) && (no_channel == 12)) || ((your_info.race == HOMME_BLEU) && (no_channel == 13)) || ((your_info.race == HUMAIN_ELDORIAN) && (no_channel == 14)) || ((your_info.race == KULTAR) && (no_channel == 15)) || ((your_info.race == NAIN) && (no_channel == 16)) || ((your_info.race == ELFE_NOIR) && (no_channel == 17)) || ((your_info.race == HUMAIN_SINAN) && (no_channel == 18)) || ((your_info.race == HUMAIN_INDEFINI) && (no_channel == 19))) {
			return 1;
		}
...
int tab_bar_button_click(widget_list *w, int mx, int my, Uint32 flags, int ligne) {
	...
	if (flags & ELW_RIGHT_MOUSE) {
		...
		if (ligne == 1) {
			...
			if (tabs_2[itab].channel == CHAT_CHANNEL1 || tabs_2[itab].channel == CHAT_CHANNEL2 || tabs_2[itab].channel == CHAT_CHANNEL3 || tabs_2[itab].channel == CHAT_CHANNEL4 || tabs_2[itab].channel == CHAT_CHANNEL5) {
				int x = w->len_x - 6;
				int y = 5;
				char str[256];
				// 'x' was clicked?
...
chan_name *tab_label(Uint8 chan) {
	...
	switch (chan) {
	case CHAT_ALL:
		return pseudo_chans[2];
	case CHAT_NONE:
		return pseudo_chans[3];
	case CHAT_LIST:
		return pseudo_chans[4];
	case CHAT_HIST:
		return pseudo_chans[5];
	case CHAT_LOCAL:
		return pseudo_chans[6];
	case CHAT_PERSONAL:
		return pseudo_chans[7];
	case CHAT_GM:
		return pseudo_chans[8];
	case CHAT_SERVER:
		return pseudo_chans[9];
	case CHAT_MOD:
		return pseudo_chans[10];
	case CHAT_DEV:
		return pseudo_chans[11];
	case CHAT_COORD:
		return pseudo_chans[12];
	case CHAT_COMBAT:
...
void update_tab_bar(text_message *msg) {
	...
	if (tab_bar_win_1 < 0 && tab_bar_win_2 < 0) {
		return;
	}
