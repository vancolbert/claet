	...
	for (i = 0; i < cache->num_items; i++) {
		...
		if (item) {
			...
			if (cache == cache_system) {
				...
				safe_snprintf(str, sizeof(str), "%s %6d%c - %d: %s (%d %s)", cache_size_str, size, scale, i, item->name, temp->num_items, cache_items_str);
			} else {
				safe_snprintf(str, sizeof(str), "%s %6d%c - %d: %s", cache_size_str, size, scale, i, item->name);
			}
...
void cache_system_maint() {
	if (!cur_time || !cache_system || !cache_system->time_limit || cache_system->LRU_time + cache_system->time_limit > cur_time) {
		return;
	}
...
static Uint32 cache_system_clean() {
	...
	Sint32 i;
	Uint32 mem_freed = 0;
	if (!cache_system || !cache_system->time_limit || !cache_system->cached_items) {
		return 0;
	}
...
cache_struct *cache_init(const char *name, Uint32 max_items, void (*free_item)()) {
	...
	if (cache_system) {
		cache_add_item(cache_system, name, cache, sizeof(cache_struct) + max_items * sizeof(cache_item_struct *));
	}
...
static Uint32 cache_clean(cache_struct *cache) {
	...
	for (i = cache->num_items - 1; i >= 0; i--) {
		...
		if (item && item->cache_item) {
			...
			if (item->access_count == 0 && item->access_time + cache->time_limit < cur_time) {
...
cache_item_struct *cache_find(cache_struct *cache, const char *name) {
	...
	if (cache->recent_item && cache->recent_item->name && strcmp(cache->recent_item->name, name) == 0) {
		cache_use(cache->recent_item);
		return cache->recent_item;
	}
	item = bsearch(name, cache->cached_items, cache->num_items, sizeof(cache_item_struct *), cache_item_cmp_str);
	if (!item) {
		return NULL;
	}
	cache_use(*item);
...
cache_item_struct *cache_add_item(cache_struct *cache, const char *name, void *item, Uint32 size) {
	...
	for (i = 0; i < cache->num_items; i++) {
		if (strcmp(name, cache->cached_items[i]->name) <= 0) {
			memmove(cache->cached_items + (i + 1), cache->cached_items + i, (cache->num_items - i) * sizeof(cache_item_struct *));
			break;
		}
	}
...
void cache_set_name(cache_struct *cache, const char *name, void *item) {
	...
	if (item_ptr) {
		item_ptr->name = name;
		qsort(cache->cached_items, cache->num_items, sizeof(item_ptr), cache_item_cmp);
	}
}
	...
	if (cache->num_items >= 0) {
		...
		if (*ci == item) {
			...
			while (ci-- > cache->cached_items) {
				if (*ci == item) {
					free(*ci);
					memmove(ci, ci + 1, (cache->num_items - (ci - cache->cached_items)) * sizeof(cache_item_struct *));
					break;
				}
			}
		}
