...
void send_login_info() {
	...
	for (i = 0; i < len; i++) {
		str[i + 1] = username_str[i];
	}
	str[i + 1] = ' ';
	i++;
	len = strlen(password_str);
	for (j = 0; j < len; j++) {
		str[i + j + 1] = password_str[j];
	}
	str[i + j + 1] = 0;
	len = strlen((char *)str);
...
void process_message_from_server(const Uint8 *in_data, int data_length) {
	...
	case INVENTORY_ITEM_TEXT:
		...
		if (!(get_show_window(items_win) || get_show_window(manufacture_win) || get_show_window(trade_win))) {
			...
			if (strncmp(inventory_item_string + 1, "Tu commences à travailler en s", 30) && strncmp(inventory_item_string + 1, "Tu as créé", 10)) {
		...
		if (my_strncompare(inventory_item_string + 1, "Tu as échoué dans la création d'un ", 35)) {
			...
			char item_name[128];
			const char *item_string = &inventory_item_string[36];
			/* look for the ending, if found use it to locate the item name */
			char *located = strstr(item_string, ", et tu as perdu les ingrédients !");
			if (located) {
		...
		if (my_strncompare(inventory_item_string + 1, "Tu as échoué dans la création d'une ", 36)) {
			size_t item_name_len = 0;
			char item_name[128];
			const char *item_string = &inventory_item_string[37];
			/* look for the ending, if found use it to locate the item name */
			char *located = strstr(item_string, ", et tu as perdu les ingrédients !");
			if (located) {
				item_name_len = (size_t)((located - item_string) / sizeof(char));
		...
		if (my_strncompare(inventory_item_string + 1, "Tu as échoué dans l'invocation d'un ", 36)) {
			...
			const char *item_string = &inventory_item_string[37];
			/* look for the ending, if found use it to locate the item name */
			char *located = strstr(item_string, ", les ingrédients sont perdus...");
			if (located) {
				item_name_len = (size_t)((located - item_string) / sizeof(char));
			}
			/* if there was no match then its not a crit fail string */
		...
		if (my_strncompare(inventory_item_string + 1, "Tu as échoué dans l'invocation d'une ", 37)) {
			size_t item_name_len = 0;
			char item_name[128];
			const char *item_string = &inventory_item_string[38];
			/* look for the ending, if found use it to locate the item name */
			char *located = strstr(item_string, ", les ingrédients sont perdus...");
			if (located) {
				item_name_len = (size_t)((located - item_string) / sizeof(char));
...
#define MATCH_PREFIX(s, p) (0 == strncmp(s, p, sizeof(p) - 1))
	...
	case PLAY_SOUND:
		...
		if (sound_on) {
			add_server_sound(SDL_SwapLE16(*((short *)(in_data + 3))), SDL_SwapLE16(*((short *)(in_data + 5))), SDL_SwapLE16(*((short *)(in_data + 7))), 1.0f);
		}
		break;
	case TELEPORT_OUT:
		if (data_length <= 6) {
	...
	case THUNDER:
		...
		if (show_weather) {
			weather_add_lightning(rand() % 5, -camera_x + (50.0 + rand() % 101) * (rand() % 2 ? 1.0 : -1.0), -camera_y + (50.0 + rand() % 101) * (rand() % 2 ? 1.0 : -1.0));
		}
		break;
	case SEND_WEATHER:
	...
	case GET_ROCHE_LIST: {
		Uint16 roches_no;
		if (data_length <= 3) {
			LOG_ERROR("CAUTION: Possibly forged GET_ROCHE_LIST packet received.\n");
			break;
		}
		roches_no = in_data[3];
		if (data_length <= roches_no * 5 + 3) {
			LOG_ERROR("CAUTION(2): Possibly forged GET_ROCHE_LIST packet received.\n");
			break;
		}
	...
	case NPC_SAY_OVERTEXT: {
		...
		add_displayed_text_to_actor(get_actor_ptr_from_id(SDL_SwapLE16(*((Uint16 *)(in_data + 3)))), buf);
		break;
	}
	case PING_REQUEST:
	...
	case DISPLAY_COORD:
		...
		tab_help_win = -1;
		tab_help_collection_id = 17;
		if (in_data[3] & drapeau_am) {
			show_am = 1;
		} else {
			show_am = 0;
		}
	...
	case SEND_SPECIAL_EFFECT:
		...
		if ((in_data[3] == SPECIAL_EFFECT_POISON) || (in_data[3] == SPECIAL_EFFECT_REMOTE_HEAL) || (in_data[3] == SPECIAL_EFFECT_HARM) || (in_data[3] == SPECIAL_EFFECT_MANA_DRAIN) || (in_data[3] == SPECIAL_EFFECT_INVASION_BEAMING) || (in_data[3] == SPECIAL_EFFECT_TELEPORT_TO_RANGE)) {
			if (data_length <= 7) {
	...
	case GET_ACTOR_TITRE:
		if (data_length <= 3) {
			LOG_ERROR("ATTENTION : paquet perdu lors de la réception de GET_ACTOR_TITRE.\n");
		}
		ajout_titre((char *)&in_data[3], data_length - 3);
		break;
	case SEND_IMAGE:
		if (data_length <= 3) {
			LOG_ERROR("ATTENTION : paquet perdu lors de la réception de SEND_IMAGE.\n");
		}
		affiche_image((char *)(in_data + 3));
	case SEND_DATE:
		if (data_length <= 3) {
			LOG_ERROR("Attention ; paquet perdu lors de la réception de SEND_DATE.\n");
		}
