	...
	if (level > 0) {
		...
		while (1) {
			...
			for (i = 0; i < windows_list.num_windows; i++) {
				...
				if ((windows_list.window[i].flags & ELW_SWITCHABLE_OPAQUE) && (last_opaque_window_backgrounds != opaque_window_backgrounds)) {
			...
			for (i = 0; i < windows_list.num_windows; i++) {
				...
				if (windows_list.window[i].displayed > 0) {
					...
					if (windows_list.window[i].order == id) {
						...
						if (first_win < 0 && mouse_in_window(i, mx, my)) {
							first_win = i;
						}
					} else if (windows_list.window[i].order < id && windows_list.window[i].order > next_id) {
						// try to find the next level
...
int     select_window_with(int win_id, int raise_parent, int raise_children) {
	...
	if (windows_list.window[win_id].order < 0) {
		return 0;
	}
...
int     create_window(const char *name, int pos_id, Uint32 pos_loc, int pos_x, int pos_y, int size_x, int size_y, Uint32 property_flags) {
	...
	if (win_id >= 0) {
		...
		win->back_color[3] = couleur_fond_fenetre.alpha;
		win->border_color[0] = couleur_bord_fenetre.rouge;
		win->border_color[1] = couleur_bord_fenetre.vert;
		win->border_color[2] = couleur_bord_fenetre.bleu;
		win->border_color[3] = couleur_bord_fenetre.alpha;
		win->line_color[0] = couleur_ligne_fenetre.rouge;
...
int     init_window(int win_id, int pos_id, Uint32 pos_loc, int pos_x, int pos_y, int size_x, int size_y) {
	...
	if (pos_id >= 0) {
		if (pos_id >= windows_list.num_windows) {
			return -1;
		}
		if (windows_list.window[pos_id].window_id != pos_id) {
			return -1;
		}
	}
	...
	if (windows_list.window[win_id].flags & ELW_SCROLLABLE) {
		...
		widget_set_color(win_id, windows_list.window[win_id].scroll_id, windows_list.window[win_id].border_color[0], windows_list.window[win_id].border_color[1], windows_list.window[win_id].border_color[2]);
	}
...
int     draw_window_title(window_info *win) {
	...
	if ((win->flags & ELW_TITLE_BAR) == ELW_TITLE_NONE) {
		return 0;
	}
	/* draw the help text if the mouse is over the title bar */
	if (show_help_text && cm_valid(win->cm_id) && (cm_window_shown() == CM_INIT_VALUE) && mouse_x > win->cur_x && mouse_x < win->cur_x + win->len_x && mouse_y > win->cur_y - ELW_TITLE_HEIGHT && mouse_y < win->cur_y) {
...
int     draw_window_border(window_info *win) {
	...
	if (win->flags & ELW_CLOSE_BOX) {
		...
		glLineWidth(1.0f);
		glBegin(GL_LINES);
		glVertex2i(win->len_x - ELW_BOX_SIZE + 4, 4);
		glVertex2i(win->len_x - 5, ELW_BOX_SIZE - 5);
		glVertex2i(win->len_x - 4, 4);
...
static void resize_scrollbar(window_info *win) {
	...
	widget_resize(win->window_id, win->scroll_id, widget_get_width(win->window_id, win->scroll_id), sblen);
...
void resize_window(int win_id, int new_width, int new_height) {
	...
	win = &(windows_list.window[win_id]);
	if (new_width < win->min_len_x) {
		new_width = win->min_len_x;
	}
	if (new_height < win->min_len_y) {
		new_height = win->min_len_y;
	}
	win->len_x = new_width;
...
int     mouse_in_window(int win_id, int x, int y) {
	...
	if (x < windows_list.window[win_id].cur_x || x >= windows_list.window[win_id].cur_x + windows_list.window[win_id].len_x) {
		return 0;
	}
	if (y < windows_list.window[win_id].cur_y - ((windows_list.window[win_id].flags & ELW_TITLE_BAR)?ELW_TITLE_HEIGHT:0) || y >= windows_list.window[win_id].cur_y + windows_list.window[win_id].len_y) {
		return 0;
	}
	return 1;
}
...
int     keypress_in_window(int win_id, int x, int y, Uint32 key, Uint32 unikey) {
	...
	int scroll_pos = 0;
	widget_list *W;
	if (win_id < 0 || win_id >= windows_list.num_windows || windows_list.window[win_id].window_id != win_id) {
		return -1;
	}
	win = &windows_list.window[win_id];
	W = win->widgetlist;
