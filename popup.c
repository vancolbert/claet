...
static void flowing_text_perform_flow(flowing_text_t *text, int max_length) {
	float text_width;
	text->lines = reset_soft_breaks(text->str, strlen(text->str), text->str_size_allocated, popup_font_zoom, max_length, NULL, &text_width);
 ...
 * \returns The new popup option structure, or NULL if some error
 */
static popup_option_t *popup_option_create(const char *const text, popup_option_group_t group, popup_option_value_t value, popup_option_type_t type) {
 ...
 */
popup_grouped_option_t *popup_find_or_create_group(popup_t *this_popup, popup_option_group_t id, popup_option_type_t type) {
 ...
 * \returns Nothing
 */
void popup_add_option(popup_t *this_popup, popup_option_group_t group_id, const char *const text, popup_option_value_t value) {
	popup_option_t *new_popup_option = popup_option_create(text, group_id, value, OPTION_TYPE_TEXTOPTION);
	popup_grouped_option_t *this_group = popup_find_or_create_group(this_popup, group_id, OPTION_TYPE_TEXTOPTION);
	if (NULL != new_popup_option) {
 ...
 * \returns Nothing
 */
static void popup_add_option_extended(popup_t *this_popup, popup_option_group_t group_id, const char *const text, popup_option_value_t value, popup_option_type_t type) {
 ...
 * \returns Nothing
 */
void popup_add_option_radio(popup_t *this_popup, popup_option_group_t group_id, const char *const text, popup_option_value_t value) {
 ...
 * \returns Nothing
 */
void popup_add_option_textentry(popup_t *this_popup, popup_option_group_t group_id, const char *const text) {
 ...
 * \returns Nothing
 */
void popup_add_option_displaytext(popup_t *this_popup, popup_option_group_t group_id, const char *const text) {
	popup_option_value_t dummy_value;
...
static __inline__ int is_mouse_over(window_info *win, int x, int y, int w, int h) {
	return mouse_x > win->cur_x + x && mouse_x < (win->cur_x + x + w) && mouse_y > y + win->cur_y && mouse_y < (y + win->cur_y + h);
}
...
static void popup_create_window(popup_t *this_popup) {
	list_node_t *group;
	POPUP_FUNC_ENTER;
	this_popup->win = create_window(this_popup->title, game_root_win, 0, popup_position_x, popup_position_y, this_popup->width, this_popup->height, ELW_WIN_DEFAULT);
	...
	if (this_popup->has_send_button) {
		this_popup->button_widget_id = button_add(this_popup->win, NULL, button_send, this_popup->width / 2 - button_width / 2, this_popup->height - button_height - POPUP_BOTTOM_MARGIN);
		widget_set_OnClick(this_popup->win, this_popup->button_widget_id, &popup_send_button_clicked);
	}
	/* Place text entries */
	list_for_each_node(group, this_popup->grouped_options) {
		if ( POPUP_GROUP_NODE(group)->type == OPTION_TYPE_TEXTENTRY ) {
			...
			if (option) { /* Just not to crash */
				option->widget_id = pword_field_add_extended(this_popup->win, POPUP_GROUP_NODE(group)->group_id, NULL, POPUP_TOP_TEXT_LEFT_MARGIN, option->computed_y_pos + option->text.height, this_popup->width - POPUP_TOP_TEXT_LEFT_MARGIN - POPUP_TOP_TEXT_RIGHT_MARGIN, POPUP_TEXTENTRY_HEIGHT, P_TEXT, popup_font_zoom, 0.77f, 0.57f, 0.39f, (unsigned char *)option->value.str, 256);
			}
		}
	}
	POPUP_FUNC_LEAVE;
