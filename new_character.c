...
// Liste des cadrages camera prédéfinis en fonction de la carte
struct cadrage {
	int rx;
	int ry;
	int rz;
...
static struct input_text {
	...
	// pourquoi mettre un nom par défaut ? (utilisé uniquement dans le textfield)
	{"", 0},
	{"", 0},
	{"", 0}
};
...
// Application du cadrage camera prédéfinis selon la race/peuple
void recadrage_camera() {
	...
	ry = cadrages[race].ry;
	rz = cadrages[race].rz;
	// on applique un zoom par défaut s'il est trop large pour éviter d'être derrière le décor
	if (zoom_level > 2.0f) {
		zoom_level = 2.0f;
	}
}
void change_actor(void) {
	...
	if (our_actor.our_model) {
		...
		model_attach_mesh(our_actor.our_model, actors_defs[our_actor.race].boots[our_actor.boots].mesh_index);
		...
		our_actor.our_model->body_parts->boots_meshindex = actors_defs[our_actor.race].boots[our_actor.boots].mesh_index;
...
void toggle_book(int id) {
	...
	// On force l'affichage du livre, même si la fenêtre doit rester en-dessous.
...
int click_done_handler(widget_list *w, int mx, int my, Uint32 flags) {
	if (w->window_id == color_race_win) {
		 ...
		 * effectue les controles et dispose de son propre système de confirmation.
...
int keypress_namepass_handler(window_info *win, int mx, int my, Uint32 key, Uint32 unikey) {
	...
	// puisque le délai d'affichage des messages est désactivé (cf display_namepass_handler)
	...
	if ((ret = check_character(active > 0, ch))) {
		if (ret == -1) {
			...
			if (active == 0 && !strcasecmp(t->str, "player")) {
				t->pos = 0;
			}
...
int init_namepass_handler(window_info *win) {
	...
	y += 20 + sep;
	label_add_extended(win->window_id, free_widget_id++, 0, 20, y, 0, very_small, 1.0f, 0.8f, 0.6f, "Il est conseillé de choisir");
	y += DEFAULT_FONT_Y_LEN * very_small;
	label_add_extended(win->window_id, free_widget_id++, 0, 20, y, 0, very_small, 1.0f, 0.8f, 0.6f, "un nom s'intégrant dans un");
	y += DEFAULT_FONT_Y_LEN * very_small;
	label_add_extended(win->window_id, free_widget_id++, 0, 20, y, 0, very_small, 1.0f, 0.8f, 0.6f, "monde médiéval fantastique.");
	y += DEFAULT_FONT_Y_LEN * very_small;
	y += 8 * sep;
	...
	y += size + sep;
	label_add_extended(win->window_id, free_widget_id++, 0, 20, y, 0, very_small, 1.0f, 0.8f, 0.6f, "(évite de choisir un mot de");
	y += DEFAULT_FONT_Y_LEN * very_small;
	label_add_extended(win->window_id, free_widget_id++, 0, 20, y, 0, very_small, 1.0f, 0.8f, 0.6f, "passe trop facile à deviner)");
	y += DEFAULT_FONT_Y_LEN * very_small;
	y += sep;
...
static void update_boots(void) {
	...
	my_strncp(our_actor.our_model->body_parts->boots_tex, actors_defs[our_actor.race].boots[our_actor.boots].boots_name, sizeof(our_actor.our_model->body_parts->boots_tex));
	// If we need a new mesh, drop the old one and load it.
	if (actors_defs[our_actor.race].boots[our_actor.boots].mesh_index != our_actor.our_model->body_parts->boots_meshindex) {
		model_detach_mesh(our_actor.our_model, our_actor.our_model->body_parts->boots_meshindex);
		model_attach_mesh(our_actor.our_model, actors_defs[our_actor.race].boots[our_actor.boots].mesh_index);
		our_actor.our_model->body_parts->boots_meshindex = actors_defs[our_actor.race].boots[our_actor.boots].mesh_index;
	}
	change_enhanced_actor(our_actor.our_model->texture_id, our_actor.our_model->body_parts);
...
int mouseover_color_race_handler(window_info *win, int mx, int my) {
	if (!(newchar_mouseover == book_eldo)) {
		image_set_uv(win->window_id, book_eldo, (float)0 / 256, 1.0f - (float)64 / 256, (float)31 / 256, 1.0f - (float)95 / 256);
	}
	if (!(newchar_mouseover == book_haut_elfe)) {
		image_set_uv(win->window_id, book_haut_elfe, (float)0 / 256, 1.0f - (float)64 / 256, (float)31 / 256, 1.0f - (float)95 / 256);
	}
	if (!(newchar_mouseover == book_dwarf)) {
		image_set_uv(win->window_id, book_dwarf, (float)0 / 256, 1.0f - (float)64 / 256, (float)31 / 256, 1.0f - (float)95 / 256);
	}
	if (!(newchar_mouseover == book_orchan)) {
		image_set_uv(win->window_id, book_orchan, (float)0 / 256, 1.0f - (float)64 / 256, (float)31 / 256, 1.0f - (float)95 / 256);
	}
	if (!(newchar_mouseover == book_sinan)) {
		image_set_uv(win->window_id, book_sinan, (float)0 / 256, 1.0f - (float)64 / 256, (float)31 / 256, 1.0f - (float)95 / 256);
	}
	if (!(newchar_mouseover == book_elfe_noir)) {
		image_set_uv(win->window_id, book_elfe_noir, (float)0 / 256, 1.0f - (float)64 / 256, (float)31 / 256, 1.0f - (float)95 / 256);
	}
	if (!(newchar_mouseover == book_gnome)) {
		image_set_uv(win->window_id, book_gnome, (float)0 / 256, 1.0f - (float)64 / 256, (float)31 / 256, 1.0f - (float)95 / 256);
	}
	if (!(newchar_mouseover == book_draegoni)) {
...
int init_color_race_handler(window_info *win) {
	...
	y++;
	y += 4 * 20 + 6 * sep;
	label_add_extended(win->window_id, free_widget_id++, 0, 15, y, 0, very_small, 1.0f, 0.8f, 0.6f, "Clique sur les icones à droite");
	y += DEFAULT_FONT_Y_LEN * very_small;
	label_add_extended(win->window_id, free_widget_id++, 0, 15, y, 0, very_small, 1.0f, 0.8f, 0.6f, "de chaque choix pour découvrir");
	y += DEFAULT_FONT_Y_LEN * very_small;
