...
int url_command(const char *text, int len) {
	...
	if (!strlen(text)) {
		...
		while (local_head != NULL) {
			...
			safe_snprintf(out_str, new_len, "%c %d) %s (vu %d fois) (%s)", ((local_head == active_url) ?'>':' '), ++irl_num, ((URLDATA *)local_head->data)->text, ((URLDATA *)local_head->data)->seen_count, ((((URLDATA *)local_head->data)->visited) ?"visité":"non visité"));
	...
	while (next_start < len) {
		...
		if (first_found < len - next_start) {
			...
			for (next_start = url_start; next_start < len; next_start++) {
				...
				if (!cur_char || cur_char == ' ' || cur_char == '\n' || cur_char == '<' || cur_char == '>' || cur_char == '|' || cur_char == '"' || cur_char == '\'' || cur_char == '`' || cur_char == ']' || cur_char == ';' || cur_char == '\\' || (cur_char & 0x80) != 0) {
					break;
				}
			}
...
static int display_url_handler(window_info *win) {
	...
	if (have_url_count) {
		...
		while (local_head != NULL) {
			...
			if ((mouse_y >= win->cur_y + currenty - url_win_line_step) && (mouse_y < win->cur_y + currenty) && (mouse_x >= win->cur_x + (int)url_win_sep) && (mouse_x - (int)url_win_sep <= win->cur_x + url_win_max_string_width)) {
			...
			if (highlight_url) {
				...
				safe_snprintf(full_help_text, full_help_len, "%s (vu %d fois) (%s)", ((URLDATA *)local_head->data)->text, ((URLDATA *)local_head->data)->seen_count, ((((URLDATA *)local_head->data)->visited)?"visité":"non visité"));
...
static int context_url_handler(window_info *win, int widget_id, int mx, int my, int option) {
	if (cm_url) {
		...
		case 2:
			((URLDATA *)cm_url->data)->visited = 1;
			break;
		case 3:
			((URLDATA *)cm_url->data)->visited = 0;
			break;
		case 5:
			delete_current_url(cm_url);
			break;
		case 7:
			destroy_url_list();
			break;
...
void fill_url_window(void) {
	...
	clear_all_button = button_add_extended(url_win, clear_all_button, NULL, url_win_sep, url_win_sep, 0, 0, 0, 0.75, 0.77f, 0.57f, 0.39f, "Tout vider");
	...
	url_scroll_id = vscrollbar_add_extended(url_win, url_scroll_id, NULL, url_win_x_len - scroll_width, url_win_text_start_y, scroll_width, url_win_text_len_y, 0, url_win_text_len_y / url_win_line_step, 0.77f, 0.57f, 0.39f, 0, 1, have_url_count);
