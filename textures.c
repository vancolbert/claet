...
#include "hash.h"
#include <png.h>
#if !defined(png_set_gray_1_2_4_to_8) && defined(PNG_LIBPNG_VER_MAJOR) && defined(PNG_LIBPNG_VER_MINOR) && (PNG_LIBPNG_VER_MAJOR > 1 || (PNG_LIBPNG_VER_MAJOR == 1 && PNG_LIBPNG_VER_MINOR >= 4))
...
static GLuint build_texture(image_t *image, const Uint32 wrap_mode_repeat, const GLenum min_filter, const Uint32 af, const texture_format_type format) {
	void *ptr;
	GLuint id;
	GLenum src_format = 0, type = 0, internal_format = 0;
	...
	case ift_ati1:
		...
		break;
	case ift_ati2:
		if ((!have_extension(ext_texture_compression_latc)) && have_extension(ati_texture_compression_3dc)) {
	...
	if ((!have_extension(ext_texture_compression_s3tc)) && ((compressed & tct_s3tc) == tct_s3tc)) {
		LOG_ERROR("Can't use s3tc compressed source formats because "
			  "GL_EXT_texture_compression_s3tc is not supported.");
		return 0;
	}
	if ((!have_extension(ext_texture_compression_latc)) && ((compressed & tct_latc) == tct_latc)) {
		LOG_ERROR("Can't use s3tc compressed source formats because "
			  "GL_EXT_texture_compression_latc is not supported.");
		return 0;
	}
	if ((!have_extension(ati_texture_compression_3dc)) && ((compressed & tct_3dc) == tct_3dc)) {
		LOG_ERROR("Can't use s3tc compressed source formats because "
	...
	if (compressed == 0) {
		...
		case tft_ati2:
			...
			break;
		default:
			LOG_ERROR("Unsupported texture format (%i)", format);
			return 0;
		}
	}
	...
	if ((!have_extension(ext_texture_compression_s3tc)) && ((compression & tct_s3tc) == tct_s3tc)) {
		LOG_ERROR("Can't use s3tc compressed texture format, because "
			  "GL_EXT_texture_compression_s3tc is not supported.");
		return 0;
	}
	if ((!have_extension(ext_texture_compression_latc)) && ((compression & tct_latc) == tct_latc)) {
		LOG_ERROR("Can't use s3tc compressed texture format, because "
			  "GL_EXT_texture_compression_latc is not supported.");
		return 0;
	}
	if ((!have_extension(ati_texture_compression_3dc)) && ((compression & tct_3dc) == tct_3dc)) {
		LOG_ERROR("Can't use s3tc compressed texture format, because "
			  "GL_ATI_texture_compression_3dc is not supported.");
		return 0;
	}
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, image->mipmaps - 1);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, min_filter);
	if (af != 0) {
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, anisotropic_filter);
	}
	width = image->width;
	height = image->height;
	CHECK_GL_ERRORS();
	for (i = 0; i < image->mipmaps; i++) {
		...
		if (compressed != 0) {
			ELglCompressedTexImage2D(GL_TEXTURE_2D, i, internal_format, width, height, 0, image->sizes[i], ptr);
		} else {
			glTexImage2D(GL_TEXTURE_2D, i, internal_format, width, height, 0, src_format, type, ptr);
		}
		CHECK_GL_ERRORS();
		if (width > 1) {
			width /= 2;
...
Uint32 load_texture_cached(const char *file_name, const texture_type type) {
	...
	idxp = bsearch(&idf, texture_cache_sorted, texture_handles_used, sizeof(Uint32), cache_cmp_identifier);
	if (idxp) {
		return *idxp;
	}
	if (texture_handles_used < TEXTURE_CACHE_MAX) {
		...
		for (i = 0; i < texture_handles_used; i++) {
			...
			if (idf.hash < texture_handles[idx].hash || (idf.hash == texture_handles[idx].hash && strcasecmp(idf.file_name, texture_handles[idx].file_name) <= 0)) {
				memmove(texture_cache_sorted + (i + 1), texture_cache_sorted + i, (texture_handles_used - i) * sizeof(Uint32));
				break;
			}
		}
		texture_cache_sorted[i] = slot;
		safe_strncpy(texture_handles[slot].file_name, idf.file_name, sizeof(texture_handles[slot].file_name));
		texture_handles[slot].hash = idf.hash;
		texture_handles[slot].type = type;
		texture_handles[slot].id = 0;
		texture_handles[slot].cache_ptr = cache_add_item(texture_cache, texture_handles[slot].file_name, &texture_handles[slot], 0);
...
static Uint32 load_texture(texture_cache_t *texture_handle) {
	...
	if (load_image_data(texture_handle->file_name, compression, 0, strip_mipmaps, base_level, &image) == 0) {
		texture_handle->load_err = 1;
		LOG_ERROR("Error loading image '%s'", texture_handle->file_name);
		return 0;
	}
...
static Uint32 load_texture_handle(const Uint32 handle) {
	...
	if (load_texture(&texture_handles[handle]) != 0) {
		cache_adj_size(texture_cache, texture_handles[handle].size, &texture_handles[handle]);
		return 1;
	}
	return 0;
}
...
static Uint32 copy_to_coordinates(const image_t *source, const Uint32 x, const Uint32 y, image_t *dest) {
...
static Uint32 copy_to_coordinates_block(const image_t *source, const Uint32 x, const Uint32 y, image_t *dest) {
	Uint32 source_height, source_width, source_offset;
...
static Uint32 copy_to_coordinates_mask2(const image_t *source0, const image_t *source1, const image_t *mask, const Uint32 x, const Uint32 y, image_t *dest, Uint8 *buffer) {
	Uint32 source_height, source_width, source_offset;
	Uint32 dest_width, dest_offset;
	Uint32 i, size;
...
Uint32 load_image_data_file_size(el_file_ptr file, const Uint32 compression, const Uint32 unpack, const Uint32 width, const Uint32 height, image_t *image) {
	Uint32 sw, sh, s, size, base_level;
	if (get_image_information(file, image) == 0) {
		LOG_ERROR("Can't get image information for '%s'!", el_file_name(file));
		return 0;
	}
	s = 1;
	base_level = 0;
	...
	if ((sw != sh) || (popcount(size) != 1)) {
		LOG_ERROR("Invalid mipmap (%d) size <%d, %d> for file '%s' "
			  "<%d, %d>.", size, width, height, el_file_name(file), image->width, image->height);
		return 0;
	}
	while (size > s) {
		base_level++;
	...
	LOG_DEBUG_VERBOSE("Using base level %d for image '%s'.", base_level, el_file_name(file));
	if (load_image_data_file(file, compression, unpack, 1, base_level, image) == 0) {
		LOG_ERROR("Can't load file '%s'.", el_file_name(file));
		return 0;
	}
	return 1;
}
static Uint32 load_to_coordinates(el_file_ptr file, const Uint32 x, const Uint32 y, const Uint32 width, const Uint32 height, const Uint32 use_compressed_image, const Uint32 scale, image_t *dst) {
	image_t image;
	...
	if (use_compressed_image == 1) {
		if (load_image_data_file_size(file, tct_s3tc, 0, tw, th, &image) == 0) {
			LOG_ERROR("Can't load file '%s'.", el_file_name(file));
			return 0;
		}
		if ((image.width != tw) || (image.height != th)) {
			LOG_ERROR("File '%s' has wrong size <%d, %d> instead "
				  "of <%d, %d>.", el_file_name(file), image.width, image.height, tw, th);
			free_image(&image);
			return 0;
		}
		if (((tw % 4) != 0) || ((th % 4) != 0) || ((tx % 4) != 0) || ((ty % 4) != 0)) {
...
static void build_alpha_mask(const Uint8 *source, const Uint32 size, Uint8 *dest) {
	Uint32 i;
	for (i = 0; i < size; i++) {
		dest[i] = source[i * 4 + 3];
	}
}
static Uint32 load_to_coordinates_mask2(el_file_ptr source0, el_file_ptr source1, el_file_ptr mask, const Uint32 x, const Uint32 y, const Uint32 width, const Uint32 height, const Uint32 use_compressed_image, const Uint32 scale, image_t *dest, Uint8 *buffer) {
	...
	ty = y * scale;
	if ((source1 == 0) || (mask == 0)) {
		return load_to_coordinates(source0, x, y, width, height, use_compressed_image, scale, dest);
	}
	...
	result = copy_to_coordinates_mask2(&src0, &src1, &msk, tx, ty, dest, buffer);
	free_image(&src0);
	free_image(&src1);
	free_image(&msk);
	return result;
}
static Uint32 open_for_coordinates_checks(const char *file_name, el_file_ptr *file, const Uint32 width, const Uint32 height, Uint16 *sizes, image_t *image) {
	char buffer[128];
	...
	*file = el_open_custom(buffer);
	...
	if (((image->width % width) != 0) || ((image->height % height) != 0) || (size_x != size_y) || (popcount(size) != 1)) {
		LOG_ERROR("File '%s' has wrong size <%d, %d> instead of "
			  "being 2 ^ x * <%d, %d>.", el_file_name(*file), image->width, image->height, width, height);
		el_close(*file);
		*file = 0;
		return 0;
	}
...
static Uint32 open_for_coordinates(const char *file_name, el_file_ptr *file, const Uint32 width, const Uint32 height, Uint16 *sizes, image_format_type *format) {
	image_t image;
	if (open_for_coordinates_checks(file_name, file, width, height, sizes, &image) == 0) {
		return 0;
	}
...
static Uint32 open_for_coordinates_mask2(const char *source0, const char *source1, const char *mask, el_file_ptr *src0, el_file_ptr *src1, el_file_ptr *msk, const Uint32 width, const Uint32 height, Uint16 *sizes, image_format_type *format) {
	image_t image;
	if ((source1 == 0) || (mask == 0)) {
		return open_for_coordinates(source0, src0, width, height, sizes, format);
	}
	if ((source1[0] == 0) || (mask[0] == 0)) {
		return open_for_coordinates(source0, src0, width, height, sizes, format);
	}
	if (open_for_coordinates_checks(source0, src0, width, height, sizes, &image) == 0) {
		LOG_ERROR("Can't open file '%s'.", source0);
		return 0;
	}
	if (open_for_coordinates_checks(source1, src1, width, height, sizes, &image) == 0) {
		LOG_ERROR("Can't open file '%s'.", source1);
		return 0;
	}
	if (open_for_coordinates_checks(mask, msk, width, height, sizes, &image) == 0) {
	...
	if ((image.format != ift_a8) && (image.alpha == 0)) {
		LOG_ERROR("Mask image '%s' has no alpha channel!", el_file_name(*msk));
		el_close(*src1);
		*src1 = 0;
		el_close(*msk);
		*msk = 0;
...
static void load_enhanced_actor_threaded(const enhanced_actor_images_t *files, image_t *image, Uint8 *buffer) {
	...
	if (files->pants_tex[0]) {
		open_for_coordinates_mask2(files->pants_tex, files->legs_base, files->pants_mask, &pants_tex, &legs_base, &pants_mask, 40, 40, &sizes, &format);
	}
	if (files->boots_tex[0]) {
		open_for_coordinates_mask2(files->boots_tex, files->boots_base, files->boots_mask, &boots_tex, &boots_base, &boots_mask, 39, 40, &sizes, &format);
	}
	if (files->torso_tex[0]) {
		open_for_coordinates_mask2(files->torso_tex, files->body_base, files->torso_mask, &torso_tex, &body_base, &torso_mask, 49, 54, &sizes, &format);
	}
	if (files->arms_tex[0]) {
		open_for_coordinates_mask2(files->arms_tex, files->arms_base, files->arms_mask, &arms_tex, &arms_base, &arms_mask, 40, 40, &sizes, &format);
	}
	if (files->hands_tex[0]) {
		open_for_coordinates_mask2(files->hands_tex, files->hands_tex_save, files->hands_mask, &hands_tex, &hands_tex_save, &hands_mask, 16, 16, &sizes, &format);
	}
	if (files->head_tex[0]) {
		open_for_coordinates_mask2(files->head_tex, files->head_base, files->head_mask, &head_tex, &head_base, &head_mask, 32, 32, &sizes, &format);
	}
	if (files->hair_tex[0]) {
		open_for_coordinates(files->hair_tex, &hair_tex, 34, 48, &sizes, &format);
	}
	if (files->weapon_tex[0]) {
		open_for_coordinates(files->weapon_tex, &weapon_tex, 39, 36, &sizes, &format);
	}
	if (files->shield_tex[0]) {
		open_for_coordinates(files->shield_tex, &shield_tex, 39, 36, &sizes, &format);
	}
	if (files->helmet_tex[0]) {
		open_for_coordinates(files->helmet_tex, &helmet_tex, 39, 14, &sizes, &format);
	}
	if (files->neck_tex[0]) {
		open_for_coordinates(files->neck_tex, &neck_tex, 10, 26, &sizes, &format);
	}
	if (files->cape_tex[0]) {
		open_for_coordinates(files->cape_tex, &cape_tex, 62, 38, &sizes, &format);
	}
	scale = 0;
	for (i = 15; i >= 0; i--) {
		scale = 1 << i;
	...
	alpha = 0;
	if (pants_tex != 0) {
		alpha += load_to_coordinates_mask2(pants_tex, legs_base, pants_mask, 39, 88, 40, 40, use_compressed_image, scale, image, buffer);
	}
	if (boots_tex != 0) {
		alpha += load_to_coordinates_mask2(boots_tex, boots_base, boots_mask, 0, 88, 39, 40, use_compressed_image, scale, image, buffer);
	}
	if (torso_tex != 0) {
		alpha += load_to_coordinates_mask2(torso_tex, body_base, torso_mask, 79, 74, 49, 54, use_compressed_image, scale, image, buffer);
	}
	if (arms_tex != 0) {
		alpha += load_to_coordinates_mask2(arms_tex, arms_base, arms_mask, 0, 48, 40, 40, use_compressed_image, scale, image, buffer);
	}
	if (hands_tex != 0) {
		alpha += load_to_coordinates_mask2(hands_tex, hands_tex_save, hands_mask, 34, 32, 16, 16, use_compressed_image, scale, image, buffer);
	}
	if (head_tex != 0) {
		alpha += load_to_coordinates_mask2(head_tex, head_base, head_mask, 34, 0, 32, 32, use_compressed_image, scale, image, buffer);
	}
	if (hair_tex != 0) {
		alpha += load_to_coordinates(hair_tex, 0, 0, 34, 48, use_compressed_image, scale, image);
	}
	if (weapon_tex != 0) {
		alpha += load_to_coordinates(weapon_tex, 89, 38, 39, 36, use_compressed_image, scale, image);
	}
	if (shield_tex != 0) {
		alpha += load_to_coordinates(shield_tex, 50, 38, 39, 36, use_compressed_image, scale, image);
	}
	if (helmet_tex != 0) {
		alpha += load_to_coordinates(helmet_tex, 40, 74, 39, 14, use_compressed_image, scale, image);
	}
	if (neck_tex != 0) {
		alpha += load_to_coordinates(neck_tex, 40, 48, 10, 26, use_compressed_image, scale, image);
	}
	if (cape_tex != 0) {
		alpha += load_to_coordinates(cape_tex, 66, 0, 62, 38, use_compressed_image, scale, image);
	}
	if (alpha > 0) {
		alpha = 1;
	}
...
Uint32 load_enhanced_actor(const enhanced_actor *actor, const char *name) {
	...
	for (i = 0; i < ACTOR_TEXTURE_CACHE_MAX; i++) {
		if ((handle != ACTOR_TEXTURE_CACHE_MAX) && (i >= max_actor_texture_handles)) {
		...
		if (hash == actor_texture_handles[i].hash) {
			if (memcmp(&actor_texture_handles[i].files, &files, sizeof(files)) == 0) {
		...
		if (handle == ACTOR_TEXTURE_CACHE_MAX) {
			...
			if (memcmp(str, actor_texture_handles[i].name, sizeof(str)) == 0) {
	...
	if (handle < ACTOR_TEXTURE_CACHE_MAX) {
		...
		safe_strncpy2(actor_texture_handles[handle].name, str, sizeof(actor_texture_handles[handle].name), strlen(str));
...
Uint32 bind_actor_texture(const Uint32 handle, char *alpha) {
	...
	if (actor_texture_handles[handle].state == tst_image_loaded) {
		...
		id = build_texture(&actor_texture_handles[handle].image, 0, min_filter, af, format);
		CHECK_GL_ERRORS();
...
void use_ready_actor_texture(const Uint32 handle) {
	...
	actor_texture_handles[handle].id = actor_texture_handles[handle].new_id;
...
void change_enhanced_actor(const Uint32 handle, enhanced_actor *actor) {
	...
	if (hash == actor_texture_handles[handle].hash) {
		if (!memcmp(&actor_texture_handles[handle].files, &files, sizeof(files))) {
			actor_texture_handles[handle].access_time = cur_time;
			CHECK_AND_UNLOCK_MUTEX(actor_texture_handles[handle].mutex);
			return;
		}
...
int load_enhanced_actor_thread(void *done) {
	...
	while (*((Uint32 *)done) == 0) {
		...
		while (actor->state == tst_unloaded) {
			...
			CHECK_AND_LOCK_MUTEX(actor->mutex);
			if ((actor->state == tst_image_loading) || (actor->state == tst_unloaded)) {
				if ((hash == actor->hash) && (!memcmp(&actor->files, &files, sizeof(files)))) {
...
void init_texture_cache() {
	...
	for (i = 0; i < ACTOR_TEXTURE_THREAD_COUNT; i++) {
		actor_texture_threads[i] = SDL_CreateThread(load_enhanced_actor_thread, &actor_texture_threads_done);
	}
}
...
void unload_texture_cache() {
	...
	for (i = 0; i < texture_handles_used; i++) {
		if (texture_handles[i].id != 0) {
			...
			cache_adj_size(texture_cache, -texture_handles[i].size, &texture_handles[i]);
		}
	}
...
// texture_struct *ReadPNGFromFile (const char *filename)
