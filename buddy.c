...
int name_onmouseover_handler(widget_list *widget, int mx, int my) {
	put_small_colored_text_in_box(c_orange1, (unsigned char *)buddy_long_name_str, strlen(buddy_long_name_str), buddy_add_x_len - 10, (char *)description_buffer);
	return 1;
}
int type_onmouseover_handler(widget_list *widget, int mx, int my) {
	put_small_colored_text_in_box(c_orange1, (unsigned char *)buddy_long_type_str, strlen(buddy_long_type_str), buddy_add_x_len - 10, (char *)description_buffer);
	return 1;
}
int delete_onmouseover_handler(widget_list *widget, int mx, int my) {
	put_small_colored_text_in_box(c_orange1, (unsigned char *)buddy_long_delete_str, strlen(buddy_long_delete_str), buddy_change_x_len - 10, (char *)description_buffer);
	return 1;
}
...
static int resize_buddy_handler(window_info *win) {
	...
	if (win->len_y > MAX_BUDDY * 10 + 22) {
		win->len_y = MAX_BUDDY * 10 + 22;
	}
	if (win->len_y < 64) {
		win->len_y = 64;
	}
	...
	if (found != 1) {
		...
		for (i = 0; i < MAX_BUDDY; i++) {
			if (buddy_list[i].type == 0xff) {
				...
				if (++buddy_nb >= MAX_BUDDY) {
					buddy_nb = MAX_BUDDY - 1;
				}
				vscrollbar_set_bar_len(buddy_win, buddy_scroll_id, buddy_nb);
				...
				if (buddy_log_notice == 1) {
					...
					time_t n_time;
					time(&n_time);
					if (difftime(c_time, n_time) > -5.0f) {
						break;
					}
					safe_snprintf(message, sizeof(message), buddy_logon_str, len, name);
					LOG_TO_CONSOLE(c_green1, message);
