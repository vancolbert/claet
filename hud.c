...
enum {	CMH_STATS=0, CMH_STATBARS, CMH_KNOWBAR, CMH_TIMER, CMH_DIGCLOCK, CMH_ANACLOCK, CMH_SECONDS, CMH_FPS,
		CMH_MINIMAP, CMH_SEP1, CMH_SOUND, CMH_MUSIC, CMH_LOCATION, CMH_SEP2, CMH_QUICKBM, CMH_QUICKBP };
...
int show_exp(char *a, int al) {
	int m = 0;
	for (; *a && *a == ' ' && al > 0; ++a, --al);
	for (int i = 0; i < NUM_WATCH_STAT - 1; ++i) {
		struct stats_struct *s = statsinfo + i;
		if (*a && al && strncmp(a, (cstr)s->skillnames->shortname, al) && strncmp(a, (cstr)s->skillnames->name, al)) continue;
		int b = clampi(s->skillattr->base, 0, num_exp_lev - 1);
		Uint64 e = *s->exp, n = *s->next_lev, l = exp_lev[b], d = subgt_u64(n, l);
		cprintf(c_green1, "%12s : niveau %-3u (%3u%%) %12" PRIu64 "/%-12" PRIu64 " suivant dans %" PRIu64 " xp", s->skillnames->name, b, d ? clampi(100 * subgt_u64(e, l) / d, 0, 100) : 100, e, n, subgt_u64(n, e));
		++m;
	}
	if (*a && al && !m) cprintf(c_red1, "Usage : &exp   ou   &exp compétence");
	return 1;
}
...
int	display_stats_bar_handler(window_info *win)
{
    ...
    if(show_attr_boosted)
    {
        ...
        if(your_info.phy.cur-your_info.phy.base >= 5)     {r_stat = 0.2f; g_stat = 1.0f; b_stat = 0.2f;}   //vert
        else if(your_info.phy.cur-your_info.phy.base == 4){r_stat = 0.5f; g_stat = 1.0f; b_stat = 0.2f;}   //vert jauni
        else if(your_info.phy.cur-your_info.phy.base == 3){r_stat = 1.0f; g_stat = 1.0f; b_stat = 0.2f;}   //jaune
        else if(your_info.phy.cur-your_info.phy.base == 2){r_stat = 1.0f; g_stat = 0.5f; b_stat = 0.0f;}   //orange
        else if(your_info.phy.cur-your_info.phy.base == 1){r_stat = 1.0f; g_stat = 0.2f; b_stat = 0.2f;}   //red
        else if(your_info.phy.cur-your_info.phy.base == 0){r_stat = 1.0f; g_stat = 1.0f; b_stat = 1.0f;}   //blanc
        ...
        xOffsetBuff+=50;
        if(your_info.coo.cur-your_info.coo.base >= 5)     {r_stat = 0.2f; g_stat = 1.0f; b_stat = 0.2f;}   //vert
        else if(your_info.coo.cur-your_info.coo.base == 4){r_stat = 0.5f; g_stat = 1.0f; b_stat = 0.2f;}   //vert jauni
        else if(your_info.coo.cur-your_info.coo.base == 3){r_stat = 1.0f; g_stat = 1.0f; b_stat = 0.2f;}   //jaune
        else if(your_info.coo.cur-your_info.coo.base == 2){r_stat = 1.0f; g_stat = 0.5f; b_stat = 0.0f;}   //orange
        else if(your_info.coo.cur-your_info.coo.base == 1){r_stat = 1.0f; g_stat = 0.2f; b_stat = 0.2f;}   //red
        else if(your_info.coo.cur-your_info.coo.base == 0){r_stat = 1.0f; g_stat = 1.0f; b_stat = 1.0f;}   //blanc
        else {r_stat = 1.0f;g_stat = 0.2f;b_stat = 0.2f;}                                                  //red
        draw_string_small_shadowed(xOffsetBuff+10, food_bar_start_y+15, agilite_str, 1, r_stat, g_stat, b_stat, 0.0f, 0.0f, 0.0f);
        xOffsetBuff+=50;
        if(your_info.rea.cur-your_info.rea.base >= 5)     {r_stat = 0.2f; g_stat = 1.0f; b_stat = 0.2f;}   //vert
        else if(your_info.rea.cur-your_info.rea.base == 4){r_stat = 0.5f; g_stat = 1.0f; b_stat = 0.2f;}   //vert jauni
        else if(your_info.rea.cur-your_info.rea.base == 3){r_stat = 1.0f; g_stat = 1.0f; b_stat = 0.2f;}   //jaune
        else if(your_info.rea.cur-your_info.rea.base == 2){r_stat = 1.0f; g_stat = 0.5f; b_stat = 0.0f;}   //orange
        else if(your_info.rea.cur-your_info.rea.base == 1){r_stat = 1.0f; g_stat = 0.2f; b_stat = 0.2f;}   //red
        else if(your_info.rea.cur-your_info.rea.base == 0){r_stat = 1.0f; g_stat = 1.0f; b_stat = 1.0f;}   //blanc
        else {r_stat = 1.0f;g_stat = 0.2f;b_stat = 0.2f;}                                                  //red
        draw_string_small_shadowed(xOffsetBuff+10, food_bar_start_y+15, intel_str, 1, r_stat, g_stat, b_stat, 0.0f, 0.0f, 0.0f);

        xOffsetBuff = window_height;
        if(your_info.wil.cur-your_info.wil.base >= 5)     {r_stat = 0.2f; g_stat = 1.0f; b_stat = 0.2f;}   //vert
        else if(your_info.wil.cur-your_info.wil.base == 4){r_stat = 0.5f; g_stat = 1.0f; b_stat = 0.2f;}   //vert jauni
        else if(your_info.wil.cur-your_info.wil.base == 3){r_stat = 1.0f; g_stat = 1.0f; b_stat = 0.2f;}   //jaune
        else if(your_info.wil.cur-your_info.wil.base == 2){r_stat = 1.0f; g_stat = 0.5f; b_stat = 0.0f;}   //orange
        else if(your_info.wil.cur-your_info.wil.base == 1){r_stat = 1.0f; g_stat = 0.2f; b_stat = 0.2f;}   //red
        else if(your_info.wil.cur-your_info.wil.base == 0){r_stat = 1.0f; g_stat = 1.0f; b_stat = 1.0f;}   //blanc
        else {r_stat = 1.0f;g_stat = 0.2f;b_stat = 0.2f;}                                                  //red
        draw_string_small_shadowed(xOffsetBuff+10, food_bar_start_y+27, volonte_str, 1, r_stat, g_stat, b_stat, 0.0f, 0.0f, 0.0f);
        xOffsetBuff+=50;
        if(your_info.ins.cur-your_info.ins.base >= 5)     {r_stat = 0.2f; g_stat = 1.0f; b_stat = 0.2f;}   //vert
        else if(your_info.ins.cur-your_info.ins.base == 4){r_stat = 0.5f; g_stat = 1.0f; b_stat = 0.2f;}   //vert jauni
        else if(your_info.ins.cur-your_info.ins.base == 3){r_stat = 1.0f; g_stat = 1.0f; b_stat = 0.2f;}   //jaune
        else if(your_info.ins.cur-your_info.ins.base == 2){r_stat = 1.0f; g_stat = 0.5f; b_stat = 0.0f;}   //orange
        else if(your_info.ins.cur-your_info.ins.base == 1){r_stat = 1.0f; g_stat = 0.2f; b_stat = 0.2f;}   //red
        else if(your_info.ins.cur-your_info.ins.base == 0){r_stat = 1.0f; g_stat = 1.0f; b_stat = 1.0f;}   //blanc
        else {r_stat = 1.0f;g_stat = 0.2f;b_stat = 0.2f;}                                                  //red
        draw_string_small_shadowed(xOffsetBuff+10, food_bar_start_y+27, instinct_str, 1, r_stat, g_stat, b_stat, 0.0f, 0.0f, 0.0f);
        xOffsetBuff+=50;
        if(your_info.vit.cur-your_info.vit.base >= 5)     {r_stat = 0.2f; g_stat = 1.0f; b_stat = 0.2f;}   //vert
        else if(your_info.vit.cur-your_info.vit.base == 4){r_stat = 0.5f; g_stat = 1.0f; b_stat = 0.2f;}   //vert jauni
        else if(your_info.vit.cur-your_info.vit.base == 3){r_stat = 1.0f; g_stat = 1.0f; b_stat = 0.2f;}   //jaune
        else if(your_info.vit.cur-your_info.vit.base == 2){r_stat = 1.0f; g_stat = 0.5f; b_stat = 0.0f;}   //orange
        else if(your_info.vit.cur-your_info.vit.base == 1){r_stat = 1.0f; g_stat = 0.2f; b_stat = 0.2f;}   //red
        else if(your_info.vit.cur-your_info.vit.base == 0){r_stat = 1.0f; g_stat = 1.0f; b_stat = 1.0f;}   //blanc
        else {r_stat = 1.0f;g_stat = 0.2f;b_stat = 0.2f;}                                                  //red
...
static u8 hover_exp_bar[16];
int click_stats_bar_handler(window_info *win, int mx, int my, Uint32 flags) {
	for (int i = 0; i < max_disp_stats && watch_this_stats[i] > 0; ++i) {
		if ((flags & ELW_LEFT_MOUSE) && hover_exp_bar[i]) {
			show_exp((char *)statsinfo[watch_this_stats[i] - 1].skillnames->shortname, 3);
			return 1;
		}
	}
	return 0;
...
void draw_exp_display()
{
	...
	for (i=0; i<max_disp_stats; i++)
	{
		if (watch_this_stats[i] > 0)
		{
			...
			if ((hover_exp_bar[i] = (statbar_cursor_x > my_exp_bar_start_x && statbar_cursor_x < my_exp_bar_start_x + stats_bar_len))) {
