...
bool BreathParticle::idle(const Uint64 delta_t) {
	...
	velocity *= std::pow(0.5f, float_time * velocity.magnitude() / 8.0f);
	switch (type) {
	...
	case BreathEffect::FIRE:
		if (state == 0) {
			if ((get_time() - born > (type == BreathEffect::POISON ? 100000 : 400000)) || (pow_randfloat(float_time * 5.0f)) < 0.5) {
				state = 1;
			}
		} else {
			if (alpha < 0.02) {
			...
			alpha *= scalar;
			const coord_t size_scalar = std::pow(0.5f, (float)delta_t / 1500000);
			size = std::min(3.0f, size / size_scalar * 0.125f + size * 0.5f);
		}
		break;
	case BreathEffect::WIND:
		if (state == 0) {
			if ((get_time() - born > 400000) || (pow_randfloat(float_time * 70.0f)) < 0.5) {
				state = 1;
			}
		} else {
			if (alpha < 0.02) {
	...
	case BreathEffect::LIGHTNING:
		if (state == 0) {
			if ((get_time() - born > 400000) || (pow_randfloat(float_time * 10.0f)) < 0.5) {
				state = 1;
			}
		} else {
			if (alpha < 0.02) {
			...
			if ((state == 1) && (alpha < 0.04)) {
				...
				if ((rand() & 0x3F) == 0x3F) {
					...
					Particle
					*p = new BreathParticle(effect, mover, pos, velocity_offset * 3.6, size / 2, 1.0, color[0], color[1], color[2], EC_SHIMMER, LOD, type);
					p->state = 2;
...
BreathSmokeParticle::BreathSmokeParticle(Effect *_effect, ParticleMover *_mover, const Vec3 _pos, const Vec3 _velocity, const coord_t _size, const alpha_t _alpha, TextureEnum _texture, const Uint16 _LOD, const BreathEffect::BreathType _type) :
	Particle(_effect, _mover, _pos, _velocity) {
	texture = _texture;
	type = _type;
	switch (type) {
...
bool BreathSmokeParticle::idle(const Uint64 delta_t) {
	...
	velocity *= std::pow(0.5f, float_time * velocity.magnitude() / 2.0f);
	if (state == 0) {
		...
		const alpha_t alpha_scalar = 1.0 - std::pow(0.5f, (float)delta_t / 1000000);
...
BreathEffect::BreathEffect(EyeCandy *_base, bool *_dead, Vec3 *_pos, Vec3 *_target, std::vector<ec::Obstruction *> *_obstructions, const BreathType _type, const Uint16 _LOD, const percent_t _scale) {
	...
	while ((int)particles.size() < LOD * 64) {
		...
		case FIRE: {
			Particle
			*p = new BreathParticle(this, mover, coords, velocity, 0.5 * size_scalar, 0.6, 0.8 + randcolor(0.2), 0.4 + randcolor(0.4), randcolor(0.4), EC_FLARE, LOD, type);
		...
		case ICE: {
			Particle
			*p = new BreathParticle(this, mover, coords, velocity, 0.5 * size_scalar, 0.6, randcolor(0.4), 0.4 + randcolor(0.4), 0.8 + randcolor(0.2), EC_CRYSTAL, LOD, type);
			if (!base->push_back_particle(p)) {
				return;
			}
			break;
		...
		case POISON: {
			Particle
			*p = new BreathParticle(this, mover, coords, velocity, 0.5 * size_scalar, 0.1, randcolor(0.3), 0.8 + randcolor(0.2), randcolor(0.3), EC_WATER, LOD, type);
			if (!base->push_back_particle(p)) {
				return;
			}
			break;
		...
		case MAGIC: {
			Particle
			*p = new BreathParticle(this, mover, coords, velocity, 0.5 * size_scalar, 0.6, randcolor(1.0), randcolor(1.0), randcolor(1.0), EC_TWINFLARE, LOD, type);
			if (!base->push_back_particle(p)) {
				return;
			}
			break;
		...
		case LIGHTNING: {
			Particle
			*p = new BreathParticle(this, mover, coords, velocity, 0.25 * size_scalar, 0.6, 0.9 + randcolor(0.1), 0.85 + randcolor(0.15), 0.8 + randcolor(0.2), EC_FLARE, LOD, type);
			if (!base->push_back_particle(p)) {
				return;
			}
			break;
		...
		case WIND: {
			Particle
			*p = new BreathParticle(this, mover, coords, velocity, 0.5 * size_scalar, 0.6, 0.8 + randcolor(0.2), 0.8 + randcolor(0.2), 0.8 + randcolor(0.2), EC_FLARE, LOD, type);
			if (!base->push_back_particle(p)) {
				return;
			}
			break;
...
bool BreathEffect::idle(const Uint64 usec) {
	...
	while (count > 0) {
		...
		case POISON: {
			Particle
			*p = new BreathParticle(this, mover, coords, velocity, 0.5 * size_scalar, 0.1, 0.9 + randcolor(0.1), 0.8 + randcolor(0.2), 0.6 + randcolor(0.2), EC_FLARE, LOD, type);
			if (!base->push_back_particle(p)) {
				return true;
			}
			break;
		...
		case WIND: {
			Particle
			*p = new BreathParticle(this, mover, coords, velocity, 0.5 * size_scalar, 0.1, 0.8 + randcolor(0.2), 0.8 + randcolor(0.2), 0.8 + randcolor(0.2), EC_FLARE, LOD, type);
			if (!base->push_back_particle(p)) {
				return true;
			}
			break;
