...
bool SmokeParticle::idle(const Uint64 delta_t) {
	...
	const alpha_t alpha_scalar = 1.0 - std::pow(0.5f, (float)delta_t / (60000000 * sqrt_scale));
	alpha -= alpha_scalar;
	if (alpha < 0.006) {
		return false;
	}
	const coord_t size_scalar = std::pow(0.5f, (float)delta_t / (1500000 * sqrt_scale));
...
bool SmokeEffect::idle(const Uint64 usec) {
	...
	while (count > 0) {
		...
		velocity.y += 0.3;
		Particle
		*p = new SmokeParticle(this, mover, coords, velocity, hue_adjust, saturation_adjust, sqrt_scale, max_size, size_scalar, alpha_scalar);
		if (!base->push_back_particle(p)) {
			count = 0;
			break;
		}
