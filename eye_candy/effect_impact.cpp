...
bool ImpactParticle::idle(const Uint64 delta_t) {
	...
	case ImpactEffect::MAGIC_PROTECTION: {
		const alpha_t scalar = 1.0 - pow_randfloat(float_time * 4.0f);
		alpha -= scalar;
		if (alpha < 0.02) {
			return false;
		}
	...
	case ImpactEffect::MAGIC_IMMUNITY: {
		const alpha_t scalar = 1.0 - pow_randfloat(float_time * 2.0f);
		alpha -= scalar;
		if (alpha < 0.02) {
			return false;
		}
	...
	case ImpactEffect::POISON: {
		const alpha_t scalar = 1.0 - pow_randfloat(float_time * 1.4f);
		alpha -= scalar;
		if (alpha < 0.02) {
			return false;
		}
	...
	case ImpactEffect::BLOOD: {
		const alpha_t scalar = 1.0 - pow_randfloat(float_time * 0.8f);
		alpha -= scalar;
		if (alpha < 0.02) {
			return false;
		}
...
ImpactEffect::ImpactEffect(EyeCandy *_base, bool *_dead, Vec3 *_pos, const Vec3 _angle, const ImpactType _type, const Uint16 _LOD, const float _strength) {
	...
	case MAGIC_PROTECTION:
		...
		for (int i = 0; i < 50 * LOD; i++) {
			...
			velocity += offset;
			Particle
			*p = new ImpactParticle(this, mover, center, velocity, 0.3 * size_scalar, 1.0, 0.7, 0.2, 0.4, EC_SHIMMER, LOD, type);
	...
	case SHIELD:
		...
		for (int i = 0; i < 50 * LOD; i++) {
			...
			velocity += offset;
			Particle
			*p = new ImpactParticle(this, mover, center, velocity, 0.3 * size_scalar, 1.0, 0.9, 0.9, 0.9, EC_SHIMMER, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case MAGIC_IMMUNITY:
		...
		for (int i = 0; i < 50 * LOD; i++) {
			...
			velocity += offset;
			Particle
			*p = new ImpactParticle(this, mover, center, velocity, 0.35 * size_scalar, 1.0, randcolor(1.0), randcolor(1.0), randcolor(1.0), EC_VOID, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
