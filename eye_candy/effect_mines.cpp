...
bool MineParticle::idle(const Uint64 delta_t) {
	...
	case MineEffect::DETONATE_MAGIC_IMMUNITY_REMOVAL: {
		...
		velocity.x *= 0.82;
		velocity.z *= 0.82;
		const alpha_t scalar = pow_randfloat(float_time * 1);
		alpha *= scalar;
		break;
	}
	...
	case MineEffect::DETONATE_MANA_BURNER: {
		...
		const alpha_t scalar = pow_randfloat(float_time * 6);
		alpha *= scalar;
		break;
	}
	...
	case MineEffect::DETONATE_CALTROP_POISON:
		if (age < 700000) {
			...
			const percent_t scalar = std::pow(0.5f, float_time * 1);
			alpha *= scalar;
			if (alpha < 0.01) {
				return false;
			}
...
float MineParticle::get_burn() const {
	if ((type == MineEffect::DETONATE_CALTROP) || (type == MineEffect::DETONATE_CALTROP_POISON) || (type == MineEffect::DETONATE_TRAP) || (((type == MineEffect::DETONATE_TYPE1_SMALL) || (type == MineEffect::DETONATE_TYPE1_MEDIUM) || (type == MineEffect::DETONATE_TYPE1_LARGE)) && (state == 0))) {
		return 1.0f;
	} else {
		return 0.0f;
	}
...
MineEffect::MineEffect(EyeCandy *_base, bool *_dead, Vec3 *_pos, const MineType _type, const Uint16 _LOD) {
	...
	case DETONATE_MAGIC_IMMUNITY_REMOVAL:
		...
		while ((int)particles.size() < LOD * 150) {
			...
			velocity.y = 0.2;
			Particle
			*p = new MineParticle(this, mover, coords, velocity, 0.2, 1.0, 3.0, 3.0, 3.0, EC_SIMPLE, LOD, type);
	...
	case DETONATE_UNINVIZIBILIZER:
		...
		while ((int)particles.size() < LOD * 200) {
			...
			Particle
			*p = new MineParticle(this, mover, coords, velocity, 1.2, 1.0, 3.0, 3.0, 3.0, EC_SIMPLE, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case DETONATE_MANA_DRAINER:
		...
		while ((int)particles.size() < LOD * 50) {
			...
			velocity.normalize(0.2);
			Particle
			*p = new MineParticle(this, mover, coords, velocity, 0.3, 1.0, 0.8, 0.35, 0.7, EC_SIMPLE, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case DETONATE_MANA_BURNER:
		...
		while ((int)particles.size() < LOD * 100) {
			...
			Particle
			*p = new MineParticle(this, mover, coords, velocity, 0.5, 0.5, 0.8, 0.35, 0.7, EC_TWINFLARE, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case DETONATE_TRAP: {
		...
		for (int i = 0; i < LOD * 100; i++) {
			...
			dynamic_cast<OrbitalMover *>(mover)->setParticleData(p, OrbitalParticleData(i, 10, 0.45, 10));
		}
		break;
	}
	...
	case DETONATE_TYPE1_LARGE: {
		spawner = new FilledSphereSpawner(0.1);
		mover = new ParticleMover(this);
		const float scale = (type == DETONATE_TYPE1_SMALL ? 0.75 : type == DETONATE_TYPE1_MEDIUM ? 1.25 : 2.0);
		Vec3 wind;
		wind.randomize();
		wind.normalize(0.25);
		wind.y = 0;
...
MineParticleFire::MineParticleFire(Effect *_effect, ParticleMover *_mover, const Vec3 _pos, const Vec3 _velocity, const coord_t _size, const alpha_t _alpha, const color_t red, const color_t green, const color_t blue, TextureEnum _texture, const Uint16 _LOD) :
...
MineParticleSmoke::MineParticleSmoke(Effect *_effect, ParticleMover *_mover, const Vec3 _pos, const Vec3 _velocity, const coord_t _size, const alpha_t _alpha, const color_t red, const color_t green, const color_t blue, TextureEnum _texture, const Uint16 _LOD) :
	Particle(_effect, _mover, _pos, _velocity, _size) {
	color[0] = red;
	color[1] = green;
	color[2] = blue;
