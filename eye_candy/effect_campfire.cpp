...
CampfireBigParticle::CampfireBigParticle(Effect *_effect, ParticleMover *_mover, const Vec3 _pos, const Vec3 _velocity, const color_t hue_adjust, const color_t saturation_adjust, const float _sqrt_scale, const Uint16 _LOD) :
	Particle(_effect, _mover, _pos, _velocity) {
...
CampfireEffect::CampfireEffect(EyeCandy *_base, bool *_dead, Vec3 *_pos, std::vector<ec::Obstruction *> *_obstructions, const color_t _hue_adjust, const color_t _saturation_adjust, const float _scale, const Uint16 _LOD) {
	...
	for (int i = 0; i < 20; i++) {
		...
		Particle
		*p = new CampfireBigParticle(this, stationary, coords, Vec3(0.0, 0.0, 0.0), hue_adjust, saturation_adjust, sqrt_scale, LOD);
		if (!base->push_back_particle(p)) {
			break;
		}
		big_particles++;
...
bool CampfireEffect::idle(const Uint64 usec) {
	...
	while (((int)particles.size() < LOD * 100) && (pow_randfloat((interval_t)usec / 80000 * LOD) < 0.5)) {
		int state = 0;
		if (rand() & 1) {
			state = 1;
		...
		Particle
		*p = new CampfireParticle(this, mover, coords, velocity, hue_adjust, saturation_adjust, scale, sqrt_scale, state, LOD);
		if (!base->push_back_particle(p)) {
			break;
		}
	}
