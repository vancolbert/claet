...
bool GlowParticle::idle(const Uint64 delta_t) {
	...
	case GlowEffect::LEVEL_UP_TAI_GLOW_R: {
		...
		const alpha_t scalar = 1.0 - pow_randfloat(float_time * 0.75f);
		alpha -= scalar;
		if (alpha < 0.01) {
			return false;
		}
...
GlowEffect::GlowEffect(EyeCandy *_base, bool *_dead, Vec3 *_pos, const GlowType _type, const Uint16 _LOD) {
	...
	case GlowEffect::REMOTE_HEAL_GLOW: {
		...
		for (int i = 0; i < LOD * 64; i++) {
			...
			Particle
			*p = new GlowParticle(this, mover, coords, velocity, 0.65, 0.05, 0.4 + randcolor(0.3), 0.7, 0.2, EC_SHIMMER, LOD, type);
			p->state = 1;
	...
	case GlowEffect::HARM_GLOW: {
		...
		for (int i = 0; i < LOD * 64; i++) {
			...
			coords += effect_center;
			Particle
			*p = new GlowParticle(this, mover, coords, velocity, 0.95, 0.05, 1.0, 0.4 + randcolor(0.3), 0.2, EC_FLARE, LOD, type);
			p->state = 1;
			if (!base->push_back_particle(p)) {
				break;
			}
	...
	case LEVEL_UP_ATT_GLOW:
		...
		for (int i = 0; i < LOD * 32; i++) {
			...
			velocity.y -= 0.7;
			Particle
			*p = new GlowParticle(this, mover, coords, velocity, 8.0, 0.5, 1.0, 0.0, 0.0, EC_SHIMMER, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
		for (int i = 0; i < LOD * 32; i++) {
			...
			velocity.y -= 0.7;
			Particle
			*p = new GlowParticle(this, mover2, coords, velocity, 2.0, 1.0, 1.0, 0.2, 0.2, EC_SHIMMER, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case LEVEL_UP_DEF_GLOW:
		...
		for (int i = 0; i < LOD * 192; i++) {
			...
			Particle
			*p = new GlowParticle(this, mover, coords, velocity, 2.0, 0.5, 0.0, 0.0, 1.0, EC_SHIMMER, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
