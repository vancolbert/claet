...
InnerSummonParticle::InnerSummonParticle(Effect *_effect, ParticleMover *_mover, const Vec3 _pos, const Vec3 _velocity, const coord_t _size, const alpha_t _alpha, const color_t red, const color_t green, const color_t blue, TextureEnum _texture, const Uint16 _LOD) :
	Particle(_effect, _mover, _pos, _velocity,
		 _size * (0.2 + randcoord())) {
	color[0] = red;
	color[1] = green;
...
SummonEffect::SummonEffect(EyeCandy *_base, bool *_dead, Vec3 *_pos, const SummonType _type, const Uint16 _LOD) {
	...
	while ((int)particles.size() < LOD * 5) {
		...
		Particle
		*p = new InnerSummonParticle(this, gravity_mover, coords, velocity, inner_size, inner_alpha, inner_color[0], inner_color[1], inner_color[2], texture, LOD);
		if (!base->push_back_particle(p)) {
			break;
		}
	}
...
bool SummonEffect::idle(const Uint64 usec) {
	...
	if (age < 1000000) {
		...
		while (count > 0) {
			...
			coords.y += 0.05;
			Particle
			*p = new OuterSummonParticle(this, smoke_mover, coords, Vec3(0.0, 0.05, 0.0), outer_size, outer_alpha, outer_color[0], outer_color[1], outer_color[2], LOD);
			if (!base->push_back_particle(p)) {
				count = 0;
				break;
			}
