...
BagParticle::BagParticle(Effect *_effect, ParticleMover *_mover, const Vec3 _pos, const Vec3 _velocity, const coord_t _size) :
...
bool BagParticle::idle(const Uint64 delta_t) {
	...
	if (age > 220000) {
		const alpha_t alpha_scalar = std::pow(0.5f, float_time * 3.0f);
		alpha *= alpha_scalar;
		if (alpha < 0.01) {
			return false;
		}
