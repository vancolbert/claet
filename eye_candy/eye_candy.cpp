...
void Effect::build_particle_buffer(const Uint64 time_diff) {
	...
	if (particle_vertex_buffer.get_size() < size) {
		...
		particle_vertex_buffer.set_size(el::hbt_vertex, size, el::hbut_dynamic_draw);
	}
	particle_vertex_buffer.bind(el::hbt_vertex);
	buffer = static_cast<float *>(particle_vertex_buffer.map(el::hbt_vertex, el::hbat_write_only));
	if (bounds) {
...
void Effect::draw_particle_buffer() {
	...
	glColorPointer(4, GL_FLOAT, 10 * sizeof(float), static_cast<char *>(0) + 0 * sizeof(float));
	glVertexPointer(3, GL_FLOAT, 10 * sizeof(float), static_cast<char *>(0) + 4 * sizeof(float));
	ELglClientActiveTextureARB(GL_TEXTURE0);
	glEnableClientState(GL_TEXTURE_COORD_ARRAY);
	glTexCoordPointer(2, GL_FLOAT, 10 * sizeof(float), static_cast<char *>(0) + 7 * sizeof(float));
	ELglClientActiveTextureARB(GL_TEXTURE1);
	glEnableClientState(GL_TEXTURE_COORD_ARRAY);
	glTexCoordPointer(1, GL_FLOAT, 10 * sizeof(float), static_cast<char *>(0) + 9 * sizeof(float));
...
void Shape::draw() {
	...
	glNormalPointer(GL_FLOAT, 3 * sizeof(float), static_cast<char *>(0) + vertex_count * 3 * sizeof(float));
	glVertexPointer(3, GL_FLOAT, 3 * sizeof(float), static_cast<char *>(0));
...
CaplessCylinder::CaplessCylinder(EyeCandy *_base, const Vec3 _start, const Vec3 _end, const Vec3 _color, const alpha_t _alpha, const coord_t _radius, const int polys) :
	Shape(_base) {
	float *vertices;
	float *normals;
	GLushort *facets;
	...
	if (vertex_buffer.get_size() < size) {
		vertex_buffer.bind(el::hbt_vertex);
		vertex_buffer.set_size(el::hbt_vertex, size, el::hbut_dynamic_draw);
	}
	vertex_buffer.bind(el::hbt_vertex);
	vertices = static_cast<float *>(vertex_buffer.map(el::hbt_vertex, el::hbat_write_only));
	...
	if (index_buffer.get_size() < size) {
		index_buffer.bind(el::hbt_index);
		index_buffer.set_size(el::hbt_index, size, el::hbut_dynamic_draw);
	}
	index_buffer.bind(el::hbt_index);
	facets = static_cast<GLushort *>(index_buffer.map(el::hbt_index, el::hbat_write_only));
	// Add in the sides.
...
Cylinder::Cylinder(EyeCandy *_base, const Vec3 _start, const Vec3 _end, const Vec3 _color, const alpha_t _alpha, const coord_t _radius, const int polys) :
	Shape(_base) {
	float *vertices;
	float *normals;
	GLushort *facets;
...
Sphere::Sphere(EyeCandy *_base, const Vec3 _pos, const Vec3 _color, const alpha_t _alpha, const coord_t _radius, const int polys) :
	Shape(_base) {
	float *vertices;
	float *normals;
	GLushort *facets;
...
CaplessCylinders::CaplessCylinders(EyeCandy *_base, const std::vector<CaplessCylinderItem> &items) {
	...
	for (idx = 0; idx < count; idx++) {
		...
		for (angle_t rad = 0; rad < 2 * PI - 0.0001;
			...
			vertices[vertex_index + i].x = end.x + radius * (cos(rad) * normalized.z + sin(rad) * normalized.y);
			vertices[vertex_index + i].y = end.y + radius * (cos(rad) * normalized.x + sin(rad) * normalized.z);
			vertices[vertex_index + i].z = end.z + radius * (cos(rad) * normalized.y + sin(rad) * normalized.x);
			vertices[vertex_index + i].nx = cos(rad) * normalized.z + sin(rad) * normalized.y;
			vertices[vertex_index + i].ny = cos(rad) * normalized.x + sin(rad) * normalized.z;
			vertices[vertex_index + i].nz = cos(rad) * normalized.y + sin(rad) * normalized.x;
			...
			vertices[vertex_index + i + subdivisions].x = start.x + radius * (cos(rad - radian_increment / 2) * normalized.z + sin(rad - radian_increment / 2) * normalized.y);
			vertices[vertex_index + i + subdivisions].y = start.y + radius * (cos(rad - radian_increment / 2) * normalized.x + sin(rad - radian_increment / 2) * normalized.z);
			vertices[vertex_index + i + subdivisions].z = start.z + radius * (cos(rad - radian_increment / 2) * normalized.y + sin(rad - radian_increment / 2) * normalized.x);
			vertices[vertex_index + i + subdivisions].nx = cos(rad - radian_increment / 2) * normalized.z + sin(rad - radian_increment / 2) * normalized.y;
			vertices[vertex_index + i + subdivisions].ny = cos(rad - radian_increment / 2) * normalized.x + sin(rad - radian_increment / 2) * normalized.z;
			vertices[vertex_index + i + subdivisions].nz = cos(rad - radian_increment / 2) * normalized.y + sin(rad - radian_increment / 2) * normalized.x;
		}
		// Add in the sides.
		for (int i = 0; i < subdivisions; i++) {
...
void CaplessCylinders::draw(const float alpha_scale) {
	...
	glEnableClientState(GL_COLOR_ARRAY);
	ELglMultiTexCoord1f(GL_TEXTURE1, get_texture_coordinate(1.0f));
	glColorPointer(4, GL_UNSIGNED_BYTE, sizeof(CaplessCylindersVertex), static_cast<char *>(0) + 6 * sizeof(float));
	glNormalPointer(GL_FLOAT, sizeof(CaplessCylindersVertex), static_cast<char *>(0) + 3 * sizeof(float));
	glVertexPointer(3, GL_FLOAT, sizeof(CaplessCylindersVertex), static_cast<char *>(0));
	glDrawElements(GL_TRIANGLES, facet_count * 3, GL_UNSIGNED_SHORT, 0);
	glDisableClientState(GL_VERTEX_ARRAY);
	glDisableClientState(GL_NORMAL_ARRAY);
	glDisableClientState(GL_COLOR_ARRAY);
...
CylinderObstruction::CylinderObstruction(Vec3 *_start, Vec3 *_end, const coord_t _max_distance, const coord_t _force) :
...
Vec3 BoxObstruction::get_force_gradient(Particle& p) { // Arbitrary-rotation box.
...
coord_t PolarCoordElement::get_radius(const angle_t angle) const {
	...
	coord_t ret = std::pow(fabsf(temp_cos), power) * scalar + scalar;
	ret = copysign(ret, temp_cos);
	return ret;
}
Particle::Particle(Effect *_effect, ParticleMover *_mover, const Vec3 _pos, const Vec3 _velocity, const coord_t _size) {
	effect = _effect;
	base = effect->base;
...
void Particle::draw(const Uint64 usec) {
	...
	if (effect->motion_blur_points > 0) {
		...
		for (int i = 0; i < effect->motion_blur_points; i++) {
			effect->draw_particle(motion_blur[i].size, motion_blur[i].texture, motion_blur[i].color[0], motion_blur[i].color[1], motion_blur[i].color[2], motion_blur[i].alpha, motion_blur[i].pos, burn);
		}
...
BoundingMover::BoundingMover(Effect *_effect, const Vec3 _center_pos, BoundingRange *_bounding_range, const coord_t _force) :
...
GravityMover::GravityMover(Effect *_effect, Vec3 *_center, const energy_t _mass) :
	GradientMover(_effect) {
	mass = _mass;
...
Vec3 IFSSinusoidalElement::get_new_coords(const Vec3& pos) {
	const Vec3 result(sin(pos.x * scalar.x + offset.x) * scalar2.x,
			  sin(pos.y * scalar.y + offset.y) * scalar2.y, sin(pos.z * scalar.z + offset.z) * scalar2.z);
...
coord_t NoncheckingFilledBoundingSpawner::get_area() const { // Not 100% accurate, but goot enough.  :)
	coord_t avg_radius = 0;
	for (float f = 0; f < 2 * PI; f += (2 * PI / 256.0)) {
		avg_radius += bounding_range->get_radius(f);
	}
...
coord_t HollowBoundingSpawner::get_area() const { // Not 100% accurate, but goot enough.  :)
	coord_t avg_radius = 0;
	for (float f = 0; f < 2 * PI; f += (2 * PI / 256.0)) {
		avg_radius += bounding_range->get_radius(f);
	}
...
void EyeCandy::idle() {
	...
	for (int i = 0; i < (int)effects.size(); ) {
		...
		if (!e->active) {
			if (e->bounds) {
				const angle_t angle = atan2(shifted_pos.x, shifted_pos.z);
				if (distance_squared < MAX_DRAW_DISTANCE_SQUARED + e->bounds->get_radius(angle) && same_cluster) {
					if (EC_DEBUG) {
			...
			if (e->bounds) {
				const angle_t angle = atan2(shifted_pos.x, shifted_pos.z);
				if (distance_squared > e->bounds->get_radius(angle) + MAX_DRAW_DISTANCE_SQUARED || !same_cluster) {
					if (EC_DEBUG) {
