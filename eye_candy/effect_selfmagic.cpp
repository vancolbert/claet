...
bool SelfMagicParticle::idle(const Uint64 delta_t) {
	...
	case SelfMagicEffect::HEAL: {
		...
		if (state == 0) {
			if ((get_time() - born > 500000) || (pow_randfloat(float_time * 5.0f)) < 0.5) {
				state = 1;
			}
		} else {
			if (alpha < 0.04) {
	...
	case SelfMagicEffect::MAGIC_PROTECTION: {
		const interval_t float_time = delta_t / 1000000.0;
		if (state == 0) {
			if ((get_time() - born > 700000) || (pow_randfloat(float_time * 3.0f)) < 0.5) {
				state = 1;
			}
		} else {
			if (alpha < 0.02) {
	...
	case SelfMagicEffect::SHIELD: {
		...
		if (state == 0) {
			...
			const float scalar = std::max(0.0001f, pow_randfloat(float_time * 4.5f));
			if (size < 10) {
				size /= scalar;
			}
			alpha *= square(scalar);
	...
	case SelfMagicEffect::BONES_TO_GOLD: {
		const interval_t float_time = delta_t / 1000000.0;
		if (state == 0) {
			if ((get_time() - born > 500000) || (pow_randfloat(float_time * 6.0f)) < 0.5) {
				for (int i = 0; i < 5; i++) {
					Vec3 new_velocity;
					new_velocity.randomize(1.5);
					new_velocity += velocity;
					Particle
					*p = new SelfMagicParticle(effect, mover, pos, new_velocity, 2.0, 1.0, 1.0, 1.0, 0.5, EC_SHIMMER, LOD, type);
					p->state = 1;
...
SelfMagicEffect::SelfMagicEffect(EyeCandy *_base, bool *_dead, Vec3 *_pos, const SelfMagicType _type, const Uint16 _LOD) {
	...
	case HEAL:
		...
		while ((int)particles.size() < LOD * 96) {
			...
			Particle
			*p = new SelfMagicParticle(this, mover, coords, velocity, 0.7, 0.5, 0.4, 0.7, 0.2, EC_FLARE, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case MAGIC_PROTECTION:
		...
		while ((int)particles.size() < LOD * 96) {
			...
			Particle
			*p = new SelfMagicParticle(this, mover, coords, velocity, 1.1, 1.0, 0.7, 0.2, 0.4, EC_VOID, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case SHIELD:
		...
		while ((int)particles.size() < LOD * 96) {
			...
			velocity.y += 0.7;
			Particle
			*p = new SelfMagicParticle(this, mover, coords, velocity, 2.0, 1.0, 0.5, 0.5, 0.6, EC_SHIMMER, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case HEATSHIELD:
		...
		while ((int)particles.size() < LOD * 48) {
			...
			velocity.y += 0.7;
			Particle
			*p = new SelfMagicParticle(this, mover, coords, velocity, 5.0, 0.9, 1.0, 0.55, 0.05, EC_SHIMMER, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
		while ((int)particles.size() < LOD * 96) {
			...
			velocity.y += 0.7;
			Particle
			*p = new SelfMagicParticle(this, mover2, coords, velocity, 2.0, 1.0, 1.0, 0.55, 0.05, EC_SHIMMER, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case COLDSHIELD:
		...
		while ((int)particles.size() < LOD * 36) {
			...
			velocity.y -= 0.7;
			Particle
			*p = new SelfMagicParticle(this, mover, coords, velocity, 8.0, 0.5, 0.05, 0.50, 0.95, EC_SHIMMER, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
		while ((int)particles.size() < LOD * 96) {
			...
			velocity.y -= 0.7;
			Particle
			*p = new SelfMagicParticle(this, mover2, coords, velocity, 2.0, 1.0, 0.05, 0.50, 0.95, EC_SHIMMER, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case RADIATIONSHIELD:
		...
		while ((int)particles.size() < LOD * 32) {
			...
			velocity.y += 0.3;
			Particle
			*p = new SelfMagicParticle(this, mover, coords, velocity, 4.0, 0.9, 0.05, 1.0, 0.25, EC_SHIMMER, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
		while ((int)particles.size() < LOD * 96) {
			...
			velocity.y += 0.3;
			Particle
			*p = new SelfMagicParticle(this, mover2, coords, velocity, 2.0, 1.0, 0.05, 0.8, 0.25, EC_SHIMMER, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case ALERT:
		...
		while ((int)particles.size() < LOD * 32) {
			...
			coords += effect_center;
			Particle
			*p = new SelfMagicParticle(this, mover, coords, velocity, 7.0, 0.12, 1.0, 1.0, 1.0, EC_VOID, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case BONES_TO_GOLD:
		if (age < 750000) {
			...
			while (count > 0) {
				...
				velocity.y += 2.8;
				Particle
				*p = new SelfMagicParticle(this, mover, coords, velocity, 2.0, 1.0, 0.6, 0.5, 0.5, EC_INVERSE, LOD, type);
				if (!base->push_back_particle(p)) {
					count = 0;
					break;
				}
