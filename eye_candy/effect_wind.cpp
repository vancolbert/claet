...
bool WindParticle::idle(const Uint64 delta_t) {
	...
	const float scalar = std::pow(0.5f, (interval_t)delta_t / divisor);
	...
	if (state == 0) {
		...
		if (radius > max_radius) { // Pass it off to a neighboring effect.
...
void WindEffect::set_pass_off(std::vector<WindEffect *> pass_off_to) {
	...
	if (neighbors.size()) {
		...
		for (int i = 0; i < (int)neighbors.size(); i++) {
			...
			if (end_angle > current->start_angle) { // Narrow down the angles.
	...
	for (int i = count - (int)particles.size(); i >= 0; i--) {
		...
		velocity.y /= 2;
		Particle
		*p = new WindParticle(this, mover, coords, velocity, hue_adjust, saturation_adjust, scalar, center.y + 0.05, center.y + 1.0, type);
...
bool WindEffect::idle(const Uint64 usec) {
	...
	for (int i = count - (int)particles.size(); i >= 0; i--) {
		...
		velocity.y /= 2;
		Particle
		*p = new WindParticle(this, mover, coords, velocity, hue_adjust, saturation_adjust, scalar, center.y + 0.05, center.y + 4.0, type);
		if (!base->push_back_particle(p)) {
			break;
		}
	}
