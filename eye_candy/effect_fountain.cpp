...
FountainParticle::FountainParticle(Effect *_effect, ParticleMover *_mover, const Vec3 _pos, const Vec3 _velocity, const color_t hue_adjust, const color_t saturation_adjust, const coord_t _base_height, const bool _backlight, const float _sqrt_scale, const coord_t _max_size, const coord_t size_scalar) :
...
bool FountainParticle::idle(const Uint64 delta_t) {
	...
	if (state == 0) {
		const float scalar = std::pow(0.5f, (float)delta_t / 2500000);
		alpha *= scalar;
		...
		const float scalar = std::pow(0.5f, (float)delta_t / 200000);
		alpha *= scalar;
		size = std::min(max_size, size / scalar * 0.25f + size * 0.75f);
	}
	return true;
...
bool FountainEffect::idle(const Uint64 usec) {
	...
	while (count > 0) {
		...
		Particle
		*p = new FountainParticle(this, mover, coords, velocity, hue_adjust, saturation_adjust, base_height, backlight, sqrt_scale, max_size, size_scalar);
		if (!base->push_back_particle(p)) {
			count = 0;
			break;
		}
