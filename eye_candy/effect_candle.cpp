...
bool CandleParticle::idle(const Uint64 delta_t) {
	...
	const float scalar = 1.0 - std::pow(0.5f, (interval_t)delta_t * LOD / 42000000.0f);
	alpha -= scalar;
	if (alpha < 0.02) {
		return false;
	}
...
bool CandleEffect::idle(const Uint64 usec) {
	...
	while (((int)particles.size() < LOD * 20) && ((pow_randfloat((LOD * 20 - particles.size()) * (interval_t)usec / 80 / square(LOD)) < 0.5) || ((int)particles.size() < LOD * 10))) {
		...
		Particle
		*p = new CandleParticle(this, mover, coords, velocity, hue_adjust, saturation_adjust, scale, LOD);
