...
public:
enum HarvestingType {
	RADON_POUCH, CAVERN_WALL, MOTHER_NATURE, QUEEN_OF_NATURE, BEES, BAG_OF_GOLD, RARE_STONE, TOOL_BREAK, };
HarvestingEffect(EyeCandy *_base, bool *_dead, Vec3 *_pos, const HarvestingType _type, const Uint16 _LOD);
HarvestingEffect(EyeCandy *_base, bool *_dead, Vec3 *_pos, Vec3 *_pos2, const HarvestingType _type, const Uint16 _LOD);
~HarvestingEffect();
...
public:
HarvestingParticle(Effect *_effect, ParticleMover *_mover, const Vec3 _pos, const Vec3 _velocity, const coord_t _size, const alpha_t _alpha, const color_t red, const color_t green, const color_t blue, TextureEnum _texture, const Uint16 _LOD, const HarvestingEffect::HarvestingType _type);
~HarvestingParticle() {}
