...
bool TeleporterParticle::idle(const Uint64 delta_t) {
	...
	const alpha_t scalar = std::pow(0.5f, (float)delta_t / 800000);
	alpha *= scalar;
	return true;
}
...
bool TeleporterEffect::idle(const Uint64 usec) {
	...
	while (((int)particles.size() < LOD * 50) && (pow_randfloat((float)usec / 100000 * LOD) < 0.5)) {
		const Vec3 coords = spawner->get_new_coords() + *pos + Vec3(0.0, randcoord() * randcoord() * 8.0 * sqrt_LOD, 0.0);
		Vec3 velocity;
		velocity.randomize(0.2);
		Particle
		*p = new TeleporterParticle(this, mover, coords, velocity, hue_adjust, saturation_adjust, size_scalar);
		if (!base->push_back_particle(p)) {
			break;
		}
	}
