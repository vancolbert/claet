...
angle_t angle_to(const Vec3 rhs) const {
	...
	return std::acos(lhs_normal.x * rhs_normal.x + lhs_normal.y * rhs_normal.y + lhs_normal.z * rhs_normal.z);
}
...
bool is_valid() const {
	return is_valid(x) && is_valid(y) && is_valid(z);
}
Vec3 as_el() {
	return Vec3(x, y, z);
}
...
public:
CaplessCylinderItem(const Vec3 _start, const Vec3 _end, const Vec3 _color, const alpha_t _alpha, const coord_t _radius, const int _polys) {
	start = _start;
	end = _end;
	color = _color;
	alpha = _alpha;
...
CaplessCylinders(EyeCandy *_base, const std::vector<CaplessCylinderItem> &items);
...
class Particle {
public:
Particle(Effect *_effect, ParticleMover *_mover, const Vec3 _pos, const Vec3 _velocity, const coord_t _size = 1.0f);
virtual ~Particle();
...
public:
SpiralMover(Effect *_effect, Vec3 *_center, const coord_t _spiral_speed, const coord_t _pinch_rate) :
	GradientMover(_effect) {
	center = _center;
...
public:
IFSSinusoidalElement(const coord_t _scale, const Vec3 _offset, const Vec3 _scalar, const Vec3 _scalar2) :
	IFSParticleElement(_scale) {
	offset = _offset;
	scalar = _scalar;
	scalar2 = _scalar2;
...
public:
IFSSphericalElement(const coord_t _scale, const Vec3 _numerator_adjust, const Vec3 _denominator_adjust) :
	IFSParticleElement(_scale) {
...
public:
IFSRingElement(const coord_t _scale, const Vec3 _numerator_adjust, const Vec3 _denominator_adjust) :
	IFSParticleElement(_scale) {
	numerator_adjust = _numerator_adjust;
	denominator_adjust = _denominator_adjust;
}
...
public:
FilledBoundingSpawner(BoundingRange *_bounding_range, Vec3 *_center, Vec3 *_base_center, float _range_scalar = 1.0) {
...
public:
CappedSimpleCylinderObstruction(Vec3 *_pos, const coord_t _max_distance, const coord_t _force, const coord_t _bottom, const coord_t _top) :
	Obstruction(_max_distance, _force) {
	pos = _pos;
	bottom = _bottom;
	top = _top;
...
public:
SphereObstruction(Vec3 *_pos, const coord_t _max_distance, const coord_t _force) :
	Obstruction(_max_distance, _force) {
	pos = _pos;
}
...
public:
BoxObstruction(const Vec3 _start, const Vec3 _end, Vec3 *_center, float *_sin_rot_x, float *_cos_rot_x, float *_sin_rot_y, float *_cos_rot_y, float *_sin_rot_z, float *_cos_rot_z, float *_sin_rot_x2, float *_cos_rot_x2, float *_sin_rot_y2, float *_cos_rot_y2, float *_sin_rot_z2, float *_cos_rot_z2, const coord_t _force) :
...
void draw_particle(const coord_t size, const Uint32 texture, const color_t r, const color_t g, const color_t b, const alpha_t alpha, const Vec3 pos, const alpha_t burn);
