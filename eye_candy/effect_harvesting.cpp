...
bool HarvestingParticle::idle(const Uint64 delta_t) {
	...
	case HarvestingEffect::MOTHER_NATURE: {
		...
		const alpha_t scalar = pow_randfloat(float_time * 3);
		if (size < 10) {
			size /= scalar;
		}
		alpha *= scalar;
	...
	case HarvestingEffect::BAG_OF_GOLD:
		if (age > 220000) {
			const alpha_t alpha_scalar = std::pow(0.5f, float_time * 6.0f);
			alpha *= alpha_scalar;
			if (alpha < 0.02) {
				return false;
			}
	...
	case HarvestingEffect::RARE_STONE:
		...
		if (age < 700000) {
			...
			const percent_t scalar = std::pow(0.5f, float_time * 0.5f);
			size *= scalar;
			alpha *= scalar;
			if (alpha < 0.01) {
				return false;
...
HarvestingEffect::HarvestingEffect(EyeCandy *_base, bool *_dead, Vec3 *_pos, const HarvestingType _type, const Uint16 _LOD) {
	...
	case RADON_POUCH:
		...
		while ((int)particles.size() < LOD * 50) {
			...
			Particle
			*p = new HarvestingParticle(this, mover, coords, velocity, 5.25, 0.5, 0.6, 0.7, 0.2, EC_FLARE, LOD, type);
			p->state = 0;
	...
	case MOTHER_NATURE:
		...
		while ((int)particles.size() < LOD * 100) {
			...
			velocity.y += 1.4;
			Particle
			*p = new HarvestingParticle(this, mover, coords, velocity, 3.0, 0.2, 1.0, 0.5 + randcolor(0.5), 0.5, EC_TWINFLARE, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case BEES:
		...
		while ((int)particles.size() < LOD * 4) {
			const Vec3 coords = spawner->get_new_coords() + effect_center - direction;
			Vec3 velocity;
			velocity.randomize();
...
HarvestingEffect::HarvestingEffect(EyeCandy *_base, bool *_dead, Vec3 *_pos, Vec3 *_pos2, const HarvestingType _type, const Uint16 _LOD) {
	if (EC_DEBUG) {
		std::cout << "HarvestingEffect (" << this << ") created (" << type << ")." << std::endl;
	}
	base = _base;
	...
	case TOOL_BREAK:
		...
		while ((int)particles.size() < LOD * 32) {
			...
			velocity.y = 0.0;
			Particle
			*p = new HarvestingParticle(this, mover, coords, velocity, 1.25, 1.0, 0.5, 0.5, 0.5, EC_FLARE, LOD, type);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
