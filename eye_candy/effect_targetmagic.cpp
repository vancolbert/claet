...
bool TargetMagicParticle::idle(const Uint64 delta_t) {
	...
	if ((state == 0) && (age < 500000)) {
		...
		case TargetMagicEffect::POISON: {
			const alpha_t scalar = pow_randfloat(float_time * 2.0f);
			alpha *= scalar;
			break;
		}
		...
		case TargetMagicEffect::LIFE_DRAIN: {
			const coord_t scalar = std::pow(0.5f, float_time * 0.8f);
			if (size < 4.0) {
				size /= scalar;
			}
			break;
		...
		velocity = mover->nonpreserving_vec_shift(velocity, to_target * speed, 1.0 - std::pow(0.5f, 3 * float_time));
		...
		case TargetMagicEffect::HARM:
			break;
		case TargetMagicEffect::LIFE_DRAIN: {
			const color_t scalar = std::pow(0.5f, float_time * 4.0f);
			if (color[0] > 0.2) {
				color[0] *= scalar;
			}
			if (color[1] < 1.0) {
		...
		case TargetMagicEffect::REMOTE_HEAL: {
			...
			const float scalar = pow_randfloat(float_time * 5.0f);
			energy *= scalar;
			if (size < 10) {
				size /= scalar;
			}
		...
		case TargetMagicEffect::TELEPORT_TO_RANGE: {
			...
			const alpha_t scalar = std::pow(0.5f, (float)delta_t / 500000);
			alpha *= scalar;
			break;
		}
		case TargetMagicEffect::HARM:
		...
		case TargetMagicEffect::DRAIN_MANA: {
			...
			const float scalar = pow_randfloat(float_time * 2.5f);
			energy *= scalar;
			alpha *= scalar;
			break;
		}
...
float TargetMagicParticle::get_burn() const {
	if (((type == TargetMagicEffect::POISON) && (state == 2)) || (((type == TargetMagicEffect::HARM) || (type == TargetMagicEffect::SMITE_SUMMONED)) && (state) && (rand() & 1))) {
		return 0.0f;
	} else {
		return 1.0f;
	}
...
void TargetMagicEffect::initialize(EyeCandy *_base, bool *_dead, Vec3 *_pos, const std::vector<Vec3 *> _targets, const TargetMagicType _type, std::vector<ec::Obstruction *> *_obstructions, const Uint16 _LOD) {
	...
	case REMOTE_HEAL: {
		...
		velocity.y += 10.0;
		Particle
		*p = new TargetMagicParticle(this, mover, coords, velocity, 7.5, 1.0, 0.3 + randcolor(0.3), 0.7, 0.2, EC_FLARE, LOD, type, spawner2, mover, targets[0], 0, 0);
	...
	case POISON:
		...
		while (particles.size() < 16) {
			...
			Particle
			*p = new TargetMagicParticle(this, mover, coords, velocity, 8.0, 1.0, randcolor(0.3), 0.5 + randcolor(0.3), randcolor(0.5), EC_INVERSE, LOD, type, NULL, mover2, targets[0], 0, 0);
	...
	case TELEPORT_TO_RANGE: {
		...
		while (particles.size() < 12) {
			...
			velocity.randomize(0.5);
			Particle
			*p = new TargetMagicParticle(this, mover2, coords, velocity, 3.75, 0.8 + randcoord(0.2), 1.0, 1.0, 1.0, EC_VOID, LOD, type, spawner, mover, targets[0], 0, 0);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
		...
		for (int i = 0; i < LOD * 100; i++) {
			...
			velocity /= size;
			Particle
			*p = new TargetMagicParticle(this, mover, coords, velocity, size, 1.0, 0.8 + randcolor(0.2), 0.8 + randcolor(0.2), 0.8 + randcolor(0.2), EC_SHIMMER, LOD, type, spawner, mover, targets[0], 0, 0);
			p->state = 1;
			if (!base->push_back_particle(p)) {
				break;
			}
	...
	case HARM:
		...
		while (particles.size() < 16) {
			...
			velocity.randomize(0.5);
			Particle
			*p = new TargetMagicParticle(this, mover2, coords, velocity, 12.0, 1.0, 0.7 + randcolor(0.3), 0.2 + randcolor(0.3), 0.2, EC_FLARE, LOD, type, spawner, mover, targets[0], 0, 0);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case LIFE_DRAIN: {
		...
		for (int i = 0; i < 8; i++) {
			...
			velocity.y += i;
			Particle
			*p = new TargetMagicParticle(this, mover2, coords, velocity, 7.5, 1.0, 0.7 + randcolor(0.3), 0.25 + randcolor(0.25), 0.15 + randcolor(0.15), EC_VOID, LOD, type, spawner, mover2, targets[0], 0, 2);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
		for (int i = 0; i < 8; i++) {
			...
			velocity.y += i;
			Particle
			*p = new TargetMagicParticle(this, mover, coords, velocity, 15.0, 1.0, 0.7 + randcolor(0.3), 0.15 + randcolor(0.15), 0.25 + randcolor(0.25), EC_TWINFLARE, LOD, type, spawner, mover, targets[0], 0, 0);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case HEAL_SUMMONED:
		...
		for (int i = 0; i < 4 * (int)targets.size(); i++) {
			...
			velocity.randomize(0.5);
			Particle
			*p = new TargetMagicParticle(this, mover, coords, velocity, 8.25, 1.0, 0.25 + randcolor(0.4), 0.6 + randcolor(0.3), 0.15 + randcolor(0.1), EC_TWINFLARE, LOD, type, spawner2, mover, targets[i / 4], i / 4, 0);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case SMITE_SUMMONED:
		...
		for (int i = 0; i < 4 * (int)targets.size(); i++) {
			...
			velocity.randomize(0.5);
			Particle
			*p = new TargetMagicParticle(this, mover2, coords, velocity, 14.25, 1.0, 0.7 + randcolor(0.3), 0.2 + randcolor(0.3), 0.2, EC_TWINFLARE, LOD, type, spawner, mover, targets[i / 4], i / 4, 0);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case DRAIN_MANA:                 //Use crystal particles.
	{
		...
		for (int i = 0; i < 8; i++) {
			...
			velocity.y += i;
			Particle
			*p = new TargetMagicParticle(this, mover2, coords, velocity, 7.5, 1.0, 0.7 + randcolor(0.3), 0.15 + randcolor(0.15), 0.6 + randcolor(0.35), EC_VOID, LOD, type, spawner2, mover2, targets[0], 0, 2);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
		for (int i = 0; i < 8; i++) {
			...
			velocity.y += i;
			Particle
			*p = new TargetMagicParticle(this, mover, coords, velocity, 7.0, 1.0, 0.7 + randcolor(0.3), 0.15 + randcolor(0.15), 0.6 + randcolor(0.35), EC_CRYSTAL, LOD, type, spawner2, mover, targets[0], 0, 0);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
...
bool TargetMagicEffect::idle(const Uint64 usec) {
	...
	case TELEPORT_TO_RANGE:
		if (age > 1200000) {
			const alpha_t scalar = std::pow(0.5f, (interval_t)usec / 400000);
			for (int i = 0; i < LOD * 2; i++) {
...
TargetMagicEffect2::TargetMagicEffect2(EyeCandy *_base, TargetMagicEffect *_effect, Vec3 *_pos, const TargetMagicEffect::TargetMagicType _type, ParticleSpawner *_spawner, ParticleMover *_mover, float *_target_alpha, Uint16 _effect_id, const Uint16 _LOD) {
	...
	case TargetMagicEffect::REMOTE_HEAL:
		for (int i = 0; i < 20 * LOD; i++) {
			...
			coords += center;
			Particle
			*p = new TargetMagicParticle(this, mover, coords, velocity, 0.7, 1.0, 0.4 + randcolor(0.3), 0.7, 0.2, EC_FLARE, LOD, type, NULL, NULL, &center, effect_id, 1);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case TargetMagicEffect::TELEPORT_TO_RANGE: {
		...
		for (int i = 0; i < LOD * 10; i++) {
			...
			velocity /= size;
			Particle
			*p = new TargetMagicParticle(this, mover, coords, velocity, size, 1.0, 0.8 + randcolor(0.2), 0.8 + randcolor(0.2), 0.8 + randcolor(0.2), EC_SHIMMER, LOD, type, spawner, mover, effect->targets[0], effect_id, 1);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case TargetMagicEffect::LIFE_DRAIN:
		...
		for (int i = 0; i < 20 * LOD; i++) {
			...
			coords += center;
			Particle
			*p = new TargetMagicParticle(this, mover, coords, velocity, 0.6, 1.0, 0.4 + randcolor(0.3), 0.7, 0.2, EC_FLARE, LOD, type, NULL, NULL, &center, effect_id, 1);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case TargetMagicEffect::HEAL_SUMMONED:
		for (int i = 0; i < 10 * LOD; i++) {
			...
			coords += center;
			Particle
			*p = new TargetMagicParticle(this, mover, coords, velocity, 1.0, 1.0, 0.4 + randcolor(0.3), 0.7, 0.2, EC_CRYSTAL, LOD, type, NULL, NULL, &center, effect_id, 1);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
	...
	case TargetMagicEffect::DRAIN_MANA:
		...
		for (int i = 0; i < 20 * LOD; i++) {
			...
			coords += center;
			Particle
			*p = new TargetMagicParticle(this, mover, coords, velocity, 1.6, 1.0, 0.4 + randcolor(0.3), 0.2, 0.7, EC_CRYSTAL, LOD, type, NULL, NULL, &center, effect_id, 1);
			if (!base->push_back_particle(p)) {
				break;
			}
		}
