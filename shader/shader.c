...
static __inline__ int load_shader(GLhandleARB object, const char *file_name, const char *defines) {
	int size[2];
	el_file_ptr file;
	const char *buffer[2];
...
static __inline__ void log_shader_compile_log(GLhandleARB object, const char *shader_file_name, GLint error) {
	GLint blen, slen;
	GLcharARB *info_log;
...
static __inline__ GLhandleARB build_shader(const char *vertex_shader_file_name, const char *vertex_shader_defines, const char *fragment_shader_file_name, const char *fragment_shader_defines) {
	GLint ret, error = 0;
...
int is_shader_supported() {
	if (have_extension(arb_fragment_program) && have_extension(arb_vertex_program) && have_extension(arb_fragment_shader) && have_extension(arb_vertex_shader) && have_extension(arb_shader_objects) && have_extension(arb_shading_language_100) && supports_gl_version(1, 3)) {
		return 1;
	} else {
		return 0;
	}
...
void init_shaders() {
	...
	if (is_shader_supported()) {
		...
		for (i = 0; i < shader_data_size; i++) {
			shader[i] = build_shader(shader_data_list[i].vertex_shader_file_name, shader_data_list[i].vertex_shader_defines, shader_data_list[i].fragment_shader_file_name, shader_data_list[i].fragment_shader_defines);
		}
