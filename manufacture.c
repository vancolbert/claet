	...
	for (recipe_index = 0; recipe_index < SHOW_MAX_RECIPE; recipe_index++) {
		...
		for (recipe_slot_index = 0; recipe_slot_index < NUM_MIX_SLOTS; recipe_slot_index++) {
			...
			for (last_mix_slot_index = 0; last_mix_slot_index < NUM_MIX_SLOTS; last_mix_slot_index++) {
				if ((last_mix_cpy[last_mix_slot_index].quantity > 0) && (recipe[recipe_slot_index].quantity == last_mix_cpy[last_mix_slot_index].quantity) && (recipe[recipe_slot_index].image_id == last_mix_cpy[last_mix_slot_index].image_id) && (recipe[recipe_slot_index].id == last_mix_cpy[last_mix_slot_index].id)) {
...
void save_recipes() {
	...
	if (!recipes_loaded && file_exists_config(fname)) {
		return;
	}
...
//	for (i=0; i<6; i++) manufacture_list[ITEM_WEAR_START+i].quantity = 0;
			...
			if (recipes[k][i].quantity > 0) {
				...
				for (j = 0; j < ITEM_WEAR_START; j++) {
					...
					if (manufacture_list[j].id != unset_item_uid) {
						if (manufacture_list[j].id != recipes[k][i].id) {
							continue;
						}
					} else if (recipes[k][i].image_id != manufacture_list[j].image_id) {
						continue;
					}
	...
	for (i = ITEM_WEAR_START; i < ITEM_WEAR_START + 6; i++) {
		...
		for (j = 0; j < ITEM_WEAR_START; j++) {
			...
			if (manufacture_list[j].id != unset_item_uid) {
				if (manufacture_list[j].id != manufacture_list[i].id) {
					continue;
				}
			} else if (manufacture_list[j].image_id != manufacture_list[i].image_id) {
				continue;
			}
...
static int      display_manufacture_handler(window_info *win) {
	...
	if (recipes_shown) {
		if ((cur_recipe < SHOW_MAX_RECIPE) && (recipe_name[cur_recipe] != NULL)) {
			...
			if (i >= 0) {
				show_help(recipe_name[cur_recipe], 33 * 6 + 30, win->len_y + i + (33 - SMALL_FONT_Y_LEN) / 2);
			}
		}
		if (mouse_over_recipe != -1) {
			i = mouse_over_recipe * 33 - get_window_scroll_pos(recipe_win);
			if ((i >= 0) && (mouse_over_recipe != cur_recipe) && (mouse_over_recipe < SHOW_MAX_RECIPE) && (recipe_name[mouse_over_recipe] != NULL)) {
				show_help_coloured(recipe_name[mouse_over_recipe], 33 * 6 + 30, win->len_y + i + (33 - SMALL_FONT_Y_LEN) / 2, 1.0f, 1.0f, 1.0f);
			}
			if (show_help_text) {
				show_help(recipe_select_str, 0, win->len_y + windows_list.window[recipe_win].len_y + 10);
			}
		}
		mouse_over_recipe = -1;
	}
 ...
 */
		cur_recipe += 1;
		if (cur_recipe >= MAX_RECIPE) {
			cur_recipe = MAX_RECIPE - 1;
		}
	} else {
		return 0;
	}
...
//		if (recipe_status[cur_recipe] >= 0)
		{
			int i;
			for (i = 0; i < 6; i++) {
				manufacture_list[ITEM_WEAR_START + i] = recipes[cur_recipe][i];
			}
...
static int recipe_dropdown_click_handler(window_info *win, int mx, int my, Uint32 flags) {
	...
	if (recipe_status[recipe_clicked] >= 0) {
		int i;
		for (i = 0; i < 6; i++) {
			manufacture_list[ITEM_WEAR_START + i] = recipes[recipe_clicked][i];
		}
...
static int recipe_controls_click_handler(int mx, int my, Uint32 flags) {
	...
	if (mx > wpx && mx < wpx + lpx && my > wpy + lpy - 10 && my < wpy + lpy) {
		...
		if (recipes_shown || recipe_status[cur_recipe] < 0) {
			...
			for (i = 0; i < 6; i++) {
				recipes[cur_recipe][i] = manufacture_list[ITEM_WEAR_START + i];
			}
			clear_recipe_name(cur_recipe);
			if (current_recipe_name != NULL) {
				new_recipe_name(cur_recipe, current_recipe_name);
			}
			set_shown_string(c_green2, "Recette mémorisée dans la liste");
			build_manufacture_list();
		} else {
			set_shown_string(c_yellow2, "Valider l'enregistrement à cet emplacement ou en sélectionner un autre (clic droit)");
		}
		recipes_shown = 1;
		show_window(recipe_win);
		do_click_sound();
 ...
 */
						if (item_list[manufacture_list[pos].pos].quantity < manufacture_list[j].quantity) {
							manufacture_list[pos].quantity = 0;
						} else {
							manufacture_list[pos].quantity = item_list[manufacture_list[pos].pos].quantity - manufacture_list[j].quantity;
						}
					}
					// transfert haut vers bas (ajout de quantité dans un ingrédient de la recette)
						...
						if (cm_manurecipe_addnolimit) {
							...
							manufacture_list[j].quantity += quantitytomove;
							if (manufacture_list[j].quantity > 100) {
								manufacture_list[j].quantity = 100;
							}
			...
			for (j = 0; j < ITEM_WEAR_START; j++) {
				if ((manufacture_list[j].quantity >= 0) && (manufacture_list[j].pos == manufacture_list[ITEM_WEAR_START + pos].pos)) {
					...
					if ((flags & ELW_WHEEL_DOWN && !cm_manurecipe_invertwheel) || (flags & ELW_WHEEL_UP && cm_manurecipe_invertwheel)) {
						...
						if (manufacture_list[ITEM_WEAR_START + pos].quantity > 100) {
							manufacture_list[ITEM_WEAR_START + pos].quantity = 100;
						}
						// par contre la quantité en haut ne descendra pas en dessous de 0
						if (manufacture_list[j].quantity < quantitytomove) {
							manufacture_list[j].quantity = 0;
					...
					// transfert bas vers haut (retrait de quantité d'un ingrédient de la recette)
						...
						// le retrait dans la recette va au pire jusqu'à 0 (suppression de l'ingrédient)
						if (manufacture_list[ITEM_WEAR_START + pos].quantity < quantitytomove) {
							manufacture_list[ITEM_WEAR_START + pos].quantity = 0;
						} else {
							manufacture_list[ITEM_WEAR_START + pos].quantity -= quantitytomove;
						}
 ...
 */
						if (item_list[manufacture_list[j].pos].quantity < manufacture_list[ITEM_WEAR_START + pos].quantity) {
							manufacture_list[j].quantity = 0;
						} else {
							manufacture_list[j].quantity = item_list[manufacture_list[j].pos].quantity - manufacture_list[ITEM_WEAR_START + pos].quantity;
						}
...
//					manufacture_list[ITEM_WEAR_START+pos].image_id=manufacture_list[36+pos].image_id;  // bof, pourquoi dans ce sens et pas l'autre ?
					if (manufacture_list[ITEM_WEAR_START + pos].pos < 0) {
						...
						cur_valids[pos] = (manufacture_list[ITEM_WEAR_START + pos].quantity > item_list[manufacture_list[ITEM_WEAR_START + pos].pos].quantity) ? 0 : 1;
					}
					change_current_name(NULL);
					do_click_sound();
					return 1;
			...
			if ((flags & ELW_WHEEL_UP && !cm_manurecipe_invertwheel) || (flags & ELW_WHEEL_DOWN && cm_manurecipe_invertwheel)) {
				...
				manufacture_list[ITEM_WEAR_START + pos].quantity -= quantitytomove;
				if (manufacture_list[ITEM_WEAR_START + pos].quantity < 0) {
					manufacture_list[ITEM_WEAR_START + pos].quantity = 0;
				}
			}
			cur_valids[pos] = 0;
...
static int context_listrecipe_handler(window_info *win, int widget_id, int mx, int my, int option) {
	...
	case 3:         // suppression de la recette mémorisée à l'emplacement sélectionné
		...
		for (i = 0; i < 6; i++) {
			recipes[cur_recipe][i].quantity = 0;
		}
		clear_recipe_name(cur_recipe);
...
static int recipe_dropdown_resize_handler(window_info *win) {
	...
	if (win->len_y > MAX_RECIPE * 33) {
		win->len_y = MAX_RECIPE * 33;
	}
...
void display_manufacture_menu() {
	if (manufacture_win < 0) {
		...
		mixall_button_id = button_add_extended(manufacture_win, mixall_button_id, NULL, manufacture_menu_x_len - ELW_BOX_SIZE - 50, manufacture_menu_y_len - 36, 50, 0, 0, 1.0f, 0.77f, 0.57f, 0.39f, ">>");
		...
		mixone_button_id = button_add_extended(manufacture_win, mixone_button_id, NULL, manufacture_menu_x_len - ELW_BOX_SIZE - 105, manufacture_menu_y_len - 36, 50, 0, 0, 1.0f, 0.77f, 0.57f, 0.39f, ">");
		...
		clear_button_id = button_add_extended(manufacture_win, clear_button_id, NULL, manufacture_menu_x_len - ELW_BOX_SIZE - 170, manufacture_menu_y_len - 36, 55, 0, 0, 1.0f, 0.77f, 0.57f, 0.39f, clear_str);
