...
ExtendedException::ExtendedException(const Uint32 number, const std::string &description, const char *type, const char *file, const char *function, const Uint32 line) :
...
const std::string &ExtendedException::get_full_description() const {
	if (m_full_description.empty()) {
		...
		if (m_line > 0) {
			desc << " in " << m_function << " at " << m_file << " (line " << m_line << ")";
		}
