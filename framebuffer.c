...
int try_format(GLenum depth_format, GLenum stencil_format) {
	...
	if (depth_format != GL_NONE) {
		...
		ELglRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, depth_format, PROBE_SIZE, PROBE_SIZE);
		/// Attach depth
		ELglFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT, depth_rb);
	}
	if (stencil_format != GL_NONE) {
		...
		ELglRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, stencil_format, PROBE_SIZE, PROBE_SIZE);
		if (glGetError() != GL_NO_ERROR) { // NV hack
			failed = 1;
		}
		/// Attach stencil
		ELglFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_STENCIL_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT, stencil_rb);
		if (glGetError() != GL_NO_ERROR) { // NV hack
			failed = 1;
		}
	}
	...
	// Detach and destroy
	ELglFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT, 0);
	ELglFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_STENCIL_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT, 0);
	if (depth_rb != 0) {
...
void make_color_framebuffer(int width, int height, GLuint *fbo, GLuint *fbo_depth_buffer, GLuint *fbo_stencil_buffer, GLuint *fbo_texture) {
	...
	ELglFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D, *fbo_texture, 0);
	...
	if (fbo_depth_buffer != NULL) {
		...
		CHECK_GL_ERRORS();
		CHECK_FBO_ERRORS();
		ELglFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT, *fbo_depth_buffer);
		CHECK_GL_ERRORS();
		CHECK_FBO_ERRORS();
	}
	...
	if (fbo_stencil_buffer != NULL) {
		...
		CHECK_GL_ERRORS();
		CHECK_FBO_ERRORS();
		ELglFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_STENCIL_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT, *fbo_stencil_buffer);
		CHECK_GL_ERRORS();
		CHECK_FBO_ERRORS();
	}
