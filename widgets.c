...
Uint32 widget_move_win(int window_id, Uint32 widget_id, int new_win_id) {
	...
	if (window_id < 0 || window_id >= windows_list.num_windows || new_win_id < 0 || new_win_id >= windows_list.num_windows) {
		return 0;
	}
	if (windows_list.window[window_id].window_id != window_id || windows_list.window[new_win_id].window_id != new_win_id || window_id == new_win_id) {
		return 0;
	}
	w = windows_list.window[window_id].widgetlist;
	/* Find the widget */
...
Uint32 widget_add(int window_id, Uint32 wid, int (*OnInit)(), Uint16 x, Uint16 y, Uint16 lx, Uint16 ly, Uint32 Flags, float size, float r, float g, float b, const struct WIDGET_TYPE *type, void *T, void *S) {
...
int checkbox_draw(widget_list *W) {
	...
	if (*c->checked) {
		glBegin(GL_QUADS);
		glVertex3i(W->pos_x + 3, W->pos_y + 2, 0);
		glVertex3i(W->pos_x - 2 + W->len_x, W->pos_y + 2, 0);
		glVertex3i(W->pos_x - 2 + W->len_x, W->pos_y - 3 + W->len_y, 0);
...
int vscrollbar_draw(widget_list *W) {
	 ...
	 * uniquement la hauteur "en trop" fournit via bar_len.
	 * A présent on lui renseigne les données suivantes :
	 * - W->size    : hauteur du display (éléments visibles)
	 * - c->bar_len : hauteur totale (tous les éléments contenus)
	 */
	cursor_len = W->len_y - 2 * 15;
	cursor_pos = cursor_len * c->pos / c->bar_len;
	// taille du curseur proportionnelle au contenu affiché/total
	if (W->size < c->bar_len) {
		cursor_len *= W->size / c->bar_len;
	}
	// sinon (tout est affiché) le curseur est grisé
	else if (W->r != -1.0) {
		glColor3f(W->r / 3, W->g / 3, W->b / 3);
	}
	glBegin(GL_QUADS);
	glVertex3i(W->pos_x + 7 + gx_adjust, W->pos_y + gy_adjust + 15 + cursor_pos, 0);
	glVertex3i(W->pos_x - 7 + gx_adjust + W->len_x, W->pos_y + gy_adjust + 15 + cursor_pos, 0);
...
/* fonction commune pour set_pos, scroll_up, scroll_down, click et drag */
int vscrollbar_set_pos_base(widget_list *W, int pos) {
	if (W) {
		vscrollbar *c = (vscrollbar *)W->widget_info;
		c->pos = (pos + W->size > c->bar_len) ? c->bar_len - W->size : pos;
		if (c->pos < 0) {
			c->pos = 0;
		}
		return 1;
...
int vscrollbar_click(widget_list *W, int mx, int my, Uint32 flags) {
	...
	if (b->bar_len > W->size) {
		// un clic sur la barre (en dehors du curseur) effectue un page up/down
		int cursor_deb = (W->len_y - 2 * 15) * b->pos / b->bar_len + 15;
		int cursor_fin = (W->len_y - 2 * 15) * W->size / b->bar_len + cursor_deb;
		if (my < cursor_deb) {
			return vscrollbar_set_pos_base(W, b->pos - W->size + b->pos_inc);
		}
		if (my > cursor_fin) {
			return vscrollbar_set_pos_base(W, b->pos + W->size - b->pos_inc);
		}
...
int vscrollbar_set_bar_len(int window_id, Uint32 widget_id, int bar_len) {
	...
	if (w) {
		...
		if (c->pos + w->size > c->bar_len) {
			c->pos = c->bar_len - w->size;
		}
		return 1;
	}
	return 0;
...
void _text_field_set_nr_visible_lines(widget_list *w) {
	...
	if (tf != NULL /* && (w->Flags & TEXT_FIELD_EDITABLE)*/) {
		...
		if (tf->scroll_id != -1) {
			widget_set_size(w->window_id, tf->scroll_id, tf->nr_visible_lines);
		}
	}
}
...
int skip_message(const text_message *msg, Uint8 filter) {
	...
	if (channel == CHAT_COMBAT && filter == FILTER_ALL) {
		return 1;
	}
	...
	if (channel != filter) {
		switch (channel) {
		case CHAT_LOCAL:
			skip = local_chat_separate;
			break;
		case CHAT_PERSONAL:
			skip = personal_chat_separate;
			break;
		case CHAT_GM:
			skip = guild_chat_separate;
			break;
		case CHAT_SERVER:
			skip = server_chat_separate;
			break;
		case CHAT_MOD:
			skip = mod_chat_separate;
			break;
		case CHAT_MODPM:
			skip = 0;
			break;
		case CHAT_DEV:
			skip = dev_chat_separate;
			break;
		case CHAT_COORD:
			skip = coord_chat_separate;
			break;
		default:
			skip = 1;
		}
...
char *text_field_get_selected_text(const text_field *tf) {
	...
	len = 0;
	select = &tf->select;
	if (TEXT_FIELD_SELECTION_EMPTY(select)) {
		return NULL;
	}
	...
	for (; (sm <= em) && (sm < tf->buf_size); sm++) {
		if (skip_message(&tf->buffer[sm], tf->chan_nr)) {
			continue;
		}
...
static int context_edit_handler(window_info *win, int widget_id, int mx, int my, int option) {
	...
	switch (option) {
	case 0:
		text_field_keypress(w, 0, 0, K_CUT, 24);
		break;
	case 1:
		text_field_keypress(w, 0, 0, K_COPY, 3);
		break;
	case 2:
...
static void context_edit_pre_show_handler(window_info *win, int widget_id, int mx, int my, window_info *cm_win) {
	...
	is_grey = is_grey || !(w->Flags & TEXT_FIELD_EDITABLE) || ((w->Flags & TEXT_FIELD_NO_KEYPRESS) && !(w->Flags & TEXT_FIELD_MOUSE_EDITABLE));
...
int text_field_keypress(widget_list *w, int mx, int my, Uint32 key, Uint32 unikey) {
	...
	if ((keysym == SDLK_LEFT) || (keysym == SDLK_RIGHT) || (keysym == SDLK_UP) || (keysym == SDLK_DOWN) || (keysym == SDLK_HOME) || (keysym == SDLK_END)) {
		if (shift_on) {
...
int text_field_destroy(widget_list *w) {
	...
	if (tf != NULL) {
		...
		if (tf->select.lines != NULL) {
			free(tf->select.lines);
		}
		free(tf);
	}
	return 1;
...
int text_field_draw(widget_list *w) {
	...
	if ((mx > 0 && mx < w->len_x && my > 0 && my < w->len_y) && cur_time < tf->next_blink - TF_BLINK_DELAY) {
		cursor = -1;
...
int multiselect_click(widget_list *widget, int mx, int my, Uint32 flags) {
	...
	for (i = top_but; i < M->nr_buttons; i++) {
		...
		if ((flags & ELW_LEFT_MOUSE || flags & ELW_RIGHT_MOUSE) && my > button_y && my < button_y + multiselect_button_height && mx > M->buttons[i].x && mx < M->buttons[i].x + M->buttons[i].width) {
			M->selected_button = i;
			do_click_sound();
			return 1;
		}
