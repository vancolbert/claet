...
int vscrollbar_draw(widget_list *W)
{
	...
	if (W->size < c->bar_len) cursor_len*= W->size / c->bar_len;
	// sinon (tout est affiché) le curseur est grisé
	else if (W->r != -1.0) glColor3f(W->r/3, W->g/3, W->b/3);
	glBegin(GL_QUADS);
...
int vscrollbar_click (widget_list *W, int mx, int my, Uint32 flags)
{
	...
	if (b->bar_len > W->size)
	{
		...
		if (my < cursor_deb) return vscrollbar_set_pos_base(W, b->pos - W->size + b->pos_inc);
		if (my > cursor_fin) return vscrollbar_set_pos_base(W, b->pos + W->size - b->pos_inc);
