...
int nb_categories;
int categorie_courante = -1;
int nb_liste_courante;
knowledge liste_courante[KNOWLEDGE_LIST_SIZE];
int separation = 133; // aligné avec l'onglet
int saut_ligne = 13;
...
void change_categorie(int cat) {
	...
	for (int i = 0; i < knowledge_count; ++i) {
		if (knowledge_list[i].type != categories[categorie_courante].id) {
			continue;
		}
...
int display_knowledge_handler(window_info *win) {
	...
	for (int i = scroll; i < scroll + 15 && i < nb_liste_courante; ++i) {
		int highlight = 0;
	...
	if (flags & ELW_WHEEL_UP) {
		...
		if (x >= separation) {
			...
			if (idx < nb_liste_courante) {
				for (int i = 0; i < nb_liste_courante; ++i) {
...
void get_knowledge_list(int size, const u8 *bits) {
	int n = countof(knowledge_list), n_bits = clamp(0, size<<3, n);
	for (int i = 0; i < n; ++i) {
		knowledge_list[i].present = 0;
		int k = knowledge_list[i].id;
		if (k < n_bits) knowledge_list[i].present = bits[k>>3] & (1u << (k&7));
	}
	init_categories();
...
void get_new_knowledge(Uint16 idx) {
	...
	++categories[idx_categorie].nbpresent;
...
void init_categories(void) {
	...
	nb_categories = countof(noms_categories);
	for (int i = 0; i < nb_categories; ++i) {
		categories[i].nom = noms_categories[i];
		categories[i].id = i;
	}
	for (int i = 0; i < knowledge_count; ++i) {
		if (!knowledge_list[i].affiche) {
		...
		int cat = clamp(0, knowledge_list[i].type, nb_categories - 1);
