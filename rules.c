...
void free_rules(rule_string *d) {
	...
	for (i = 0; d[i].type != -1; i++) {
		if (d[i].short_str) {
			for (j = 0; d[i].short_str[j]; j++) {
				free(d[i].short_str[j]);
			}
			free(d[i].short_str);
		}
		if (d[i].long_str) {
			for (j = 0; d[i].long_str[j]; j++) {
				free(d[i].long_str[j]);
			}
			free(d[i].long_str);
		}
...
void calc_virt_win_len(rule_string *rules_ptr, int win_heigth, float text_size) {
	...
	vscrollbar_set_bar_len(rules_win, rules_scroll_id, virt_win_len);
...
int draw_rules(rule_string *rules_ptr, int x_in, int y_in, int lenx, int leny, float text_size, const float rgb[8][3]) {
	...
	for (i = 0; (y_curr - virt_win_offset) < leny; i++) {
		...
		case RULE:
			...
			break;
		case INFO:
			if (rules_ptr[i].mouseover) {
				glColor3f(rgb[4][0], rgb[4][1], rgb[4][2]);
			} else {
				glColor3f(rgb[5][0], rgb[5][1], rgb[5][2]);
			}
			zoom = text_size;
			x = x_in + 20;
			y_curr += 10 * zoom;
		...
		for (j = 0; rules_ptr[i].short_str[j]; j++) {//Draw the lines
			...
			if ((leny - (y_curr - virt_win_offset)) < (18 * zoom)) {
				break;
			}
			safe_strncpy(ptr, rules_ptr[i].short_str[j], sizeof(str) - (ptr - str));
			if (y_curr >= (virt_win_offset + y_in)) {
				if (!j) {
					draw_string_zoomed(x, (y_curr - virt_win_offset), (unsigned char *)str, 0, zoom);
				} else {
					draw_string_zoomed(x + xdiff, (y_curr - virt_win_offset), (unsigned char *)str, 0, zoom);
				}
			}
...
void draw_rules_interface(int len_x, int len_y) {
	...
	draw_string_zoomed((len_x - string_width) / 2, len_y - 25, (unsigned char *)str, 0, string_zoom);
	// affichage du compte à rebours
	if (countdown != 0) {
		safe_snprintf(str, sizeof(str), you_can_proceed, countdown / 2);
		string_width = strlen(str) * DEFAULT_FONT_X_LEN;
		draw_string_zoomed_shadowed((len_x - string_width) / 2, len_y - 90, (unsigned char *)str, 0, 1, 0.25f, 0.0f, 0.0f, 1.0f, 0.8f, 0.5f);
	}
	// ou si nécessaire incitation supplémentaire à parcourir les règles
	else if (!read_all_rules) {
		safe_strncpy(str, "descendre en bas des règles pour continuer...", sizeof(str));
		string_width = strlen(str) * DEFAULT_FONT_X_LEN;
		string_zoom = 1;
		if (string_width > 600) {
			string_zoom = 600 / string_width;
			string_width *= string_zoom;
		}
		draw_string_zoomed_shadowed((len_x - string_width) / 2, len_y - 90, (unsigned char *)str, 0, string_zoom, 0.25f, 0.0f, 0.0f, 1.0f, 0.8f, 0.5f);
	}
	set_font(0);
