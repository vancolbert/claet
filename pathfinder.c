...
static void pf_add_tile_to_open_list(PF_TILE *current, PF_TILE *neighbour) {
	if (!neighbour || neighbour->state == PF_STATE_CLOSED || neighbour->z == 0 || (current && PF_DIFF(current->z, neighbour->z) > 2)) {
		return;
	}
	if (current) {
		int f, g, h;
...
void pf_move() {
	...
	if (PF_DIFF(x, pf_dst_tile->x) < 2 && PF_DIFF(y, pf_dst_tile->y) < 2) {
		...
		for (pf_cur_tile = pf_dst_tile; pf_cur_tile; pf_cur_tile = pf_cur_tile->parent) {
			if (PF_DIFF(x, pf_cur_tile->x) <= 12 && PF_DIFF(y, pf_cur_tile->y) <= 12 && !pf_is_tile_occupied(pf_cur_tile->x, pf_cur_tile->y)) {
				Uint8 str[5];
				str[0] = MOVE_TO;
				*((short *)(str + 1)) = SDL_SwapLE16((short)pf_cur_tile->x);
				*((short *)(str + 3)) = SDL_SwapLE16((short)pf_cur_tile->y);
...
void pf_move_to_mouse_position() {
	...
	int tries;
	if (!pf_get_mouse_position(mouse_x, mouse_y, &clicked_x, &clicked_y)) {
		return;
	}
	x = clicked_x;
	y = clicked_y;
...
void pf_move_to_position(int x_dest, int y_dest) {
	int x, y;
	int tries;
	if (pf_find_path(x_dest, y_dest)) {
		return;
	}
	for (x = x_dest - 3, tries = 0; x <= x_dest + 3 && tries < 4 ; x++) {
		for (y = y_dest - 3; y <= y_dest + 3 && tries < 4; y++) {
