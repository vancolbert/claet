...
int is_frame_equal(cafkeyframe *a, cafkeyframe *b) {
	float norm;
	norm = SQ(a->tx - b->tx) + SQ(a->ty - b->ty) + SQ(a->tz - b->tz) + SQ(a->rx - b->rx) + SQ(a->ry - b->ry) + SQ(a->rz - b->rz) + SQ(a->rw - b->rw);
	return norm < EPSILON;
}
...
int hardblend(caftrack *tr, caftrack *fr, cafanim *bl) {
	...
	for (kf = 0; kf < tr->keyframes; kf++) {
		...
		if (kf == 0 || kf == tr->keyframes - 1) {
			tr->frames[kf] = bl->tracks[bltr].frames[0];
		} else {
			tr->frames[kf] = fr->frames[kf];
		}
...
void striptracks(cafanim *from, cafanim *to, csfdata *csf, cafanim *bl) {
	...
	for (i = 0; i < from->header.tracks; i++) {
		...
		printf("%-15s %4s   %sKeep/Blend/Force/%sStrip -->", csf->bones[from->tracks[i].bone].name, (st) ? ("N"):("Y"), (st) ? (""):("*"), (!st) ? (""):("*"));
		fflush(stdout);
		fgets(cmd, 32, stdin);
		if (cmd[0] == '\n') {
			cmd[0] = (st) ? ('s'):('k');
...
int main(int argc, char *argv[]) {
	...
	if (argc >= 5) {
		...
		if (!read_caf(fb, &caf_blend)) {
			return 0;
		}
		fclose(fb);
	} else {
		printf("No Blending Animation given...\n");
	}
