...
int etat_sante(float percentage) {
	int i;
	for (i = 5; i >= 0; i--) {
		if (percentage > niveau_vie[i].pcmax) {
			return i + 1;
		}
	}
	return 0;
...
void add_actor_attachment(int actor_id, int attachment_type) {
	...
	if (!parent) {
		...
		int id = add_actor(attachment_type, actors_defs[attachment_type].skin_name, parent->x_pos, parent->y_pos, parent->z_pos, parent->z_rot, get_actor_scale(parent), 0, 0, 0, 0, 0, 0, -1);
...
void draw_actor_banner(actor *actor_id, float offset_z) {
	...
	if (first_person && me && me->actor_id == actor_id->actor_id) {
		return;
	}
	...
	if ((view_mp || view_mana_bar) && me && !actor_id->dead && me->actor_id == actor_id->actor_id) {
		...
		if (largeur_total_mana > banner_width) {
			banner_width = largeur_total_mana;
		}
		hy += hauteur_mana;
	}
	// ------------------------------------------------------------------- SANTE
	if ((view_hp || view_health_bar) && !actor_id->dead && actor_id->kind_of_actor != NPC) {
		...
		if (view_health_bar) {
			...
			if (percentage > 110.0f) {
				percentage = 110.0f;                  //deal with massive bars by trimming at 110%
			}
			if (actor_id->max_health) {
		...
		if (largeur_total > banner_width) {
			banner_width = largeur_total;
		}
	...
	if (view_names && actor_id->actor_name[0]) {
		...
		if ((actor_id->titre[0] != 0) && (actor_id->titre[0] != 48)) {
			...
			if (largeur_nom > banner_width) {
				banner_width = largeur_nom;
			}
	...
	if (actor_id->actor_id == yourself) {
		if ((mouse_x > hx - (banner_width / 2.0f)) && (mouse_x < hx + (banner_width / 2.0f)) && (window_height - mouse_y > hy) && (window_height - mouse_y < hy + banner_height)) {
			cm_mouse_over_banner = 1;
		} else {
			cm_mouse_over_banner = 0;
		}
	} else {
		if ((mouse_x > hx - (banner_width / 2.0f)) && (mouse_x < hx + (banner_width / 2.0f)) && (window_height - mouse_y > hy) && (window_height - mouse_y < hy + banner_height) && select_with_mouse_on_banner == 1 ) {
...
void draw_actor_overtext(actor *actor_ptr) {
	...
	if (!(SDL_GetAppState() & SDL_APPACTIVE)) {
		return;                                 // not actually drawing, fake it
	}
	set_font(chat_font);
	...
	if (strlen((char *)actor_ptr->current_displayed_text) > MAX_CURRENT_DISPLAYED_TEXT_LEN) {
		if (actor_ptr->current_displayed_text[MAX_CURRENT_DISPLAYED_TEXT_LEN] != ' ') {
			...
			if (position != NULL) {
				...
				index = MAX_CURRENT_DISPLAYED_TEXT_LEN;
			}
		} else {
			index = MAX_CURRENT_DISPLAYED_TEXT_LEN + 1;
		}
			...
			if (actor_ptr->current_displayed_text[MAX_CURRENT_DISPLAYED_TEXT_LEN + index] != ' ') {
				...
				if (position != NULL) {
					...
					if (strlen((char *)actor_ptr->current_displayed_text) > MAX_CURRENT_DISPLAYED_TEXT_LEN + index + index2) {
						...
						index2 = MAX_CURRENT_DISPLAYED_TEXT_LEN;
					}
				} else {
					index2 = MAX_CURRENT_DISPLAYED_TEXT_LEN;
				}
			} else {
				index2 = MAX_CURRENT_DISPLAYED_TEXT_LEN + 1;
			}
	...
	w = (float)get_string_width((unsigned char *)(ligne2));
	if (w > maxwidth) {
		maxwidth = w;
	}
	w = (float)get_string_width((unsigned char *)(ligne3));
	if (w > maxwidth) {
		maxwidth = w;
	}
	...
	if (w < zoom_level * 0.25f) {
		w = zoom_level * 0.25f;                     // largeur minimale de la bulle
	}
	x_left = -w * 0.5f;
	x_right = w * 0.5f;
	...
	set_font(0);
	//glDepthFunc(GL_LESS);
	if (actor_ptr->current_displayed_text_time_left <= 0) { // clear if needed
...
void draw_actor_without_banner(actor *actor_id, Uint32 use_lightning, Uint32 use_textures, Uint32 use_glow) {
	...
	actor *me = get_our_actor();
	if (me && me->actor_id == actor_id->actor_id && first_person) {
		return;
	}
	if (use_textures) {
...
void get_actors_in_range() {
	...
	for (i = 0; i < max_actors; i++) {
		if (actors_list[i] && (actors_list[i]->cluster == me->cluster || actors_list[i]->cluster == 0)) {
			...
			if (aabb_in_frustum(bbox)) {
				...
				if (actors_list[i]->is_enhanced_model && actors_list[i]->actor_id != me->actor_id) {
					tmp_nr_enh_act++;
					tmp_dist_to_nr_enh_act += ((me->x_pos - actors_list[i]->x_pos) * (me->x_pos - actors_list[i]->x_pos)) + ((me->y_pos - actors_list[i]->y_pos) * (me->y_pos - actors_list[i]->y_pos));
				}
			}
		}
	}
...
void display_actors(int banner, int render_pass) {
	...
	for (i = 0; i < no_near_actors; i++) {
		if (near_actors[i].ghost || (near_actors[i].buffs & BUFF_INVISIBILITY)) {
			if ((render_pass == DEFAULT_RENDER_PASS) || (render_pass == SHADOW_RENDER_PASS)) {
				has_ghosts = 1;
			}
			...
			if (cur_actor) {
				...
				if (near_actors[i].select) {
					if (cur_actor->kind_of_actor == NPC) {
						...
						if ((cur_actor->kind_of_actor == HUMAN) || (cur_actor->kind_of_actor == COMPUTER_CONTROLLED_HUMAN) || (cur_actor->is_enhanced_model && ((cur_actor->kind_of_actor == PKABLE_HUMAN) || (cur_actor->kind_of_actor == PKABLE_COMPUTER_CONTROLLED)))) {
	...
	if (banner && (SDL_GetAppState() & SDL_APPACTIVE)) {
		...
		for (i = 0; i < no_near_actors; i++) {
			actor *cur_actor = actors_list[near_actors[i].actor];
			if (cur_actor && cur_actor->actor_id >= 0) {
