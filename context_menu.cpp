...
Container::Container(void)
	...
	menus.resize(20, 0);
	if ((cm_window_id = create_window("Context Menu", -1, 0, 0, 0, 0, 0, ELW_SWITCHABLE_OPAQUE | ELW_USE_BACKGROUND | ELW_USE_BORDER | ELW_ALPHA_BORDER)) == -1) {
		return;
	}
...
int Container::show_if_active(int window_id) {
	...
	// check if this window has any widget activation settings and open if mouse in that widget
	{
		...
		for (WID_MM::iterator it = itp.first; it != itp.second; ++it) {
			...
			if ((mouse_x > win->cur_x + wid->pos_x) && (mouse_x <= win->cur_x + wid->pos_x + wid->len_x) && (mouse_y > win->cur_y + wid->pos_y) && (mouse_y <= win->cur_y + wid->pos_y + wid->len_y)) {
	...
	// check if this window has any active regions and open if mouse in that region
	{
		...
		for (REG_MM::iterator it = itp.first; it != itp.second; ++it) {
			...
			assert(win != NULL);
			Region *cr = &it->second;
			if ((mouse_x > win->cur_x + cr->pos_x) && (mouse_x < win->cur_x + cr->pos_x + cr->len_x) && (mouse_y > win->cur_y + cr->pos_y) && (mouse_y < win->cur_y + cr->pos_y + cr->len_y)) {
...
void Container::showinfo(void) {
	...
	for (std::map<int, size_t>::iterator itp = full_windows.begin(); itp != full_windows.end(); ++itp) {
		...
		std::cout << "  window_id=" << itp->first << " name=[" << ((win != NULL)?win->window_name:"") << "] menu_id=" << itp->second << std::endl;
	}
	std::cout << "\nWindow Regions:-" << std::endl;
	for (REG_MM::iterator itp = window_regions.begin(); itp != window_regions.end(); ++itp) {
		window_info *win = window_info_from_id(itp->first);
		std::cout << "  window_id=" << itp->first << " name=[" << ((win != NULL)?win->window_name:"") << "] region=(" << itp->second.pos_x << ", " << itp->second.pos_y << ", " << itp->second.len_x << ", " << itp->second.len_y << ")" << " menu_id=" << itp->second.cm_id << std::endl;
	}
...
int Menu::display(window_info *win, int mx, int my, Uint32 flags) {
	...
	for (size_t i = 0; i < menu_lines.size(); ++i) {
		...
		if (!menu_lines[i].is_grey && !menu_lines[i].is_separator && (mouse_y > win->cur_y + currenty) && (mouse_y < win->cur_y + currenty + line_step) && (mouse_x > win->cur_x + border) && (mouse_x < win->cur_x + width - border)) {
...
int Menu::set_colour(size_t cm_id, enum CM_COLOUR_NAME colour_name, float r, float g, float b) {
	switch (colour_name) {
	case CM_HIGHLIGHT_TOP:
		highlight_top.set(r, g, b);
		break;
	case CM_HIGHLIGHT_BOTTOM:
		highlight_bottom.set(r, g, b);
		break;
	case CM_TEXT:
		text_colour.set(r, g, b);
		break;
	case CM_GREY:
		grey_colour.set(r, g, b);
		break;
	default:
		return 0;
	}
...
void Menu::show_lines(size_t my_id) {
	...
	for (size_t i = 0; i < menu_lines.size(); ++i) {
		...
		std::cout << " " << ((menu_lines[i].is_grey) ?"greyed":"ungreyed") << " " << ((menu_lines[i].is_separator) ?"separator":"normal") << std::endl;
	}
}
} // end cm namespace
// C wrapper functions
extern "C" size_t cm_create(const char *menu_list, int (*handler)(window_info *, int, int, int, int)) {
	return cm::container.create(menu_list, handler);
}
extern "C" int cm_destroy(size_t cm_id) {
	return cm::container.destroy(cm_id);
}
extern "C" int cm_pre_show_check(Uint32 flags) {
	return cm::container.pre_show_check(flags);
}
extern "C" void cm_post_show_check(int force) {
	cm::container.post_show_check(force);
}
extern "C" int cm_show_if_active(int window_id) {
	return cm::container.show_if_active(window_id);
}
extern "C" int cm_show_direct(size_t cm_id, int window_id, int widget_id) {
	return cm::container.show_direct(cm_id, window_id, widget_id);
}
extern "C" int cm_bool_line(size_t cm_id, size_t line_index, int *control_var, const char *config_name) {
	return cm::container.bool_line(cm_id, line_index, control_var, config_name);
}
extern "C" int cm_grey_line(size_t cm_id, size_t line_index, int is_grey) {
	return cm::container.grey_line(cm_id, line_index, is_grey);
}
extern "C" int cm_set(size_t cm_id, const char *menu_list, int (*handler)(window_info *, int, int, int, int)) {
	return cm::container.set(cm_id, menu_list, handler);
}
extern "C" int cm_add(size_t cm_id, const char *menu_list, int (*handler)(window_info *, int, int, int, int)) {
	return cm::container.add(cm_id, menu_list, handler);
}
extern "C" int cm_set_pre_show_handler(size_t cm_id, void (*handler)(window_info *, int, int, int, window_info *)) {
	return cm::container.set_pre_show_handler(cm_id, handler);
}
extern "C" int cm_set_sizes(size_t cm_id, int border, int text_border, int line_sep, float zoom) {
	return cm::container.set_sizes(cm_id, border, text_border, line_sep, zoom);
}
extern "C" int cm_set_colour(size_t cm_id, enum CM_COLOUR_NAME colour_name, float r, float g, float b) {
	return cm::container.set_colour(cm_id, colour_name, r, g, b);
}
extern "C" int cm_add_window(size_t cm_id, int window_id) {
	return cm::container.add_window(cm_id, window_id);
}
extern "C" int cm_add_region(size_t cm_id, int window_id, int posx, int posy, int lenx, int leny) {
	return cm::container.add_region(cm_id, window_id, posx, posy, lenx, leny);
}
extern "C" int cm_add_widget(size_t cm_id, int window_id, int widget_id) {
	return cm::container.add_widget(cm_id, window_id, widget_id);
}
extern "C" int cm_remove_window(int window_id) {
	return cm::container.remove_window(window_id);
}
extern "C" int cm_remove_regions(int window_id) {
	return cm::container.remove_regions(window_id);
}
extern "C" int cm_remove_widget(int window_id, int widget_id) {
	return cm::container.remove_widget(window_id, widget_id);
}
extern "C" void cm_showinfo(void) {
	cm::container.showinfo();
}
extern "C" int cm_valid(size_t cm_id) {
	if (cm::container.valid(cm_id)) {
		return 1;
	} else {
		return 0;
	}
}
extern "C" size_t cm_window_shown(void) {
	return cm::container.window_shown();
}
extern "C" void *cm_get_data(size_t cm_id) {
	return cm::container.get_data(cm_id);
}
