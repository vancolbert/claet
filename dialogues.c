...
static int      display_dialogue_handler(window_info *win) {
	...
	if (new_dialogue) {
		...
		cm_add_region(cm_npcname_id, win->window_id, npc_name_x_start, win->len_y - (SMALL_FONT_Y_LEN + 1), npc_name_len * SMALL_FONT_X_LEN, SMALL_FONT_Y_LEN);
		cm_add_region(cm_dialog_copy_id, win->window_id, str_edge, win->len_y - (SMALL_FONT_Y_LEN + 1), copy_str_width, SMALL_FONT_Y_LEN);
		cm_add_region(cm_dialog_repeat_id, win->window_id, 4 * str_edge + copy_str_width, win->len_y - (SMALL_FONT_Y_LEN + 1), repeat_str_width, SMALL_FONT_Y_LEN);
	}
	return 0;
}
void close_dialogue() {
	...
	for (i = 0; i < MAX_RESPONSES; i++) {
		if (dialogue_responces[i].in_use) {
			if (show_keypress_letters) {
				if (mx >= dialogue_responces[i].x_start + 5 && mx <= dialogue_responces[i].x_start + 5 + dialogue_responces[i].x_len && my >= dialogue_responces[i].y_start + response_y_offset && my <= dialogue_responces[i].y_start + response_y_offset + dialogue_responces[i].y_len) {
					dialogue_responces[i].mouse_over = 1;
					return 0;
				}
			} else {
				if (mx >= dialogue_responces[i].orig_x_start + 5 && mx <= dialogue_responces[i].orig_x_start + 5 + dialogue_responces[i].orig_x_len && my >= dialogue_responces[i].orig_y_start + response_y_offset && my <= dialogue_responces[i].orig_y_start + response_y_offset + dialogue_responces[i].orig_y_len) {
					dialogue_responces[i].mouse_over = 1;
					return 0;
				}
			}
...
static void copy_dialogue_text(void) {
	...
	while (from_index < from_str_len && to_index < to_str_max - 1) {
		if (!is_color(dialogue_string[from_index]) && dialogue_string[from_index] != '\r' && (!dialogue_copy_excludes_newlines || dialogue_string[from_index] != '\n')) {
...
static int click_dialogue_handler(window_info *win, int mx, int my, Uint32 flags) {
	...
	for (i = 0; i < MAX_RESPONSES; i++) {
		if (dialogue_responces[i].in_use && (mon_acteur == -1)) {
			mon_acteur = dialogue_responces[i].to_actor;
		}
