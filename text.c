...
int filter_or_ignore_text(char *text_to_add, int len, int size, Uint8 channel) {
	...
	if (channel == CHAT_SERVER) {
		int index = 0;
		if (my_strncompare(text_to_add + 1, "Tu commences la récolte de ", 27)) {
			index = 27;
		} else if (my_strncompare(text_to_add + 1, "Tu commences la récolte d'", 26)) {
			index = 26;
		}
		if (index > 0) {
		...
		} else if ((my_strncompare(text_to_add + 1, "Tu as arrêté de récolter.", 25)) || (my_strncompare(text_to_add + 1, "Tu ne peux pas récolter pendant un combat (tsssk) !", 51)) || (my_strncompare(text_to_add + 1, "Tu ne peux pas faire ça pendant une négociation !", 49)) || (my_strncompare(text_to_add + 1, "Tu ne peux pas récolter ici !", 29)) || ((my_strncompare(text_to_add + 1, "Tu dois avoir un ", 17) && strstr(text_to_add, "pour récolter ça!") != NULL))) {
			...
			case 2:
				...
				if ((active_channels[current_channel] == 2) || ((active_channels[current_channel] >= 11) && (active_channels[current_channel] <= 19))) {
...
void put_colored_text_in_buffer(Uint8 color, Uint8 channel, const Uint8 *text_to_add, int len) {
	...
	if (channel == CHAT_LOCAL) {
		for (jbreak = 0; jbreak < len; jbreak++) {
			if (text_to_add[jbreak] == ':' && text_to_add[jbreak + 1] == ' ') {
				break;
			}
		}
	}
...
//printf("GFM rewrap... (%i)->(%i) [%s]\n", font, msg->wrap_font, msg->data);
	if (msg->wrap_width != width || msg->wrap_zoom != zoom || msg->wrap_font != font) {
		...
		if (msg->chan_idx != CHAT_NONE) {
			total_nr_lines += nlines - msg->wrap_lines;
		}
		msg->len = strlen(msg->data);
