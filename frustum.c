...
void calculate_reflection_frustum(float water_height) {
	...
	if (main_bbox_tree->intersect[INTERSECTION_TYPE_REFLECTION].intersect_update_needed == 0) {
		return;
	}
	glGetFloatv(GL_MODELVIEW_MATRIX, modl);
	glGetFloatv(GL_PROJECTION_MATRIX, proj);
	...
	reflection_frustum[BACK].plane[D] -= (reflection_frustum[BACK].plane[A] * reflection_frustum[BACK].plane[A] * (far_plane - far_reflection_plane) + reflection_frustum[BACK].plane[B] * reflection_frustum[BACK].plane[B] * (far_plane - far_reflection_plane) + reflection_frustum[BACK].plane[C] * reflection_frustum[BACK].plane[C] * (far_plane - far_reflection_plane));
	...
	for (i = start; i < stop; i++) {
		...
		y_max = y_scaled + 3.0f;
		if (tx == 0) {
			x_min -= water_tiles_extension;
		} else if (tx == tile_map_size_x - 1) {
			x_max += water_tiles_extension;
		}
		if (ty == 0) {
			y_min -= water_tiles_extension;
		} else if (ty == tile_map_size_y - 1) {
			y_max += water_tiles_extension;
		}
