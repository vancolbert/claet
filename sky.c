...
void cloudy_sky() {
	...
	glBlendFunc(GL_SRC_COLOR, GL_ONE);
	// draw the sun
	if (skybox_show_sun && !skybox_no_sun && (skybox_sun_position[0] != 0.0 || skybox_sun_position[1] != 0.0 || skybox_sun_position[2] != 0.0)) {
		...
		glBegin(GL_QUADS);
		{
			...
			glTexCoord2f(0, 0);
			glVertex3f(skybox_sun_position[0] + .08 * (perp1[0] + perp2[0]), skybox_sun_position[1] + .08 * (perp1[1] + perp2[1]), skybox_sun_position[2] + .08 * (perp1[2] + perp2[2]));
			glTexCoord2f(1, 0);
			glVertex3f(skybox_sun_position[0] + .08 * (-perp1[0] + perp2[0]), skybox_sun_position[1] + .08 * (-perp1[1] + perp2[1]), skybox_sun_position[2] + .08 * (-perp1[2] + perp2[2]));
			glTexCoord2f(1, 1);
			glVertex3f(skybox_sun_position[0] + .08 * (-perp1[0] - perp2[0]), skybox_sun_position[1] + .08 * (-perp1[1] - perp2[1]), skybox_sun_position[2] + .08 * (-perp1[2] - perp2[2]));
...
#define XML_BOOL(s) (!xmlStrcasecmp((s), (xmlChar *)"yes") || !xmlStrcasecmp((s), (xmlChar *)"true") || !xmlStrcasecmp((s), (xmlChar *)"1"))
...
int skybox_parse_colors(xmlNode *node, float container[360][4]) {
	...
	for (attr = node->properties; attr; attr = attr->next) {
		if (attr->type == XML_ATTRIBUTE_NODE && (!xmlStrcasecmp(attr->name, (xmlChar *)"reset") || !xmlStrcasecmp(attr->name, (xmlChar *)"overwrite"))) {
...
int skybox_build_gradients(float container[360][4]) {
	...
	int first = 0;
	int prev, next, diff;
	while (first < 360 && container[first][3] < 0.0) {
		++first;
	}
	if (first >= 360) {
		return 0;
	...
	do {
		next = (prev + 1) % 360;
		while (container[next][3] < 0.0) {
			next = (next + 1) % 360;
		}
		diff = (next - prev + 360) % 360;
		if (diff) { // prev != next
			for (t = prev + 1; t != next; t = (t + 1) % 360) {
				blend_colors(container[t], container[prev], container[next], (float)((t - prev + 360) % 360) / (float)diff, 4);
			}
		} else { // prev == next
			for (t = prev + 1; t != next; t = (t + 1) % 360) {
...
/* 	gluDeleteQuadric(qobj); */
	...
	if (dome_clouds_detail_colors) {
		free(dome_clouds_detail_colors);
	}
	if (dome_clouds_colors_bis) {
		free(dome_clouds_colors_bis);
	}
	if (dome_clouds_detail_colors_bis) {
		free(dome_clouds_detail_colors_bis);
	}
	if (dome_clouds_tex_coords_bis) {
		free(dome_clouds_tex_coords_bis);
	}
	if (fog_colors) {
		free(fog_colors);
	}
