...
#include "memory.h"
#include <assert.h>
static Uint32 decompression_needed(const DdsHeader *header, const Uint32 compression, const Uint32 unpack) {
...
static image_format_type detect_dds_file_format(const DdsHeader *header, Uint8 *alpha, Uint32 *unpack) {
	...
	*unpack = 0;
	if ((header->m_pixel_format.m_flags & DDPF_FOURCC) == DDPF_FOURCC) {
		...
		if (((luminace_mask == red_mask) || (red_mask == 0xFF)) && ((luminace_mask == green_mask) || (green_mask == 0xFF)) && ((luminace_mask == blue_mask) || (blue_mask == 0xFF))) {
		...
		if ((red_mask == 0x0F00) && (green_mask == 0x00F0) && (blue_mask == 0x000F) && (alpha_mask == 0xF000)) {
			return ift_rgba4;
		}
		if ((red_mask == 0xF800) && (green_mask == 0x07E0) && (blue_mask == 0x001F) && (alpha_mask == 0x0000)) {
			return ift_r5g6b5;
		}
		if ((red_mask == 0x7C00) && (green_mask == 0x03E0) && (blue_mask == 0x001F) && (alpha_mask == 0x8000)) {
			return ift_rgb5_a1;
		}
		if ((red_mask == 0x000000FF) && (green_mask == 0x0000FF00) && (blue_mask == 0x00FF0000) && (alpha_mask == 0x00000000)) {
			return ift_rgb8;
		}
		if ((red_mask == 0x000000FF) && (green_mask == 0x0000FF00) && (blue_mask == 0x00FF0000) && (alpha_mask == 0xFF000000)) {
			return ift_rgba8;
		}
		if ((red_mask == 0x00FF0000) && (green_mask == 0x0000FF00) && (blue_mask == 0x000000FF) && (alpha_mask == 0xFF000000)) {
			return ift_bgra8;
		}
		if ((red_mask == 0x00FF0000) && (green_mask == 0x0000FF00) && (blue_mask == 0x000000FF) && (alpha_mask == 0x00000000)) {
			return ift_bgr8;
		}
	}
	*unpack = 1;
...
static Uint32 validate_header(DdsHeader *header, const char *file_name) {
	...
	if (header->m_size != DDS_HEADER_SIZE) {
		LOG_ERROR("File '%s' is invalid. Size of header is"
			  " %d bytes, but must be %d bytes for valid DDS files.", file_name, header->m_size, DDS_HEADER_SIZE);
		return 0;
	}
	if (header->m_pixel_format.m_size != DDS_PIXEL_FORMAT_SIZE) {
		LOG_ERROR("File '%s' is invalid. Size of pixe format header is"
			  " %d bytes, but must be %3% bytes for valid DDS files.", file_name, header->m_pixel_format.m_size, DDS_PIXEL_FORMAT_SIZE);
		return 0;
	}
	if ((header->m_flags & DDSD_MIN_FLAGS) != DDSD_MIN_FLAGS) {
		LOG_ERROR("File '%s' is invalid. At least the "
			  ...
			  "DDSD_HEIGHT flags must be set for a valid DDS file.", file_name);
		return 0;
	}
	...
	if (((header->m_caps.m_caps2 & DDSCAPS2_CUBEMAP) == DDSCAPS2_CUBEMAP) && ((header->m_caps.m_caps2 & DDSCAPS2_CUBEMAP_ALL_FACES) == 0)) {
		LOG_ERROR("File '%s' is invalid. At least one cube"
			  " map face must be set for a valid cube map DDS file.", file_name);
		return 0;
	}
	if (((header->m_caps.m_caps2 & DDSCAPS2_CUBEMAP) == DDSCAPS2_CUBEMAP) && ((header->m_caps.m_caps2 & DDSCAPS2_VOLUME) == DDSCAPS2_VOLUME)) {
		LOG_ERROR("File '%s' is invalid. A valid DDS file "
			  "can only be a cube map or a volume, not both.", file_name);
		return 0;
	}
	if (((header->m_flags & DDSD_DEPTH) == DDSD_DEPTH) && ((header->m_caps.m_caps2 & DDSCAPS2_VOLUME) != DDSCAPS2_VOLUME)) {
		LOG_ERROR("File '%s' is invalid. Only volmue images can have"
			  " a detph value in a valid DDS file.", file_name);
		return 0;
	}
	if (((header->m_caps.m_caps2 & DDSCAPS2_CUBEMAP) == DDSCAPS2_CUBEMAP) && ((header->m_caps.m_caps1 & DDSCAPS_COMPLEX) != DDSCAPS_COMPLEX)) {
		LOG_ERROR("File '%s' is invalid. DDSCAPS_COMPLEX cap "
			  "should be set for a valid cube map DDS file.", file_name);
	}
	if (((header->m_caps.m_caps2 & DDSCAPS2_VOLUME) == DDSCAPS2_VOLUME) && ((header->m_caps.m_caps1 & DDSCAPS_COMPLEX) != DDSCAPS_COMPLEX)) {
		LOG_ERROR("File '%s' is invalid. DDSCAPS_COMPLEX cap "
			  "should be set for a valid volume DDS file.", file_name);
	}
	if (((header->m_caps.m_caps1 & DDSCAPS_MIPMAP) == DDSCAPS_MIPMAP) && ((header->m_caps.m_caps1 & DDSCAPS_COMPLEX) != DDSCAPS_COMPLEX)) {
		LOG_ERROR("File '%s' is invalid. DDSCAPS_COMPLEX cap "
			  "should be set for a valid DDS file with mipmaps.", file_name);
	}
	if (((header->m_caps.m_caps1 & DDSCAPS_COMPLEX) == DDSCAPS_COMPLEX) && ((header->m_caps.m_caps1 & DDSCAPS_MIPMAP) != DDSCAPS_MIPMAP) && ((header->m_caps.m_caps2 & DDSCAPS2_VOLUME) != DDSCAPS2_VOLUME) && ((header->m_caps.m_caps2 & DDSCAPS2_CUBEMAP) != DDSCAPS2_CUBEMAP)) {
	...
	if (((header->m_pixel_format.m_flags & DDPF_FOURCC) == DDPF_FOURCC) && ((header->m_pixel_format.m_flags & DDPF_RGB) == DDPF_RGB)) {
		LOG_ERROR("File '%s' is invalid. A valid DDS file must set "
			  "either DDPF_FORCC or DDPF_RGB as pixel format flags.", file_name);
		return 0;
	}
	if (((header->m_pixel_format.m_flags & DDPF_FOURCC) != DDPF_FOURCC) && ((header->m_pixel_format.m_flags & DDPF_RGB) != DDPF_RGB)) {
	...
	if ((header->m_caps.m_caps1 & DDSCAPS_MIPMAP) != DDSCAPS_MIPMAP) {
		...
		if (header->m_mipmap_count == 0) {
			LOG_ERROR("File '%s' is invalid. Mipmaped images must"
				  " have a mipmap count greather than zero.", file_name);
			return 0;
		}
	}
	...
	bit_count = (popcount(header->m_pixel_format.m_red_mask | header->m_pixel_format.m_blue_mask | header->m_pixel_format.m_green_mask | header->m_pixel_format.m_alpha_mask) + 7) & 0xF8;
	if ((header->m_pixel_format.m_bit_count != bit_count) && ((header->m_pixel_format.m_flags & DDPF_RGB) == DDPF_RGB)) {
...
static void decompress_block(el_file_ptr file, const Uint32 format, const Uint32 x, const Uint32 y, const Uint32 width, const Uint32 height, const Uint32 offset, Uint8 *dst) {
	Uint8 values[64];
...
static Uint32 get_level_size(const Uint32 format, const Uint32 bpp, const Uint32 width, const Uint32 height, const Uint32 level, const Uint32 decompress) {
	Uint32 w, h;
	w = max2u(width >> level, 1);
	h = max2u(height >> level, 1);
	if ((format == DDSFMT_DXT1) || (format == DDSFMT_DXT2) || (format == DDSFMT_DXT3) || (format == DDSFMT_DXT4) || (format == DDSFMT_DXT5) || (format == DDSFMT_ATI2) || (format == DDSFMT_ATI1)) {
		if (decompress != 0) {
			return w * h * 4;
		} else {
			w = (w + 3) / 4;
...
static Uint32 get_dds_level_size(const DdsHeader *header, const Uint32 level, const Uint32 decompress, const Uint32 unpack) {
...
static Uint32 get_dds_size(const DdsHeader *header, const Uint32 decompress, const Uint32 strip_mipmaps, const Uint32 base_level) {
...
static void *decompress_dds(el_file_ptr file, DdsHeader *header, const Uint32 strip_mipmaps, const Uint32 base_level) {
	...
	if ((header->m_height % 4) != 0) {
		LOG_ERROR("Can`t decompressed DDS file %s because height is"
			  " %d and not a multiple of four.", el_file_name(file), header->m_height);
		return 0;
	}
	if ((header->m_width % 4) != 0) {
		LOG_ERROR("Can`t decompressed DDS file %s because width is"
			  " %d and not a multiple of four.", el_file_name(file), header->m_width);
		return 0;
	}
	format = header->m_pixel_format.m_fourcc;
	if ((format != DDSFMT_DXT1) && (format != DDSFMT_DXT2) && (format != DDSFMT_DXT3) && (format != DDSFMT_DXT4) && (format != DDSFMT_DXT5) && (format != DDSFMT_ATI1) && (format != DDSFMT_ATI2)) {
		return 0;
	}
	index = 0;
	...
	for (i = base_level; i < mipmap_count; i++) {
		...
		for (y = 0; y < h; y++) {
			for (x = 0; x < w; x++) {
				decompress_block(file, format, x * 4, y * 4, width, height, index, dest);
			}
		}
		index += width * height;
		if (width > 1) {
...
static void *unpack_dds(el_file_ptr file, DdsHeader *header, const Uint32 strip_mipmaps, const Uint32 base_level) {
	Uint8 *dest;
	...
	bpp = header->m_pixel_format.m_bit_count / 8;
	dest = malloc_aligned(size, 16);
	fast_unpack(el_get_pointer(file) + sizeof(DdsHeader) + offset + 4, size / bpp, header->m_pixel_format.m_red_mask, header->m_pixel_format.m_green_mask, header->m_pixel_format.m_blue_mask, header->m_pixel_format.m_alpha_mask, dest);
	return dest;
}
static void *read_dds(el_file_ptr file, DdsHeader *header, const Uint32 strip_mipmaps, const Uint32 base_level) {
	Uint8 *dst;
	Uint32 size, offset;
	size = get_dds_size(header, 0, strip_mipmaps, base_level);
	offset = get_dds_offset(header, base_level);
...
static void get_dds_sizes_and_offsets(const DdsHeader *header, const Uint32 decompress, const Uint32 unpack, const Uint32 strip_mipmaps, const Uint32 base_level, image_t *image) {
...
Uint32 load_dds(el_file_ptr file, const Uint32 compression, const Uint32 unpack, const Uint32 strip_mipmaps, Uint32 base_level, image_t *image) {
	DdsHeader header;
	...
	if (init_dds_image(file, &header) != 0) {
		...
		image->format = detect_dds_file_format(&header, &image->alpha, &format_unpack);
		if (decompression_needed(&header, compression, unpack) == 1) {
			image->image = decompress_dds(file, &header, strip_mipmaps, start_mipmap);
			image->format = ift_rgba8;
			get_dds_sizes_and_offsets(&header, 1, 1, strip_mipmaps, start_mipmap, image);
		} else {
			if ((unpack != 0) || (format_unpack != 0)) {
				image->image = unpack_dds(file, &header, strip_mipmaps, start_mipmap);
				image->format = ift_rgba8;
				get_dds_sizes_and_offsets(&header, 0, 1, strip_mipmaps, start_mipmap, image);
			} else {
				image->image = read_dds(file, &header, strip_mipmaps, start_mipmap);
				get_dds_sizes_and_offsets(&header, 0, 0, strip_mipmaps, start_mipmap, image);
			}
		}
