...
void CalSubmesh::setMorphTargetWeight( const unsigned int& morphName,float weight)
{
    m_vectorMorphTargetWeight[morphName]= weight;
    return;
}
...
void
CalSubmesh::clearMorphTargetState(  const unsigned int& morphName )
{
    m_vectorMorphTargetWeight[morphName] = 0.0f;
    m_vectorAccumulatedWeight[ morphName] = 0.0f;
    m_vectorReplacementAttenuation[ morphName ] = ReplacementAttenuationNull;
}
...
/** Sets weight of a morph target with the given name
  ...
  *****************************************************************************/
void
CalSubmesh::blendMorphTargetScale(  const unsigned int& morphName,
                                    float scale,
                                    ...
                                    bool replace )
{
    ...
    case CalMorphTargetTypeAdditive:
    {
        ...
        m_vectorMorphTargetWeight[ morphName ] += scale * rampValue;
        break;
    }
    case CalMorphTargetTypeClamped:
    {
        ...
        m_vectorMorphTargetWeight[ morphName ] += scale * rampValue;
        if( m_vectorMorphTargetWeight[ morphName ] > 1.0 )
        {
            m_vectorMorphTargetWeight[ morphName ] = 1.0;
        }
        break;
    }
    ...
    case CalMorphTargetTypeAverage:
    {
        ...
        unsigned int subMorphTargetGroupIndex = m_pCoreSubmesh->subMorphTargetGroupIndex( morphName );
        ...
        if( subMorphTargetGroupIndex == 0xffffffff )
        {
            ...
            {
                if( m_vectorReplacementAttenuation[ morphName ] != ReplacementAttenuationNull )
                {
                    attenuatedWeight *= m_vectorReplacementAttenuation[ morphName ];
                }
                else
                {
                    if( replace )
                    {
                        ...
                        m_vectorReplacementAttenuation[ morphName ] = attenuation;
                        m_vectorMorphTargetWeight[ morphName ] *= attenuation;
                        m_vectorAccumulatedWeight[ morphName ] *= attenuation;
                    }
                }
            }
        }
        ...
        if( m_vectorAccumulatedWeight[ morphName ] == 0.0f )
        {
            m_vectorMorphTargetWeight[ morphName ] = rampedScale;
        }
        else
        {
            float factor = attenuatedWeight / ( m_vectorAccumulatedWeight[ morphName ] + attenuatedWeight );
            m_vectorMorphTargetWeight[ morphName ] = m_vectorMorphTargetWeight[ morphName ] * ( 1.0f - factor ) + rampedScale * factor;
        }
        m_vectorAccumulatedWeight[ morphName ] += attenuatedWeight;
        break;
    }
    default:
    {
...
bool CalSubmesh::getMorphTargetWeight( const unsigned int& morphName, float * weightOut)
{
    ...
    *weightOut = m_vectorMorphTargetWeight[morphName];
    return true;
}
