...
static Uint32 download_file(const char *file_name, FILE *file, const char *server, const char *path, Uint64 *file_size, const Uint32 size, char *buffer, const Uint32 etag_size, char *etag) {
	char str[64];
	char *pos;
	IPaddress http_ip;
	TCPsocket http_sock;
	...
	if ((etag != 0) && (strlen(etag) > 0)) {
		safe_snprintf(buffer, size, "GET %s%s HTTP/1.1\r\nHost: %s\r\n"
			      "CONNECTION:CLOSE\r\nREFERER:%s\r\n"
			      "USER-AGENT:AUTOUPDATE\r\nIf-None-Match: \"%s\"\r\n\r\n", path, file_name, server, FILE_VERSION, etag);
	} else {
		safe_snprintf(buffer, size, "GET %s%s HTTP/1.1\r\nHost: %s\r\n"
		...
		if (!got_header) {
			...
			if ((etag_size > 0) && (etag != 0)) {
				...
				if (pos != 0) {
					...
					safe_snprintf(str, sizeof(str), "ETag: \"%%%ds", etag_size - 1);
					sscanf(pos, str, etag);
					if (strlen(etag) > 0) {
			...
			for (i = 0; i < len; i++) {
				if (buffer[i] == 0x0D && buffer[i + 1] == 0x0A && buffer[i + 2] == 0x0D && buffer[i + 3] == 0x0A) {
					...
					if (http_status == 200) {
						*file_size += len - i - 4;
						fwrite(buffer + i + 4, 1, len - i - 4, file);
					}
					break;
				}
			}
...
static int download_files_thread(void *_data) {
	...
	while (1) {
		...
		if (data->update_progress_function("Downloading files", count, index, data->user_data) != 1) {
			CHECK_AND_LOCK_MUTEX(data->mutex);
			data->running = 0;
			CHECK_AND_UNLOCK_MUTEX(data->mutex);
			error = 2;
		...
		for (i = 0; i < 5; i++) {
			...
			result = download_file(info->file_name, file, data->server, data->path, &size, download_buffer_size, download_buffer, 0, 0);
			if (result != 0) {
				LOG_ERROR("Download error %d while updating "
					  "file '%s' from server '%s', retrying"
					  " it", result, info->file_name, data->server);
				error = 3;
				continue;
			}
			if (file_read(file, size, &file_buffer, &file_size) != 0) {
				LOG_ERROR("Read error while updating file '%s' "
					  "from server '%s', retrying it", info->file_name, data->server);
				error = 3;
				continue;
			}
			convert_md5_digest_to_comment_string(info->digest, sizeof(comment), comment);
			len = strlen(info->file_name);
			safe_snprintf(file_name, sizeof(file_name), "%s", info->file_name);
			if (has_suffix(file_name, len, ".xz", 3)) {
				file_name[len - 3] = 0;
			}
			CHECK_AND_LOCK_MUTEX(data->mutex);
			add_to_zip(file_name, file_size, file_buffer, data->dest, comment);
			data->index++;
			CHECK_AND_UNLOCK_MUTEX(data->mutex);
			free(file_buffer);
			error = 0;
...
static void init_threads(download_files_thread_data_t *data, const Uint32 count, const char *server, const char *path, zipFile dest, progress_fnc update_progress_function, void *user_data) {
	Uint32 i;
	data->files = 0;
	...
	for (i = 0; i < UPDATE_DOWNLOAD_THREAD_COUNT; i++) {
		data->threads[i] = SDL_CreateThread(download_files_thread, data);
	}
}
...
static Uint32 download_files(update_info_t *infos, const Uint32 count, const char *server, const char *path, const Uint32 source_count, unzFile *sources, zipFile dest, progress_fnc update_progress_function, void *user_data) {
	char file_name[256];
	...
	error = 0;
	init_threads(&thread_data, count, server, path, dest, update_progress_function, user_data);
	for (i = 0; i < count; i++) {
		CHECK_AND_LOCK_MUTEX(thread_data.mutex);
		index = thread_data.index;
		CHECK_AND_UNLOCK_MUTEX(thread_data.mutex);
		if (update_progress_function("Updating files", count, index, user_data) != 1) {
			error = 2;
			break;
		}
		download = 1;
		len = strlen(infos[i].file_name);
		safe_snprintf(file_name, sizeof(file_name), "%s", infos[i].file_name);
		if (has_suffix(file_name, len, ".xz", 3)) {
			file_name[len - 3] = 0;
		}
		for (j = 0; j < source_count; j++) {
			if (check_md5_from_zip(sources[j], file_name, infos[i].digest) == 0) {
				CHECK_AND_LOCK_MUTEX(thread_data.mutex);
...
static Uint32 add_to_downloads(const char *buffer, const Uint64 buffer_size, update_info_t **infos, Uint32 *count, progress_fnc update_progress_function, void *user_data) {
	char name[256];
	char md5[33];
	MD5_DIGEST digest;
	...
	while (line_buffer_size > 0) {
		...
		sscanf(line_buffer, "%*[^(](%250[^)])%*[^0-9a-zA-Z]%32s", name, md5);
		// check for something to process
		if (*name && *md5 && (name[0] != '/') && (name[0] != '\\') && (name[1] != ':')) {
...
static Uint32 check_server_digest_file(const char *file, FILE *tmp_file, const char *server, const char *path, const Uint32 size, char *buffer, const Uint32 digest_size, char *digest) {
	Uint64 file_size;
	Uint32 result;
	result = download_file(file, tmp_file, server, path, &file_size, size, buffer, 0, 0);
...
static Uint32 check_server_digest_files(const char *file, FILE *tmp_file, const char *server, const char *path, const Uint32 size, char *buffer, char md5[33]) {
	char *file_name = NULL;
	...
	for (i = 0; i < 2; i++) {
		...
		result = check_server_digest_file(file_name, tmp_file, server, path, size, buffer, 32, md5);
		if (result < 2) {
			free(file_name);
			return result;
		}
...
static Uint32 build_update_list(const char *server, const char *file, const char *path, update_info_t **infos, Uint32 *count, char md5[33], const Uint32 etag_size, char *etag, progress_fnc update_progress_function, void *user_data) {
	char *buffer = NULL;
	...
	result = check_server_digest_files(file, tmp_file, server, path, buffer_size, buffer, md5);
	if (result == 0) {
	...
	fseek(tmp_file, 0, SEEK_SET);
	result = download_file(file, tmp_file, server, path, &file_size, buffer_size, buffer, etag_size, etag);
	if (result == 304) {
		fclose(tmp_file);
		update_progress_function("No update needed", 0, 0, user_data);
		free(buffer);
	...
	if (result != 0) {
		...
		fseek(tmp_file, 0, SEEK_SET);
		result = download_file(file_name, tmp_file, server, path, &file_size, buffer_size, buffer, etag_size, etag);
		free(file_name);
		if (result == 304) {
			fclose(tmp_file);
			update_progress_function("No update needed", 0, 0, user_data);
	...
	if (result != 0) {
		...
		safe_snprintf(error_str, error_str_size, "Can't get update list"
			      " file '%s' from server '%s' using path '%s', error %d.", file, server, path, result);
		LOG_ERROR(error_str);
	...
	*count = 0;
	...
	fclose(tmp_file);
	if (add_to_downloads(file_buffer, file_size, infos, count, update_progress_function, user_data) != 1) {
		char *error_str = NULL;
		const size_t error_str_size = 4096;
		free(file_buffer);
		error_str = (char *)calloc(sizeof(char), error_str_size);
...
Uint32 update(const char *server, const char *file, const char *dir, const char *zip, progress_fnc update_progress_function, void *user_data) {
	...
	result = build_update_list(server, file, path, &infos, &count, md5, etag_size, etag, update_progress_function, user_data);
	if (result != 0) {
		free(path);
		free(str);
		free(etag);
	...
	result = download_files(infos, count, server, path, MAX_OLD_UPDATE_FILES, source_zips, dest_zip, update_progress_function, user_data);
	if (result == 0) {
		memset(str, 0, str_size);
	...
	if (result == 2) {
		update_progress_function("Canceled updating", 0, 0, user_data);
	}
	free(infos);
	if (result != 0) {
		free(path);
