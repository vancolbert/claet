...
static Uint32 xz_unpack_data(const void *file_buffer, const Uint64 file_size, void **buffer, Uint64 *size) {
	CXzUnpacker state;
	...
	*size = 0;
	...
	do {
		...
		*buffer = realloc(*buffer, uncompressed_size);
		...
		err = XzUnpacker_Code(&state, (Byte *)*buffer + dst_idx, &dst_size, (Byte *)file_buffer + src_idx, &src_size, CODER_FINISH_ANY, &status);
		src_idx += src_size;
		dst_idx += dst_size;
...
Uint32 file_read(FILE *file, const Uint64 file_size, void **buffer, Uint64 *size) {
	...
	*buffer = NULL;
	...
	if (file_size > XZ_SIG_SIZE && memcmp(file_buffer, XZ_SIG, XZ_SIG_SIZE) == 0) {
...
Uint32 xz_file_read(FILE *file, void **buffer, Uint64 *size) {
	...
	*buffer = 0;
	...
	fseek(file, 0, SEEK_SET);
	if (fread(file_buffer, file_size, 1, file) != 1 || memcmp(file_buffer, XZ_SIG, XZ_SIG_SIZE) != 0) {
		free(file_buffer);
		return 1;
	}
	result = xz_unpack_data(file_buffer, file_size, buffer, size);
