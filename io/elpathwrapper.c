...
const char *get_path_config_base(void) {
	 ...
	 */
	if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_PERSONAL | CSIDL_FLAG_CREATE, NULL, 0, locbuffer)) && (strlen(locbuffer) < MAX_PATH + 5)) {
		if (pwd[0] != '\0') {
...
int move_file_to_updates(const char *from_file, char *to_file, int custom) {
	...
	if (custom) {
		to_file = check_custom_dir(to_file);
	}
	if ((strlen(cfgdir) + strlen(from_file) + 1 > MAX_PATH) || (strlen(updatesdir) + strlen(to_file) + 2 > MAX_PATH)) {
		errno = ENAMETOOLONG;
		return -1;
	}
...
int file_update_check(char *filename, const unsigned char *md5, int custom) {
	...
	int res_u = 0;
	if (custom) {
		cust_filename = check_custom_dir(filename);
	}
