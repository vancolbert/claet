...
Uint32 convert_md5_digest_to_comment_string(const MD5_DIGEST digest, const Uint32 size, char *str) {
	...
	snprintf(str, size, "MD5: %02x%02x%02x%02x"
		 "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", digest[0], digest[1], digest[2], digest[3], digest[4], digest[5], digest[6], digest[7], digest[8], digest[9], digest[10], digest[11], digest[12], digest[13], digest[14], digest[15]);
	return 0;
}
Uint32 add_to_zip(const char *file_name, const Uint32 size, const Uint8 *buffer, zipFile dest, const char *comment) {
	zip_fileinfo info;
	time_t raw_time;
	struct tm *ptm;
	...
	zipOpenNewFileInZip2_64(dest, file_name, &info, 0, 0, 0, 0, comment, Z_DEFLATED, 9, 0, 1);
...
Uint32 copy_from_zip(unzFile source, zipFile dest) {
	...
	unzGetCurrentFileInfo64(source, &src_info, file_name, sizeof(file_name), 0, 0, comment, sizeof(comment));
	...
	zipOpenNewFileInZip2_64(dest, file_name, &info, 0, 0, 0, 0, comment, method, level, 1, 1);
	zipWriteInFileInZip(dest, buffer, size);
	zipCloseFileInZipRaw64(dest, src_info.uncompressed_size, crc);
	free(buffer);
	return 0;
}
static Uint32 check_crc_from_zip_current(unzFile source, const Uint32 size, const Uint32 crc) {
	void *buffer;
	buffer = malloc(size);
...
Uint32 check_crc_from_zip(unzFile source, const char *file_name) {
	...
	if (unzGetCurrentFileInfo64(source, &file_info, 0, 0, 0, 0, 0, 0) != UNZ_OK) {
		return 0;
	}
	return check_crc_from_zip_current(source, file_info.uncompressed_size, file_info.crc);
}
Uint32 check_md5_from_zip(unzFile source, const char *file_name, const MD5_DIGEST digest) {
	...
	if (unzGetCurrentFileInfo64(source, &file_info, 0, 0, 0, 0, comment, sizeof(comment)) != UNZ_OK) {
		return 1;
	}
