...
static int do_load_map(const char *file_name, update_func *update_function) {
	...
	if (carte_modif == 1) {
		my_strncp(nom_carte, file_name, strlen(file_name) - strlen("_neige.elm") + 1);
		sprintf(map_file_name, "%s.elm", nom_carte);
	} else if (carte_modif == 2) {
		my_strncp(nom_carte, file_name, strlen(file_name) - strlen("_hallow.elm") + 1);
		sprintf(map_file_name, "%s.elm", nom_carte);
	} else {
		my_strcp(map_file_name, file_name);
	}
	...
	if (cur_map_header.obj_3d_struct_len != sizeof(object3d_io) || cur_map_header.obj_2d_struct_len != sizeof(obj_2d_io) || cur_map_header.lights_struct_len != sizeof(light_io) || (cur_map_header.particles_struct_len != sizeof(particles_io) && cur_map_header.particles_no > 0)) {
	...
	// in the map
	have_clusters = (cur_map_header.clusters_offset > 0) && ((cur_map_header.clusters_offset + tile_map_size_x * tile_map_size_y * 6 * 6 * sizeof(short)) <= el_get_size(file));
...
/* 		water_tiles_extension = 0.0; */
	...
	for (i = 0; i < cur_map_header.obj_3d_no; i++) {
		...
		LOG_DEBUG("Adding 3d object (%d) '%s' at <%d, %f, %f> with "
			  ...
			  "color <%f, %f, %f>.", i, cur_3d_obj_io.file_name, cur_3d_obj_io.x_pos, cur_3d_obj_io.y_pos, cur_3d_obj_io.z_pos, cur_3d_obj_io.x_rot, cur_3d_obj_io.y_rot, cur_3d_obj_io.z_rot, cur_3d_obj_io.self_lit, cur_3d_obj_io.blended, cur_3d_obj_io.r, cur_3d_obj_io.g, cur_3d_obj_io.b);
		if (cur_3d_obj_io.blended != 20) {
			int id;
			if (cur_3d_obj_io.blended != 1) {
				cur_3d_obj_io.blended = 0;
			}
			id = add_e3d(cur_3d_obj_io.file_name, cur_3d_obj_io.x_pos, cur_3d_obj_io.y_pos, cur_3d_obj_io.z_pos, cur_3d_obj_io.x_rot, cur_3d_obj_io.y_rot, cur_3d_obj_io.z_rot, cur_3d_obj_io.self_lit, cur_3d_obj_io.blended, cur_3d_obj_io.r, cur_3d_obj_io.g, cur_3d_obj_io.b, 0);
			if (!have_clusters) {
	...
	for (i = 0; i < cur_map_header.obj_2d_no; i++) {
		...
		LOG_DEBUG("Adding 2d object (%d) '%s' at <%d, %f, %f> with "
			  "rotation <%f, %f, %f>.", i, cur_2d_obj_io.file_name, cur_2d_obj_io.x_pos, cur_2d_obj_io.y_pos, cur_2d_obj_io.z_pos, cur_2d_obj_io.x_rot, cur_2d_obj_io.y_rot, cur_2d_obj_io.z_rot);
#ifndef SHOW_FLICKERING
...
#endif
		id = add_2d_obj(i, cur_2d_obj_io.file_name, cur_2d_obj_io.x_pos, cur_2d_obj_io.y_pos, cur_2d_obj_io.z_pos, cur_2d_obj_io.x_rot, cur_2d_obj_io.y_rot, cur_2d_obj_io.z_rot, 0);
		if (!have_clusters) {
		...
		for (j = 0; j < tile_map_size_x; j++) {
			...
			if (cur_tile != 255) {
				...
				if (IS_WATER_TILE(cur_tile)) {
					bbox.bbmin[Z] = -0.25f;
					bbox.bbmax[Z] = -0.25f;
					if (IS_REFLECTING(cur_tile)) {
						add_water_to_list(main_bbox_tree_items, get_terrain_id(j, i), bbox, cur_tile, 1);
					} else {
						add_water_to_list(main_bbox_tree_items, get_terrain_id(j, i), bbox, cur_tile, 0);
					}
				} else {
					bbox.bbmin[Z] = 0.0f;
					bbox.bbmax[Z] = 0.0f;
	...
	for (i = 0; i < cur_map_header.lights_no; i++) {
		...
		LOG_DEBUG("Adding light(%d) at <%d, %f, %f> with color "
			  "<%f, %f, %f>.", i, cur_light_io.pos_x, cur_light_io.pos_y, cur_light_io.pos_z, cur_light_io.r, cur_light_io.g, cur_light_io.b);
		if (cur_light_io.pos_x < 0.0f || cur_light_io.pos_x > tile_map_size_x * 60 || cur_light_io.pos_y < 0.0f || cur_light_io.pos_y > tile_map_size_y * 60 || cur_light_io.pos_z < -1000.0f || cur_light_io.pos_z > 1000.0f || cur_light_io.r < -1.0f || cur_light_io.r > 1000.0f || cur_light_io.g < -1.0f || cur_light_io.g > 1000.0f || cur_light_io.b < -1.0f || cur_light_io.b > 1000.0f) {
			LOG_ERROR("Bad light (number %d) when loading '%s'; co-ords [%f %f %f] "
				  "colour [%f %f %f]", i, file_name, cur_light_io.pos_x, cur_light_io.pos_y, cur_light_io.pos_z, cur_light_io.r, cur_light_io.g, cur_light_io.b);
	...
	for (i = 0; i < cur_map_header.particles_no; i++) {
		...
		LOG_DEBUG("Adding particle(%d) '%s' at <%d, %f, %f>.", i, cur_particles_io.file_name, cur_particles_io.x_pos, cur_particles_io.y_pos, cur_particles_io.z_pos);
