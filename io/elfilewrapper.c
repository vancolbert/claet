...
int compare_el_zip_file_entry(const void *a, const void *b) {
	if (((el_zip_file_entry_t *)a)->hash == ((el_zip_file_entry_t *)b)->hash) {
		return strcmp(((el_zip_file_entry_t *)a)->file_name, ((el_zip_file_entry_t *)b)->file_name);
	} else {
		if (((el_zip_file_entry_t *)a)->hash < ((el_zip_file_entry_t *)b)->hash) {
			return 1;
		} else {
...
static Uint32 find_in_zip(el_zip_file_t *zip, const el_zip_file_entry_t *key) {
	...
	LOG_DEBUG("Searching file '%s' in zip file '%s'.", zip->file_name, key->file_name);
	if (zip->count == 0) {
		return 0;
	}
	if (bsearch(key, zip->files, zip->count, sizeof(el_zip_file_entry_t), compare_el_zip_file_entry) != 0) {
		return 1;
	} else {
		return 0;
	}
...
static Uint32 locate_in_zip(el_zip_file_t *zip, const el_zip_file_entry_t *key) {
	...
	file = (el_zip_file_entry_t *)bsearch(key, zip->files, zip->count, sizeof(el_zip_file_entry_t), compare_el_zip_file_entry);
	if (file != 0) {
...
static void init_key(const char *file_name, el_zip_file_entry_t *key, const Uint32 size, char *buffer) {
	char *ptr;
	...
	while ((src_idx < len) && (dst_idx < size)) {
		...
		if (ptr != 0) {
			...
			memcpy(buffer + dst_idx, file_name + src_idx, min2u(count, size - dst_idx - 1));
		} else {
			count = len - src_idx;
			memcpy(buffer + dst_idx, file_name + src_idx, min2u(count, size - dst_idx - 1));
		}
...
void load_zip_archive(const char *file_name) {
	...
	for (i = 0; i < count; i++) {
		...
		unzGetCurrentFileInfo64(file, 0, files[i].file_name, size, 0, 0, 0, 0);
		LOG_DEBUG("Loading file (%d) '%s' from zip file '%s'.", i, files[i].file_name, file_name);
	...
	qsort(files, count, sizeof(el_zip_file_entry_t), compare_el_zip_file_entry);
	CHECK_AND_LOCK_MUTEX(zip_mutex);
	index = num_zip_files;
...
static Uint32 do_file_exists(const char *file_name, const char *path, const Uint32 size, char *buffer) {
	struct stat fstat;
	Uint32 found;
	...
	LOG_DEBUG("Checking file '%s': %s.", buffer, found ? "found" :"not found");
	if (found) {
		return 1;
	}
	safe_strncpy2(buffer, path, size, strlen(path));
...
static el_file_ptr xz_file_open(const char *file_name) {
	...
	if (error == 0) {
		...
		LOG_DEBUG("File '%s' [crc:0x%08X] opened.", file_name, el_crc32(result));
		return result;
	}
	free(result);
	return 0;
...
static el_file_ptr gz_file_open(const char *file_name) {
	...
	result->end = result->buffer + size;
	gzclose(file);
	LOG_DEBUG_VERBOSE("File '%s' [crc:0x%08X] opened.", file_name, el_crc32(result));
	return result;
}
...
static el_file_ptr zip_file_open(unzFile file) {
	...
	if (unzGetCurrentFileInfo64(file, &file_info, 0, 0, 0, 0, 0, 0) != UNZ_OK) {
		return NULL;
	}
	result = calloc(1, sizeof(el_file_t));
	...
	if (unzGetCurrentFileInfo64(file, 0, result->file_name, size, 0, 0, 0, 0) != UNZ_OK) {
		free_el_file(result);
		return NULL;
	}
	...
	if (result->crc32 != crc) {
		LOG_ERROR("crc value is 0x%08X, but should be 0x%08X", crc, result->crc32);
		free_el_file(result);
		return NULL;
	}
	LOG_DEBUG_VERBOSE("File '%s' [crc:0x%08X] opened.", result->file_name, result->crc32);
	return result;
}
