...
static void read_vertex_buffer(el_file_ptr file, float *buffer, const Uint32 vertex_count, const Uint32 vertex_size, const Uint32 options, const Uint32 format) {
	float temp[3];
	Uint16 tmp[3];
	Uint32 i, idx, offset;
	Uint8 color[4];
...
static e3d_object *do_load_e3d_detail(e3d_object *cur_object) {
	...
	LOG_DEBUG("E3d file vertex count %d and size %d.", cur_object->vertex_no, vertex_size);
	LOG_DEBUG("E3d file index count %d and size %d.", cur_object->index_no, index_size);
	LOG_DEBUG("E3d file material count %d and size %d.", cur_object->material_no, material_size);
	LOG_DEBUG("E3d file version %d.%d.%d.%d.", version[0], version[1], version[2], version[3]);
	if ((version[0] == 1) && (version[1] == 1)) {
		if ((header.vertex_options & 0xF0) != 0) {
			LOG_ERROR("Unknow options (%d) for file %s.", header.vertex_options, cur_object->file_name);
		}
		header.vertex_options &= 0x0F;
		if ((header.vertex_format & 0xE0) != 0) {
			LOG_ERROR("Unknow format (%d) for file %s.", header.vertex_format, cur_object->file_name);
		}
	...
	if (material_size != get_material_size(header.vertex_options)) {
		LOG_ERROR("File '%s' has wrong material size! Expected size %d, found size %d.", cur_object->file_name, get_material_size(header.vertex_options), material_size);
		free_e3d_pointer(cur_object);
		el_close(file);
		return 0;
	}
	if (short_index(header.vertex_format)) {
		if (index_size != sizeof(Uint16)) {
			LOG_ERROR("File '%s' has wrong index size! Expected size %d, found size %d.", cur_object->file_name, sizeof(Uint16), index_size);
			free_e3d_pointer(cur_object);
			el_close(file);
			return 0;
		}
		...
		if (index_size != sizeof(Uint32)) {
			LOG_ERROR("File '%s' has wrong index size! Expected size %d, found size %d.", cur_object->file_name, sizeof(Uint32), index_size);
			free_e3d_pointer(cur_object);
			el_close(file);
			return 0;
		}
	}
	LOG_DEBUG("Reading vertices at %d from e3d file '%s'.", SDL_SwapLE32(header.vertex_offset), cur_object->file_name);
	...
	if (!CHECK_POINTER(cur_object->vertex_data, "vertex data")) {
		return 0;
	}
	read_vertex_buffer(file, (float *)(cur_object->vertex_data), cur_object->vertex_no, vertex_size, header.vertex_options, header.vertex_format);
	LOG_DEBUG("Reading indices at %d from e3d file '%s'.", SDL_SwapLE32(header.index_offset), cur_object->file_name);
	...
	if (use_vertex_buffers) {
		...
		index_pointer = cur_object->indices;
	}
	...
	if (cur_object->materials == 0) {
		...
		if (!CHECK_POINTER(cur_object->materials, "materials")) {
			return 0;
		}
	...
	LOG_DEBUG("Reading materials at %d from e3d file '%s'.", SDL_SwapLE32(header.material_offset), cur_object->file_name);
	...
	if (use_vertex_buffers) {
		...
		ELglBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, cur_object->indices_vbo);
		ELglBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, cur_object->index_no * indices_size, cur_object->indices, GL_STATIC_DRAW_ARB);
		free(cur_object->indices);
		cur_object->indices = 0;
