...
float calculate_average_cache_miss_ratio(const Uint32 *indices, const Uint32 offset, const Uint32 count, const Uint32 cache_size) {
...
bool optimize_vertex_cache_order(Uint32 *tri_indices, const Uint32 offset, const Uint32 count, const Uint32 cache_size) {
	...
	while (tris_left > 0) {
		...
		for (i = 0; i < cache_size; i++) {
			...
			if ((cache_idx[i] != a) && (cache_idx[i] != b) && (cache_idx[i] != c)) {
		...
		for (i = 0; i < cache_size; i++) {
			if (cache_idx[i] < 0xFFFFFFFF) {
				...
				if (v[index].remaining_tris.size() > 0) {
					...
					new_score = cache_score[i] + calc_new_score(v[index].remaining_tris.size());
