...
void load_harvestable_list() {
	...
	while (nr_harvestable_objects < MAX_HARVESTABLE_OBJECTS) {
		...
		my_tolower(strLine);
		my_strncp(harvestable_objects[nr_harvestable_objects], strLine, OBJ_NAME_SIZE);
	...
	qsort(harvestable_objects, nr_harvestable_objects, OBJ_NAME_SIZE, (int (*)(const void *, const void *)) strcmp);
}
int is_harvestable(const char *fname) {
	return bsearch(fname, harvestable_objects, nr_harvestable_objects, OBJ_NAME_SIZE, (int (*)(const void *, const void *)) strcmp) != NULL;
}
void load_entrable_list() {
	...
	while (nr_entrable_objects < MAX_ENTRABLE_OBJECTS) {
		...
		my_tolower(strLine);
		off = *strLine == '/' ? 1 : 0;
		my_strncp(entrable_objects[nr_entrable_objects], strLine + off, OBJ_NAME_SIZE);
		nr_entrable_objects++;
		if (!fgets(strLine, sizeof(strLine), f)) {
			break;
		}
	...
	fclose(f);
	// Sort the list so we can use binary search
	qsort(entrable_objects, nr_entrable_objects, OBJ_NAME_SIZE, (int (*)(const void *, const void *)) strcmp);
}
int is_entrable(const char *fname) {
	return bsearch(fname, entrable_objects, nr_entrable_objects, OBJ_NAME_SIZE, (int (*)(const void *, const void *)) strcmp) != NULL;
}
void load_cursors() {
