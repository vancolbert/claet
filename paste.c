...
void copy_to_clipboard(const char *text) {
	...
	err = PasteboardPutItemFlavor(gClipboard, (PasteboardItemID)1, CFSTR("com.apple.traditional-mac-plain-text"), textData, 0);
	CFRelease(textData);
	CFRelease(gClipboard);
}
...
static void copy_to_clipboard_target(const char *text, int clipboard) {
	...
	if (SDL_GetWMInfo(&wminfo) && (wminfo.subsystem == SDL_SYSWM_X11)) {
		...
		if (clipboard) {
			...
			if (cur_text_clipboard) {
				free(cur_text_clipboard);
			}
			cur_text_clipboard = strdup(text);
		} else {
			selection = XA_PRIMARY;
			if (cur_text_primary) {
				free(cur_text_primary);
			}
...
void process_copy(XSelectionRequestEvent *e) {
	...
	if (e->target == XA_STRING) {
		// Copy the string
		char *buf = e->selection == XA_PRIMARY? cur_text_primary : cur_text_clipboard;
		XChangeProperty(e->display, e->requestor, e->property, XA_STRING, 8, PropModeReplace, (unsigned char *)buf, strlen(buf));
