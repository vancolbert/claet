...
static int langsel_display_error_handler(window_info *win) {
	...
	if (first_time) {
		...
		save_button = button_add_extended(langsel_win, 100, NULL, 10, win->len_y - 40, 0, 0, 0, 1.0f, langsel_winRGB[3][0], langsel_winRGB[3][1], langsel_winRGB[3][2], "Save");
		widget_set_OnClick(langsel_win, save_button, langsel_save_handler);
		save_widget = widget_find(langsel_win, save_button);
		quit_button = button_add_extended(langsel_win, 101, NULL, 20 + save_widget->len_x, win->len_y - 40, 0, 0, 0, 1.0f, langsel_winRGB[3][0], langsel_winRGB[3][1], langsel_winRGB[3][2], "Quit");
		widget_set_OnClick(langsel_win, quit_button, langsel_quit_handler);
		first_time = 0;
		return 1;
	}
	draw_string_small(10, 10, (unsigned char *)message, 6);
	draw_string_small(10, 10 + SMALL_FONT_Y_LEN * 6, (unsigned char *)((langsel_list_error) ?langsel_list_error : "Unknown error"), 1);
	return 1;
}
...
static int display_langsel_handler(window_info *win) {
	...
	if (second_time || (langsel_chosen_node && (last_langsel_chosen_node != langsel_chosen_node))) {
		...
		save_button = button_add_extended(langsel_win, 100, NULL, 0, 0, 0, 0, 0, 1.0f, langsel_winRGB[3][0], langsel_winRGB[3][1], langsel_winRGB[3][2], save);
		save_widget = widget_find(langsel_win, save_button);
		...
		if (langwin_save_note) {
			...
			put_small_colored_text_in_box(c_grey1, (const Uint8 *)langwin_save_note, strlen(langwin_save_note), winwidth - (2 * winsep + save_widget->len_x), langsel_save_note_boxed);
		}
		if (add_scroll_bar) {
			langsel_scroll_id = vscrollbar_add_extended(langsel_win, 102, NULL, winwidth - scroll_width, winsep, scroll_width, line_step * max_lang_lines, 0, max_lang_lines, langsel_winRGB[3][0], langsel_winRGB[3][1], langsel_winRGB[3][2], 0, 1, num_lang_lines);
	...
	for (local_head = first_node; local_head; local_head = local_head->prev, lang_line_num++) {
		...
		else if ((mouse_y > win->cur_y + current_y) && (mouse_y < win->cur_y + current_y + line_step) && (mouse_x >= win->cur_x + winsep) && (mouse_x - winsep <= win->cur_x + max_str_width)) {
...
int display_langsel_win(void) {
	...
	langsel_win = create_window("", langsel_rootwin, -1, (window_width - 400) / 2, (window_height - 400 / 1.62) / 2, 400, 400 / 1.62, ELW_WIN_DEFAULT ^ (ELW_CLOSE_BOX | ELW_TITLE_BAR));
