...
void cal_actor_set_random_idle(int id) {
	...
	for (i = 0; i < actors_defs[actors_list[id]->actor_type].group_count; ++i) {
		...
		if (random_anim < actors_defs[actors_list[id]->actor_type].idle_group[i].count) {
			random_anim_index = actors_defs[actors_list[id]->actor_type].idle_group[i].anim[random_anim].anim_index;
		} else {
			random_anim_index = -1;
		}
		if (actors_list[id]->IsOnIdle == 1) {
...
void print_queue(actor *act) {
	...
	for (k = 0; k < MAX_CMD_QUEUE; k++) {
		if (act->que[k] == enter_combat) {
			printf("IC");
		}
		if (act->que[k] == leave_combat) {
			printf("LC");
		}
		if (act->que[k] >= move_n && act->que[k] <= move_nw) {
			printf("M");
		}
		if (act->que[k] >= turn_n && act->que[k] <= turn_nw) {
			printf("R");
		}
		printf("%2i|", act->que[k]);
	}
...
void flush_delayed_item_changes(actor *a) {
	...
	for (item = 0; item < a->delayed_item_changes_count; ++item) {
		if (a->delayed_item_changes[item] < 0) {
			unwear_item_from_actor(a->actor_id, a->delayed_item_type_changes[item]);
		} else {
			actor_wear_item(a->actor_id, a->delayed_item_type_changes[item], a->delayed_item_changes[item]);
		}
	}
	...
	for (k = 0; k < MAX_CMD_QUEUE; k++) {
		if (act->que[k] == nothing) {
			...
			if (k > MAX_CMD_QUEUE - 2) {
				...
				else if ((command == stand_up || command == sit_down) && (act->que[k - 1] == stand_up || act->que[k - 1] == sit_down)) {
					act->que[k - 1] = command;
					break;
				}
			}
...
void sanitize_cmd_queue(actor *act) {
	...
	for (k = MAX_CMD_QUEUE - 1; k > 0 && j > 0; k--, j--) {
		act->que[k] = nothing;
	}
}
void add_command_to_actor(int actor_id, unsigned char command) {
	...
	if (!act) {
		...
		UNLOCK_ACTORS_LISTS();
		if (k != k2) {
			LOG_ERROR("Inconsistency between queues of attached actors %s (%d) and %s (%d)!", act->actor_name, act->actor_id, actors_list[act->attached_actor]->actor_name, actors_list[act->attached_actor]->actor_id);
...
int parse_actor_shirt(actor_types *act, const xmlNode *cfg, const xmlNode *defaults) {
	...
	if (act->shirt == NULL) {
		...
		for (i = actor_part_sizes[ACTOR_SHIRT_SIZE]; i--;) {
...
int parse_actor_skin(actor_types *act, const xmlNode *cfg, const xmlNode *defaults) {
	...
	if (act->skin == NULL) {
		...
		for (i = actor_part_sizes[ACTOR_SKIN_SIZE]; i--;) {
			act->skin[i].mesh_index = -1;
		}
	}
...
int parse_actor_legs(actor_types *act, const xmlNode *cfg, const xmlNode *defaults) {
	...
	if (act->legs == NULL) {
		...
		for (i = actor_part_sizes[ACTOR_LEGS_SIZE]; i--;) {
...
int parse_actor_helmet(actor_types *act, const xmlNode *cfg, const xmlNode *defaults) {
	...
	if (act->helmet == NULL) {
		...
		for (i = actor_part_sizes[ACTOR_HELMET_SIZE]; i--;) {
...
int parse_actor_neck(actor_types *act, const xmlNode *cfg, const xmlNode *defaults) {
	...
	if (act->neck == NULL) {
		...
		for (i = actor_part_sizes[ACTOR_NECK_SIZE]; i--;) {
...
int parse_actor_head(actor_types *act, const xmlNode *cfg, const xmlNode *defaults) {
	...
	if (act->head == NULL) {
		...
		for (i = actor_part_sizes[ACTOR_HEAD_SIZE]; i--;) {
...
int parse_actor_hair(actor_types *act, const xmlNode *cfg, const xmlNode *defaults) {
	...
	if (act->hair == NULL) {
		...
		for (i = actor_part_sizes[ACTOR_HAIR_SIZE]; i--;) {
			act->hair[i].mesh_index = -1;
		}
	}
...
int cal_get_idle_group(actor_types *act, char *name) {
	...
	for (i = 0; i < act->group_count; ++i) {
		if (strcmp(name, act->idle_group[i].name) == 0) {
			res = i;
		}
	}
	if (res >= 0) {
...
int parse_actor_frames(actor_types *act, const xmlNode *cfg, const xmlNode *defaults) {
	...
	for (item = cfg; item; item = item->next) {
		if (item->type == XML_ELEMENT_NODE) {
			...
			if (index >= 0) {
				get_string_value(str, sizeof(str), item);
				act->cal_frames[index] = cal_load_anim(act, str, get_string_property(item, "sound"), get_string_property(item, "sound_scale"), get_int_property(item, "duration"));
			} else if (index != -2) {
	...
	for (item = cfg->children; item; item = item->next) {
		if (item->type == XML_ELEMENT_NODE) {
			if (xmlStrcasecmp(item->name, (xmlChar *)"holder") == 0) {
				...
				get_string_value(str, sizeof(str), item);
				att->actor_type[actor_type].cal_frames[cal_attached_walk_frame] = cal_load_anim(held_act, str, get_string_property(item, "sound"), get_string_property(item, "sound_scale"), get_int_property(item, "duration"));
			} else if (xmlStrcasecmp(item->name, (xmlChar *)"CAL_held_run") == 0) {
				get_string_value(str, sizeof(str), item);
				att->actor_type[actor_type].cal_frames[cal_attached_run_frame] = cal_load_anim(held_act, str, get_string_property(item, "sound"), get_string_property(item, "sound_scale"), get_int_property(item, "duration"));
			} else if (xmlStrcasecmp(item->name, (xmlChar *)"CAL_held_idle") == 0) {
				get_string_value(str, sizeof(str), item);
				att->actor_type[actor_type].cal_frames[cal_attached_idle_frame] = cal_load_anim(held_act, str, get_string_property(item, "sound"), get_string_property(item, "sound_scale"), get_int_property(item, "duration"));
			} else if (xmlStrcasecmp(item->name, (xmlChar *)"CAL_held_pain") == 0) {
				get_string_value(str, sizeof(str), item);
				att->actor_type[actor_type].cal_frames[cal_attached_pain_frame] = cal_load_anim(held_act, str, get_string_property(item, "sound"), get_string_property(item, "sound_scale"), get_int_property(item, "duration"));
			} else {
...
int parse_actor_boots(actor_types *act, const xmlNode *cfg, const xmlNode *defaults) {
	...
	if (act->boots == NULL) {
		...
		for (i = actor_part_sizes[ACTOR_BOOTS_SIZE]; i--;) {
...
int cal_load_mesh(actor_types *act, const char *fn, const char *kind) {
	...
	if (res >= 0) {
		...
		if ((mesh) && (act->mesh_scale != 1.0)) {
			CalCoreMesh_Scale(mesh, act->mesh_scale);
		}
	} else {
...
int cal_load_weapon_mesh(actor_types *act, const char *fn, const char *kind) {
	...
	if (res >= 0) {
		...
		if ((mesh) && (act->skel_scale != 1.0)) {
			CalCoreMesh_Scale(mesh, act->skel_scale);
		}
	} else {
		LOG_ERROR("cal3d erreur : %s : %s\n", fn, CalError_GetLastErrorDescription());
	}
...
int parse_actor_script(const xmlNode *cfg) {
	...
	if (act_idx < 0 || act_idx >= MAX_ACTOR_DEFS) {
		...
		safe_snprintf(str, sizeof(str), "Data Error in %s(%d): Actor ID out of range %d", name, act_idx, act_idx);
		LOG_ERROR(str);
		return 0;
	}
	...
	if (act->actor_type > 0 || *act->actor_name) {
		...
		char name[256];
		safe_strncpy(name, get_string_property(cfg, "type"), sizeof(name));
		safe_snprintf(str, sizeof(str), "Data Error in %s(%d): Already loaded %s(%d)", name, act_idx, act->actor_name, act->actor_type);
		LOG_ERROR(str);
	}
	ok = 1;
