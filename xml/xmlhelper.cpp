...
static inline void reader_read_helper(xmlTextReaderPtr reader) {
	if (reader == 0) {
		...
		case 0:
			EXTENDED_EXCEPTION(ExtendedException::ec_item_not_found, "Nothing to parse any more");
		case 1:
			break;
		default:
...
void reader_read(xmlTextReaderPtr reader, bool ignore_whitespace) {
	...
	if (ignore_whitespace) {
		while (xmlTextReaderNodeType(reader) == XML_READER_TYPE_SIGNIFICANT_WHITESPACE) {
			reader_read_helper(reader);
		}
	}
}
