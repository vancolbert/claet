...
static void init_map_loading(const char *file_name) {
	...
	char *texte;
	safe_strncpy((char *)copie_nom_carte, file_name, sizeof(copie_nom_carte));
	texte = strrchr(copie_nom_carte, '_');
	if (texte != NULL) {
		if (((carte_modif == 1) && (strcmp(texte, "_neige.elm") == 0)) || ((carte_modif == 2) && (strcmp(texte, "_hallow.elm") == 0))) {
			texte[0] = '.';
			texte[1] = 'e';
			texte[2] = 'l';
			texte[3] = 'm';
			texte[4] = '\0';
			destroy_map();
			cur_map = get_cur_map(copie_nom_carte);
		} else {
			destroy_map();
...
void change_map(const char *mapname) {
	...
	if (carte_modif == 1) {
		// On n'arrive pas à charger la carte avec neige
		if (!el_load_map(nom_carte_modif)) {
			char erreur_neige[255];
			safe_snprintf(erreur_neige, sizeof(erreur_neige), "Carte de neige demandée, mais non présente. Chargement de la carte standard.");
			LOG_TO_CONSOLE(c_red4, erreur_neige);
			carte_modif = 0;
			if (!el_load_map(nom_carte)) {
				char error[255];
		...
		// On n'arrive pas à charger la carte avec halloween
		if (!el_load_map(nom_carte_modif)) {
			char erreur_hallow[255];
			safe_snprintf(erreur_hallow, sizeof(erreur_hallow), "Carte halloween demandee, mais non presente. Chargement de la carte standard");
			LOG_TO_CONSOLE(c_red4, erreur_hallow);
			carte_modif = 0;
			if (!el_load_map(nom_carte)) {
				char error[255];
				safe_snprintf(error, sizeof(error), cant_change_map, mapname);
	...
	if (!musique_carte) {
		for (i = 0; i < MAX_NOMBRE_MUSIQUE ; i++) {
			if (liste_musiques[i].selection == 1) {
				modif_playlist(i);
				break;
			}
		}
...
void display_map_marks() {
	...
	for (i = 0; i < max_mark; i++) {
		...
		x += (TILESIZE_X / 2);
		y += (TILESIZE_Y / 2);
		if (DST(ax, ay, x, y) > MARK_DIST || marks[i].x < 0 || !marks_3d) {
			continue;
		}
		z = get_tile_height(marks[i].x, marks[i].y);
		for (j = z - fr / 5, ff = 1; j < z + 2; j += 0.1, ff = (2 - (j - z)) / 2) {
			if (marks[i].server_side) {
				glColor4f(0.0f, 0.0f, 1.0f, 0.9f - (j - z) / 3);
			} else {
				glColor4f((float)marks[i].r / 255, (float)marks[i].g / 255, (float)marks[i].b / 255, 0.7f - (j - z) / 3);
			}
			glBegin(GL_QUADS);
