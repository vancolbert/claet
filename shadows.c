...
void calc_shadow_matrix() {
	...
	if (use_shadow_mapping) {
		...
		glOrtho(-light_view_hscale, light_view_hscale, light_view_bottom, light_view_top, light_view_near, light_view_far);
		...
		dot = ground_plane[0] * light_pos[0] + ground_plane[1] * light_pos[1] + ground_plane[2] * light_pos[2] + ground_plane[3] * light_pos[3];
		// first column
...
void render_light_view() {
	...
	if (use_shadow_mapping) {
		if (!use_frame_buffer && !depth_map_id) {
			...
			CHECK_GL_ERRORS();
			glTexImage2D(depth_texture_target, 0, internalformat, shadow_map_size, shadow_map_size, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
			glTexParameteri(depth_texture_target, GL_TEXTURE_COMPARE_MODE_ARB, GL_COMPARE_R_TO_TEXTURE_ARB);
...
void setup_2d_texgen() {
	...
	plane[0] = shadow_texgen_mat[0];
	plane[1] = shadow_texgen_mat[4];
	plane[2] = shadow_texgen_mat[8];
	plane[3] = shadow_texgen_mat[12];
	glTexGenfv(GL_S, GL_EYE_PLANE, plane);
	glEnable(GL_TEXTURE_GEN_T);
	glTexGeni(GL_T, GL_TEXTURE_GEN_MODE, GL_EYE_LINEAR);
	plane[0] = shadow_texgen_mat[1];
	plane[1] = shadow_texgen_mat[5];
	plane[2] = shadow_texgen_mat[9];
	plane[3] = shadow_texgen_mat[13];
	glTexGenfv(GL_T, GL_EYE_PLANE, plane);
	glEnable(GL_TEXTURE_GEN_R);
	glTexGeni(GL_R, GL_TEXTURE_GEN_MODE, GL_EYE_LINEAR);
	plane[0] = shadow_texgen_mat[2];
	plane[1] = shadow_texgen_mat[6];
	plane[2] = shadow_texgen_mat[10];
	plane[3] = shadow_texgen_mat[14];
	glTexGenfv(GL_R, GL_EYE_PLANE, plane);
	glEnable(GL_TEXTURE_GEN_Q);
	glTexGeni(GL_Q, GL_TEXTURE_GEN_MODE, GL_EYE_LINEAR);
	plane[0] = shadow_texgen_mat[3];
	plane[1] = shadow_texgen_mat[7];
	plane[2] = shadow_texgen_mat[11];
	plane[3] = shadow_texgen_mat[15];
...
void setup_shadow_mapping() {
	...
	if (!is_day && lightning_falling) {
		if (lightning_ambient_color[0] + 0.2 > shadow_color[0]) {
			shadow_color[0] = lightning_ambient_color[0] + 0.2;
		}
		if (lightning_ambient_color[1] + 0.2 > shadow_color[1]) {
			shadow_color[1] = lightning_ambient_color[1] + 0.2;
		}
		if (lightning_ambient_color[2] + 0.2 > shadow_color[2]) {
			shadow_color[2] = lightning_ambient_color[2] + 0.2;
		}
	}
	glPushMatrix();
	glLoadIdentity();
