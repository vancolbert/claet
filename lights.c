...
void enable_local_lights() {
	...
	if (show_lights != max_enabled) {
		max_enabled = show_lights;
	}
	glEnable(GL_LIGHT0);
	if (show_lights >= 1) {
		glEnable(GL_LIGHT1);
...
void draw_lights() {
	...
	if (max_enabled >= 0 && show_lights != max_enabled) {
		enable_local_lights();
	}
	j = 0;
...
int add_light(GLfloat x, GLfloat y, GLfloat z, GLfloat r, GLfloat g, GLfloat b, GLfloat intensity, unsigned int dynamic) {
	...
	if ((main_bbox_tree_items != NULL) && (dynamic == 0)) {
		add_light_to_list(main_bbox_tree_items, i, bbox);
	} else {
		add_light_to_abt(main_bbox_tree, i, bbox, dynamic);
	}
	return i;
}
...
void draw_global_light() {
	...
	if (sun_use_static_position) {
		...
		if ((sun_position[0] == 0.0f) && (sun_position[1] == 0.0f) && (sun_position[2] == 0.0f) && (sun_position[3] == 0.0f)) {
			glLightfv(GL_LIGHT7, GL_POSITION, global_light_position);
		} else {
...
void build_global_light_table() {
	...
	make_gradient_light(90, 30, (float *)sky_lights_c1, 0.6f, 0.0f, 0.2f, 0.3f, 0.7f, 0.9f);
	make_gradient_light(0, 30, (float *)sky_lights_c2, 0.2f, 0.5f, 0.8f, 0.6f, 0.3f, 0.0f);
	make_gradient_light(30, 30, (float *)sky_lights_c2, 0.6f, 0.3f, 0.0f, 0.0f, 0.025f, 0.05f);
	make_gradient_light(60, 30, (float *)sky_lights_c2, 0.0f, 0.025f, 0.05f, 0.6f, 0.3f, 0.0f);
	make_gradient_light(90, 30, (float *)sky_lights_c2, 0.6f, 0.3f, 0.0f, 0.2f, 0.5f, 0.8f);
	make_gradient_light(0, 30, (float *)sky_lights_c3, 0.1f, 0.3f, 0.7f, 0.2f, 0.2f, 0.3f);
	make_gradient_light(30, 30, (float *)sky_lights_c3, 0.2f, 0.2f, 0.3f, 0.0f, 0.01f, 0.02f);
	make_gradient_light(60, 30, (float *)sky_lights_c3, 0.0f, 0.01f, 0.02f, 0.2f, 0.2f, 0.3f);
	make_gradient_light(90, 30, (float *)sky_lights_c3, 0.2f, 0.2f, 0.3f, 0.1f, 0.3f, 0.7f);
	make_gradient_light(0, 30, (float *)sky_lights_c4, 0.05f, 0.2f, 0.6f, 0.0f, 0.1f, 0.4f);
	make_gradient_light(30, 30, (float *)sky_lights_c4, 0.0f, 0.1f, 0.4f, 0.01f, 0.01f, 0.01f);
	make_gradient_light(60, 30, (float *)sky_lights_c4, 0.01f, 0.01f, 0.01f, 0.0f, 0.1f, 0.4f);
	make_gradient_light(90, 30, (float *)sky_lights_c4, 0.0f, 0.1f, 0.4f, 0.05f, 0.2f, 0.6f);
}
