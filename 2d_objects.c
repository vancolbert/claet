...
void draw_2d_object(obj_2d *object_id) {
	...
	if (dungeon || (!clouds_shadows && !use_shadow_mapping)) {
		...
		ELglMultiTexCoord2fARB(detail_unit, x1 / texture_scale + clouds_movement_u, y1 / texture_scale + clouds_movement_v);
...
static void parse_2d0(const char *desc, Uint32 len, const char *cur_dir, obj_2d_def *def) {
	...
	while (1) {
		...
		if (!strcasecmp(name, "file_x_len")) {
			...
			safe_snprintf(texture_file_name, sizeof(texture_file_name), "%s/%s", cur_dir, value);
			def->texture_id = load_texture_cached(texture_file_name, tt_mesh);
		} else if (!strcmp(name, "type")) {
			switch (*value) {
			case 'g':
			case 'G':
				def->object_type = GROUND;
				break;
			case 'p':
			case 'P':
				def->object_type = PLANT;
				break;
			case 'f':
			case 'F':
				def->object_type = FENCE;
				break;
			default:
				def->object_type = INVALID;
			}
		}
	}
...
static obj_2d_def *load_obj_2d_def_cache(const char *file_name) {
	obj_2d_def *def, **defp;
	int i;
	defp = bsearch(file_name, obj_2d_def_cache, obj_2d_cache_used, sizeof(obj_2d_def *), cache_cmp_string);
	if (defp) {
		return *defp;
	}
	...
	if (obj_2d_cache_used < MAX_OBJ_2D_DEF) {
		for (i = 0; i < obj_2d_cache_used; i++) {
			if (strcmp(file_name, obj_2d_def_cache[i]->file_name) <= 0) {
				memmove(obj_2d_def_cache + (i + 1), obj_2d_def_cache + i, (obj_2d_cache_used - i) * sizeof(obj_2d_def *));
				break;
			}
		}
...
const char *get_2dobject_at_location(float x_pos, float y_pos) {
	...
	for (i = 0; i < MAX_OBJ_2D; i++) {
		if (obj_2d_list[i] && obj_2d_list[i]->x_pos > (x_pos - offset) && obj_2d_list[i]->x_pos < (x_pos + offset) && obj_2d_list[i]->y_pos > (y_pos - offset) && obj_2d_list[i]->y_pos < (y_pos + offset) && obj_2d_list[i]->display && obj_2d_list[i]->obj_pointer->object_type == GROUND) {
