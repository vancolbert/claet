...
public:
inline HardwareMeshData(const Sint32 mesh_index, const Uint32 size) : m_mesh_index(mesh_index),
	m_size(size) {
...
inline HardwareMeshData(const HardwareMeshData &hmd) :
	...
	m_buffer = new float[m_size * 4];
	memcpy(m_buffer, hmd.m_buffer, sizeof(float) * 4 * m_size);
}
...
static inline GLuint load_vertex_program(const std::string &name) {
	...
	ELglProgramStringARB(GL_VERTEX_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB, str.size(), str.c_str());
...
static inline void render_mesh_shader(actor_types *a, actor *act, Sint32 index, const HardwareMeshData &hmd, const bool use_glow) {
	...
	if (index >= 0) {
		...
		if (use_glow) {
			if (glow > 0) {
				ELglVertexAttrib4f(4, glow_colors[glow].r * 3.0f, glow_colors[glow].g * 3.0f, glow_colors[glow].b * 3.0f, 1.0f);
			} else {
		...
		for (i = 0; i < count; i++) {
			ELglProgramLocalParameter4fvARB(GL_VERTEX_PROGRAM_ARB, i, hmd.get_buffer(i * 4));
		}
		if (bone_id != -1) {
			glPushMatrix();
		...
		glDrawElements(GL_TRIANGLES, a->hardware_model->getFaceCount() * 3, a->index_type, reinterpret_cast<void *>(element_index));
		if (bone_id != -1) {
			glPopMatrix();
		}
	}
...
extern "C" int load_vertex_programs() {
	...
	ELglGetProgramivARB(GL_VERTEX_PROGRAM_ARB, GL_MAX_PROGRAM_INSTRUCTIONS_ARB, &max_instructions);
	...
	if ((vertex_program_ids[0] == 0) || (vertex_program_ids[1] == 0) || (vertex_program_ids[2] == 0) || (vertex_program_ids[3] == 0) || (vertex_program_ids[4] == 0)) {
		return 0;
	} else {
		return 1;
	}
...
extern "C" void cal_render_actor_shader(actor *act, Uint32 use_lightning, Uint32 use_textures, Uint32 use_glow) {
	...
	if (last_actor_type != act->actor_type) {
		...
		ELglVertexAttribPointerARB(0, 3, GL_FLOAT, GL_FALSE, sizeof(ActorVertex), reinterpret_cast<void *>(0 * sizeof(float) + 0 * sizeof(Uint8)));
		ELglVertexAttribPointerARB(1, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(ActorVertex), reinterpret_cast<void *>(3 * sizeof(float) + 0 * sizeof(Uint8)));
		if (use_normals && use_lightning) {
			ELglEnableVertexAttribArrayARB(2);
			ELglVertexAttribPointerARB(2, 3, GL_FLOAT, GL_FALSE, sizeof(ActorVertex), reinterpret_cast<void *>(3 * sizeof(float) + 4 * sizeof(Uint8)));
		} else {
			ELglDisableVertexAttribArrayARB(2);
		}
		ELglVertexAttribPointerARB(3, 4, GL_UNSIGNED_BYTE, GL_FALSE, sizeof(ActorVertex), reinterpret_cast<void *>(6 * sizeof(float) + 4 * sizeof(Uint8)));
		if (use_textures) {
			ELglEnableVertexAttribArrayARB(8);
			ELglVertexAttribPointerARB(8, 2, GL_FLOAT, GL_FALSE, sizeof(ActorVertex), reinterpret_cast<void *>(6 * sizeof(float) + 8 * sizeof(Uint8)));
		} else {
			ELglDisableVertexAttribArrayARB(8);
		}
...
static inline void calculate_face_and_vertex_count(CalCoreModel *core_model, Uint32 &face_count, Uint32 &vertex_count) {
...
extern "C" void build_buffers(actor_types *a) {
	...
	a->hardware_model->setVertexBuffer(reinterpret_cast<char *>(vertex_buffer), 3 * sizeof(float));
	a->hardware_model->setNormalBuffer(reinterpret_cast<char *>(normal_buffer), 3 * sizeof(float));
	a->hardware_model->setWeightBuffer(reinterpret_cast<char *>(weight_buffer), 4 * sizeof(float));
	a->hardware_model->setMatrixIndexBuffer(reinterpret_cast<char *>(matrix_index_buffer), 4 * sizeof(float));
	a->hardware_model->setTextureCoordNum(1);
	a->hardware_model->setTextureCoordBuffer(0, reinterpret_cast<char *>(texture_coordinate_buffer), 2 * sizeof(float));
...
/*			if (weight_buffer[i * 4 + j] > 0.0f)
	...
	ELglBindBufferARB(GL_ARRAY_BUFFER_ARB, a->vertex_buffer);
	ELglBufferDataARB(GL_ARRAY_BUFFER_ARB, a->hardware_model->getTotalVertexCount() * sizeof(ActorVertex), buffer, GL_STATIC_DRAW_ARB);
	ELglBindBufferARB(GL_ARRAY_BUFFER_ARB, 0);
	max_index = 0;
	...
	if (max_index <= std::numeric_limits<Uint16>::max()) {
		...
		ELglBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, a->hardware_model->getTotalFaceCount() * 3 * sizeof(GLushort), data16, GL_STATIC_DRAW_ARB);
		delete[] data16;
	} else {
		a->index_type = GL_UNSIGNED_INT;
		a->index_size = sizeof(GLuint);
		ELglBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, a->hardware_model->getTotalFaceCount() * 3 * sizeof(GLuint), data32, GL_STATIC_DRAW_ARB);
	}
	ELglBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, 0);
...
static inline void set_transformation_buffer(actor_types *a, actor *act, const Uint32 index, HardwareMeshData &hmd) {
	Sint32 i, count;
		...
		for (i = 0; i < count; i++) {
			if (a->hardware_model->getVectorHardwareMesh()[i].meshId == mesh_id) {
				std::pair<Sint32, HardwareMeshData> p = std::pair<Sint32,
										  HardwareMeshData>(i, HardwareMeshData(mesh_id, max_bones_per_mesh * 3));
				im->insert(p);
