		...
		if(my>10 && my<202){
			...
			if(mx>10 && mx<230){
				...
				if(view_only_storage && item_dragged!=-1 && left_click){
					...
					// si l'objet vient des équipements il faut d'abord le déséquiper dans le sac
					if (item_dragged >= ITEM_WEAR_START)
					{
						...
						// demande de déplacement dans le sac
...
*/
				...
				else if (!view_only_storage && cur_item_over!=-1 && (flags & ELW_SHIFT))
				{
					...
					if (temp_case < 0) return 0;
					// recherche d'une case libre dans les équipements
					dest_pos = -1;
					...
					*((Uint32*)(str+3)) = SDL_SwapLE32(1);
					...
					my_tcp_send(my_socket, str, 3);
					do_drop_item_sound();
				// Ctrl+clic sur un objet : chargement dans l'inventaire d'une unité
				// Double-clic (drop sur lui-même) : chargement de la quantité sélectionnée
...
static int resize_storage_handler(window_info *win)
{
	...
	// contrôle sur la largeur min de la fenêtre (liste + 1 case + scrolls + marges + X)
	if (win->len_x < 240 + 32 + 2*20 + 3*10 + 10) win->len_x = 240 + 32 + 2*20 + 3*10 + 10;

	// nombre de lignes pour la zone texte selon la largeur de la fenêtre (arbitrairement 45 caractères)
	storage_textarea_lines = 1 + (45 * 8 / (win->len_x - 2*10 - 2*5));
	storage_textarea_size = 5 + storage_textarea_lines*15 + 5;

	// contrôle sur la hauteur max de la fenêtre par rapport à l'écran
	if (win->len_y > window_height) win->len_y = window_height;
	// contrôle sur la hauteur min de la fenêtre (1 ligne + textarea)
	...
	// calcul du nombre de catégories visibles
