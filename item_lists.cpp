...
void Category_Maps::load(void) {
	...
	while (!in.eof()) {
		...
		if ((category < 0) || (actual_num_image_ids != expected_num_image_ids) || (actual_num_item_ids != expected_num_item_ids)) {
			...
			LOG_ERROR("%s: %s cat=%d expected/actual image=%d/%d item %d/%d\n", __FILE__, item_list_cat_format_error_str, category, expected_num_image_ids, actual_num_image_ids, expected_num_item_ids, actual_num_item_ids);
			break;
		}
	}
	must_save = false;
...
static void quantity_input_handler(const char *input_text, void *data) {
	...
	if ((Vars::lists()->get_active() != input->get_list()) || (input->get_item() >= Vars::lists()->get_list().get_num_items())) {
		return;
	}
	int quantity;
...
void List_Window::show(window_info *win) {
	if (win_id < 0 ) {
		...
		cm_add_region(cm_names_menu, win_id, 0, get_size_y() - get_names_size_y(), get_size_y(), get_names_size_y());
		names_scroll_id = vscrollbar_add_extended(win_id, 1, NULL, get_size_x() - ELW_BOX_SIZE, get_grid_size() * num_grid_rows, ELW_BOX_SIZE, get_names_size_y() - ELW_BOX_SIZE, 0, 1.0, 0.77f, 0.57f, 0.39f, 0, 1, Vars::lists()->size() - num_show_names_list);
...
int List_Window::mouseover(window_info *win, int mx, int my) {
	...
	if (Vars::lists()->valid_active_list() && mx >= 0 && mx < (get_grid_size() * 6) && my >= 0 && my < (get_grid_size() * num_grid_rows)) {
	...
	if ((my > start_names) && (my < (start_names + get_names_size_y()))) {
		name_under_mouse = vscrollbar_get_pos(win_id, names_scroll_id) + static_cast<int>((my - start_names - get_list_gap() / 2) / (get_list_gap() + names_list_height));
	}
