...
void resize_root_window() {
	...
	if (isometric) {
		...
		glFrustum(-perspective * window_ratio * near_plane, perspective * window_ratio * near_plane, -perspective * near_plane, perspective * near_plane, near_plane, far_plane);
		if (!first_person) {
...
void set_new_video_mode(int fs, int mode) {
	...
	if (book_win != -1) {
		destroy_window(book_win);
		book_win = -1;
	}
	// On doit supprimer ce qui a ete fait avec les livres
