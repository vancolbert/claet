...
void ogg_error(int code) {
	switch (code) {
	case OV_EREAD:
		LOG_ERROR(snd_media_read);
		break;
	case OV_ENOTVORBIS:
		LOG_ERROR(snd_media_notvorbis);
		break;
	case OV_EVERSION:
		LOG_ERROR(snd_media_ver_mismatch);
		break;
	case OV_EBADHEADER:
		LOG_ERROR(snd_media_invalid_header);
		break;
	case OV_EFAULT:
		LOG_ERROR(snd_media_internal_error);
		break;
	case OV_EOF:
		LOG_ERROR(snd_media_eof);
		break;
	case OV_HOLE:
		LOG_ERROR(snd_media_hole);
		break;
	case OV_EINVAL:
		LOG_ERROR(snd_media_einval);
		break;
	case OV_EBADLINK:
		LOG_ERROR(snd_media_ebadlink);
		break;
	case OV_FALSE:
		LOG_ERROR(snd_media_false);
		break;
	case OV_ENOSEEK:
		LOG_ERROR(snd_media_enoseek);
		break;
	default:
...
**************************/
static char *get_stream_type(int type) {
	return type == STREAM_TYPE_MUSIC ? "Music" :(type == STREAM_TYPE_CROWD ? "Crowd" :(type == STREAM_TYPE_SOUNDS ? "Background" : "None"));
}
void play_stream(int sound, stream_data *stream, ALfloat gain) {
	char *file;
	...
	if (stream->type == STREAM_TYPE_MUSIC) {
		if (!have_music || sound < 0 || sound >= MAX_PLAYLIST_ENTRIES) {
			return;
		}
		...
		if (sound < 0 || sound >= MAX_SOUNDS || sound_type_data[sound].num_variants <= 0) {
			return;
		}
		// Choose a variant
		...
		if (!have_sound || sound == -1 || !strcmp(file, "")) {
			return;
		}
	}
	...
	if (!found_bg && !found_crowd) {
		...
		if ((snd_cur_map > -1) && (sound_map_data[snd_cur_map].num_defaults > 0)) {
			...
			for (i = 0; i < cur_map->num_defaults; i++) {
				if (!found_bg && time_of_day_valid(cur_map->boundaries[cur_map->defaults[i]].time_of_day_flags) && cur_map->boundaries[cur_map->defaults[i]].bg_sound > -1) {
					add_stream(cur_map->boundaries[cur_map->defaults[i]].bg_sound, STREAM_TYPE_SOUNDS, -1);
					found_bg = 1;
				}
				if (!found_crowd && time_of_day_valid(cur_map->boundaries[cur_map->defaults[i]].time_of_day_flags) && cur_map->boundaries[cur_map->defaults[i]].crowd_sound > -1) {
		...
		if (!found_bg && sound_num_background_defaults > 0) {
			for (i = 0; i < sound_num_background_defaults; i++) {
				if (time_of_day_valid(sound_background_defaults[i].time_of_day_flags) && (sound_background_defaults[i].map_type == dungeon && sound_background_defaults[i].sound > -1)) {
...
int check_stream(stream_data *stream, int day_time, int tx, int ty) {
	...
	if (stream->fade >= 0) {
		...
		case STREAM_TYPE_MUSIC:
			if ((list_pos >= 0) && playlist[list_pos].file_name[0] && (tx < playlist[list_pos].min_x || tx > playlist[list_pos].max_x || ty < playlist[list_pos].min_y || ty > playlist[list_pos].max_y || (playlist[list_pos].time != 2 && playlist[list_pos].time != day_time))) {
				stream->fade = -1;
			}
			break;
...
void voir_musique_sur_carte() {
	...
	int i = 0;
	while (1) {
		if (strlen(playlist[i].file_name) > 3) {
			carte_x_min = (51 * 3 + 200 * playlist[i].min_x * 3 / (tile_map_size_x * 6));
			carte_x_max = (51 * 3 + 200 * playlist[i].max_x * 3 / (tile_map_size_x * 6));
			carte_y_min = (201 * 3 - 200 * playlist[i].min_y * 3 / (tile_map_size_y * 6));
			carte_y_max = (201 * 3 - 200 * playlist[i].max_y * 3 / (tile_map_size_y * 6));
			if (playlist[i].time == 0) { // Nuit en bleu
				glColor3f(0.0f, 0.0f, 1.0f);
			}
			if (playlist[i].time == 1) { // Jour en vert
				glColor3f(0.0f, 1.0f, 0.0f);
			}
			if (playlist[i].time == 2) { // Jout et nuit en rouge
				glColor3f(1.0f, 0.0f, 0.0f);
			}
			glDisable(GL_TEXTURE_2D);
			glBegin(GL_LINE_LOOP);
			glVertex2i(carte_x_min, carte_y_min);
			glVertex2i(carte_x_min, carte_y_max);
			glVertex2i(carte_x_max, carte_y_max);
			glVertex2i(carte_x_max, carte_y_min);
			glEnd();
			glEnable(GL_TEXTURE_2D);
			draw_string_zoomed(carte_x_min + 2, carte_y_max + 2, (unsigned char *)playlist[i].file_name, 1, 0.3 * 3);
...
void affiche_playlist() {
	...
	char timestr[20];
	while (1) {
		if (strlen(playlist[i].file_name) > 3) { // Pour chaque morceau de la playlist, on affiche les informations
			if (playlist[i].time == 0) {
				safe_strncpy(timestr, "Nuit", 5);
			}
			if (playlist[i].time == 1) {
				safe_strncpy(timestr, "Jour", 5);
			}
			if (playlist[i].time == 2) {
				safe_strncpy(timestr, "Tout", 5);
			}
			safe_snprintf(str, sizeof(str), "%i) XYMin : %i.%i - XYMax : %i.%i - %s - Fichier : %s", i, playlist[i].min_x, playlist[i].min_y, playlist[i].max_x, playlist[i].max_x, timestr, playlist[i].file_name);
...
void modif_playlist(int num) {
	...
	char file[64];
	FILE *fp;
	// Erreur sur le numéro de la musique
	if (num > max_musique) {
		return;
	}
	...
	/* On indique qu'on utilise plus les musique de carte */
	musique_carte = 0;
}
void raz_playlist() {
...
int redimensionnement_fenetre_musique(window_info *win) {
	/* On recalcul la hauteur de la liste des musiques */
	hauteur_liste_musique = win->len_y - hauteur_options_musique;
	/* On déplace, redimensionne et recalcul la longueur affichée de la barre de défilement */
	widget_resize(fenetre_musique, musique_barre_defilement, ELW_BOX_SIZE, hauteur_liste_musique - ELW_BOX_SIZE);
	widget_move(fenetre_musique, musique_barre_defilement, win->len_x - ELW_BOX_SIZE, ELW_BOX_SIZE);
	widget_set_size(fenetre_musique, musique_barre_defilement, hauteur_liste_musique / hauteur_ligne_musique);
	/* On déplace le bouton du volume de la musique */
	widget_move(fenetre_musique, volume_musique, position_widget, hauteur_liste_musique + 5);
	/* On déplace le bouton pour jouer les musiques de carte */
	widget_move(fenetre_musique, bouton_musique_carte, position_widget, hauteur_liste_musique + 35);
	/* On déplace le bouton pour jouer les musiques venant du serveur */
	widget_move(fenetre_musique, bouton_serveur_musique, position_widget, hauteur_liste_musique + 55);
	/* On déplace le bouton pour jouer aléatoireement les musiques */
	widget_move(fenetre_musique, bouton_aleatoire_musique, position_widget, hauteur_liste_musique + 75);
	return 1;
}
int clique_fenetre_musique(window_info *win, int mx, int my, Uint32 flags) {
	/* On ne prend pas en compte les cliques sur la barre de titre, ni
	   ceux dans la partie "option" */
	if ((my < 0) || (my > hauteur_liste_musique)) {
		return 0;
	}
	/* Molette vers le haut */
	if (flags & ELW_WHEEL_UP) {
		vscrollbar_scroll_up(fenetre_musique, musique_barre_defilement);
		return 1;
	}
	/* Molette vers le bas */
	if (flags & ELW_WHEEL_DOWN) {
		vscrollbar_scroll_down(fenetre_musique, musique_barre_defilement);
		return 1;
	}
	/* Vu qu'on a cliqué, on commence par déselectionner la musique */
	if (musique_selectionne != -1) {
		liste_musiques[musique_selectionne].selection = 0;
	}
	modif_playlist(my / hauteur_ligne_musique + vscrollbar_get_pos(win->window_id, musique_barre_defilement));
	return 1;
}
void parcours_fichier_musique(xmlNode *noeud) {
	int num;
	if (!xmlStrEqual(noeud->name, (xmlChar *)"musique")) {
		LOG_ERROR("Balise \"musique\" attendu au lieu de : %s\n", noeud->name);
		return;
	}
	num = xmlGetInt(noeud, (xmlChar *)"num");
	if (num >= MAX_NOMBRE_MUSIQUE) {
		LOG_ERROR("Le nombre de musique est de %d alors que son maximum est de %d\n", num, MAX_NOMBRE_MUSIQUE);
		return;
	}
	for (noeud = noeud->children; noeud; noeud = noeud->next) {
		...
		if (xmlStrEqual(noeud->name, (xmlChar *)"nom")) {
			get_string_value(liste_musiques[num].nom, sizeof(liste_musiques[num].nom), noeud);
			liste_musiques[num].selection = 0;
			if (max_musique < num) {
				max_musique = num;
			}
		}
		if (xmlStrEqual(noeud->name, (xmlChar *)"fichier")) {
			get_string_value(liste_musiques[num].nom_fichier, sizeof(liste_musiques[num].nom_fichier), noeud);
			liste_musiques[num].selection = 0;
			if (max_musique < num) {
				max_musique = num;
			}
	...
	// FIX: ajuster la barre de défilement après avoir chargé la liste
	vscrollbar_set_bar_len(fenetre_musique, musique_barre_defilement, max_musique + 1);
}
int init_liste_musique() {
	char *nom_fichier = "./musiques.xml";
	xmlDoc *doc;
	xmlNode *root;
	/* Lecture du fichier des musiques */
	doc = xmlReadFile(nom_fichier, NULL, 0);
	if (doc == NULL) {
		LOG_ERROR("Impossible de lire le fichier des musiques : %s\n", nom_fichier);
		return 0;
	}
	/* On parcours le fichier */
	root = xmlDocGetRootElement(doc);
	if (root == NULL) {
		LOG_ERROR("Impossible de parcourir le fichier des musiques : %s\n", nom_fichier);
		return 0;
	} else if (!xmlStrEqual(root->name, (xmlChar *)"musiques")) {
		LOG_ERROR("La balise racine n'est pas celui attendu : %s\n", root->name);
		return 0;
	} else {
		xmlNode *noeud;
		for (noeud = root->children; noeud; noeud = noeud->next) {
			if (noeud->type == XML_ELEMENT_NODE) {
				parcours_fichier_musique(noeud);
			}
		}
	}
	xmlFreeDoc(doc);
...
int remplissage_fenetre_musique(window_info *win) {
	Uint8 i, pos, max_lignes;
	char texte[50];
	pos = vscrollbar_get_pos(win->window_id, musique_barre_defilement);
	max_lignes = trunc(((float)win->len_y - (float)hauteur_options_musique) / (float)hauteur_ligne_musique);
	/* On vient de cliquer sur le bouton pour indiquer qu'on souhaite les musiques
	   de carte, on recharge donc la liste */
	if (musique_carte == 1 && musique_selectionne != -1) {
		musique_selectionne = -1;
		get_map_playlist();
	}
	/* On n'a aucune musique de sélectionnée et on clique sur le bouton de musique
	   des cartes. On refuse lance donc la première musique */
	if (musique_carte == 0 && musique_selectionne == -1) {
		musique_selectionne = 0;
		modif_playlist(musique_selectionne);
	}
	/* Messages d'informations */
	if ((mouse_y - win->cur_y > hauteur_liste_musique) && (mouse_y - win->cur_y < hauteur_liste_musique + 30)) {
		show_help("Permet de modifier le volume de la musique.", 0, win->len_y + 10);
	}
	if ((mouse_y - win->cur_y > hauteur_liste_musique + 30) && (mouse_y - win->cur_y < hauteur_liste_musique + 50)) {
		show_help("Permet d'utiliser les musiques de carte.", 0, win->len_y + 10);
	}
	if ((mouse_y - win->cur_y > hauteur_liste_musique + 51) && (mouse_y - win->cur_y < hauteur_liste_musique + 70)) {
		show_help("Permet d'autoriser le serveur à modifier la musique.", 0, win->len_y + 10);
	}
	if ((mouse_y - win->cur_y > hauteur_liste_musique + 71) && (mouse_y - win->cur_y < hauteur_liste_musique + 90)) {
		show_help("La liste sera lu de manière aléatoire.", 0, win->len_y + 10);
	}
	/* Remplissage de la fenêtre */
	for (i = 0; i <= max_musique && i < max_lignes; i++) {
		if (musique_selectionne == i + pos) {
			glColor3f(0.0f, 1.0f, 0.0f);
			if (!music_stream || !music_stream->playing) {
				safe_snprintf(texte, sizeof(texte), "%2i %s", i + pos, liste_musiques[i + pos].nom);
			} else {
				int min_joue = (int)(ov_time_tell(&music_stream->stream) / 60);
				int sec_joue = (int)ov_time_tell(&music_stream->stream) % 60;
				int min_total = (int)(ov_time_total(&music_stream->stream, -1) / 60);
				int sec_total = (int)ov_time_total(&music_stream->stream, -1) % 60;
				safe_snprintf(texte, sizeof(texte), "%2i %s (%d:%s%d/%d:%s%d)", i + pos, liste_musiques[i + pos].nom, min_joue, (sec_joue >= 10)?"":"0", sec_joue, min_total, (sec_total >= 10)?"":"0", sec_total);
			}
		} else {
			glColor3f(1.0f, 1.0f, 1.0f);
			safe_snprintf(texte, sizeof(texte), "%2i %s", i + pos, liste_musiques[i + pos].nom);
		}
		draw_string_small(5, hauteur_ligne_musique * i, (unsigned char *)texte, 1);
	}
	/* Création de la séparation entre la liste et les options */
	glDisable(GL_TEXTURE_2D);
	glBegin(GL_LINES);
	glColor4f(0.77f, 0.57f, 0.39f, 1.0f);
	glVertex3i(0, win->len_y - hauteur_options_musique, 0);
	glVertex3i(win->len_x, win->len_y - hauteur_options_musique, 0);
	glEnd();
	glEnable(GL_TEXTURE_2D);
	/* Texte pour le volume de la musique */
	draw_string_small(5, hauteur_liste_musique + 8, (unsigned char *)"Volume musique", 1);
	/* Texte pour utiliser la musique de carte */
	draw_string_small(5, hauteur_liste_musique + 37, (unsigned char *)"Musique de carte", 1);
	/* Texte pour utiliser la musique de carte */
	draw_string_small(5, hauteur_liste_musique + 57, (unsigned char *)"Musique du serveur", 1);
	/* Texte pour lire aléatoirement la liste de musique */
	draw_string_small(5, hauteur_liste_musique + 77, (unsigned char *)"Lecture aléatoire", 1);
	return 1;
}
void affiche_fenetre_musique() {
	if (fenetre_musique < 0) {
		fenetre_musique = create_window("Musique", -1, 0, fenetre_musique_x, fenetre_musique_y, fenetre_musique_largeur, hauteur_liste_musique + hauteur_options_musique, ELW_WIN_DEFAULT | ELW_RESIZEABLE);
		set_window_handler(fenetre_musique, ELW_HANDLER_DISPLAY, &remplissage_fenetre_musique);
		set_window_handler(fenetre_musique, ELW_HANDLER_CLICK, &clique_fenetre_musique);
		set_window_handler(fenetre_musique, ELW_HANDLER_RESIZE, &redimensionnement_fenetre_musique);
		set_window_min_size(fenetre_musique, 270, 200);
		/* Paramétrage de la barre de défilement */
		musique_barre_defilement = vscrollbar_add_extended(fenetre_musique, musique_barre_defilement, NULL, fenetre_musique_largeur - ELW_BOX_SIZE, ELW_BOX_SIZE, ELW_BOX_SIZE, hauteur_liste_musique - ELW_BOX_SIZE, 0, hauteur_liste_musique / hauteur_ligne_musique, 0.77f, 0.57f, 0.39f, 0, 1, max_musique + 1);
	...
	show_window(fenetre_musique);
	select_window(fenetre_musique);
}
...
void play_song(int position) {
	// Find the music stream
	if (music_stream) {
		if (strcmp(playlist[list_pos].file_name, playlist[position].file_name) != 0) {
			play_stream(position, music_stream, 0.0f);
			music_stream->fade = 1;
		}
		return;
	}
	// Don't have a music stream yet so create one
	add_stream(position, STREAM_TYPE_MUSIC, -1);
}
void find_next_song(int tx, int ty, int day_time) {
	int position;
	int temps_total = 0;
	int temps_joue = 0;
	for (position = 0; position < MAX_PLAYLIST_ENTRIES; position++) {
		if (playlist[position].file_name[0]) {
			if (tx > playlist[position].min_x && tx < playlist[position].max_x && ty > playlist[position].min_y && ty < playlist[position].max_y && (playlist[position].time == 2 || playlist[position].time == day_time)) {
				if (list_pos != position || (position == 0 && (!music_stream || !music_stream->playing))) {
					play_song(position);
					list_pos = position;
				} else if (loop_list == 0 && musique_carte == 0) {
					temps_total = (int)(ov_time_total(&music_stream->stream, -1));
					temps_joue = (int)(ov_time_tell(&music_stream->stream));
					if (temps_total - temps_joue < 2) {
			...
			for (j = 0; j < used_sources; j++) {
				for (k = 0; k < sound_type_data[sounds_list[sound_source_data[j].loaded_sound].sound].num_variants; k++) {
					for (l = STAGE_INTRO; l <= STAGE_OUTRO; l++) {
						...
						if (sound_type_data[sounds_list[sound_source_data[j].loaded_sound].sound].variant[k].part[l] && sound_type_data[sounds_list[sound_source_data[j].loaded_sound].sound].variant[k].part[l]->sample_num == i) {
							found = 1;
							break;
						}
					}
...
unsigned int add_map_sound(int type, int x, int y) {
	...
	for (i = 0; i < MAX_BUFFERS * 2; i++) {
		if (x >= sounds_list[i].x - buffer && x <= sounds_list[i].x + buffer && y >= sounds_list[i].y - buffer && y <= sounds_list[i].y + buffer && type == sounds_list[i].sound) {
...
unsigned int add_spell_sound(int spell) {
	...
	if (spell >= 0 && spell < NUM_ACTIVE_SPELLS && sound_spell_data[spell] > -1) {
		for (i = 0; i < MAX_BUFFERS * 2; i++) {
			if (sounds_list[i].sound == sound_spell_data[spell]) {
				return sounds_list[i].cookie;
			}
		}
		// Add the sound
...
int play_sound(int sound_num, int x, int y, float initial_gain) {
	...
	if ((pVariant->part[STAGE_INTRO] && pVariant->part[STAGE_INTRO]->sample_num < 0 && pNewType->loops == 0) || (!pVariant->part[STAGE_INTRO] && pNewType->loops == 0)) {
 ...
 */
		...
		if (snd > -1) {
			...
			if (cur_sound < 0 || sounds_list[cur_sound].sound != snd) {
				// Add the new sound
				pActor->cur_anim_sound_cookie = add_walking_sound(snd, 2 * pActor->x_pos, 2 * pActor->y_pos, pActor->actor_id == yourself ? 1 : 0, pActor->cur_anim.sound_scale);
			}
		}
	}
}
...
void init_sound() {
	...
	int error;
	int i;
	// on peut initialiser la liste même si la musique est désactivée dans la conf ?
	init_liste_musique();
...
int validate_boundary(map_sound_boundary_def *bounds, char *map_name) {
	...
	if (bounds->p[0].x == bounds->o[0].x && bounds->p[0].y == bounds->o[0].y && bounds->p[1].x == bounds->o[0].x && bounds->p[1].y == bounds->o[1].y && bounds->p[2].x == bounds->o[1].x && bounds->p[2].y == bounds->o[1].y && bounds->p[3].x == bounds->o[1].x && bounds->p[3].y == bounds->o[0].y) {
...
int check_for_valid_background_details(background_default *test) {
	...
	for (i = 0; i < sound_num_background_defaults - 1; i++) {
		if ((sound_background_defaults[i].time_of_day_flags & test->time_of_day_flags) > 0 && sound_background_defaults[i].map_type == test->map_type) {
			// Conflict
			err = -1;
			break;
		}
