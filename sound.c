...
void affiche_playlist()
{
    ...
    while(1)
    {
        ...
        {
            if (playlist[i].time == 0) safe_strncpy (timestr, "Nuit", 5);
            if (playlist[i].time == 1) safe_strncpy (timestr, "Jour", 5);
            if (playlist[i].time == 2) safe_strncpy (timestr, "Tout", 5);
		...
		if (++i > MAX_PLAYLIST_ENTRIES) break;
    }
}
...
#else //FR_VERSION
        musique_barre_defilement = vscrollbar_add_extended (fenetre_musique, musique_barre_defilement, NULL, fenetre_musique_largeur - ELW_BOX_SIZE, ELW_BOX_SIZE, ELW_BOX_SIZE, hauteur_liste_musique - ELW_BOX_SIZE, 0, 1.0f, 0.77f, 0.57f, 0.39f, 0, 1, max_musique + 1 - hauteur_liste_musique/hauteur_ligne_musique);
#endif //FR_VERSION
...
void find_next_song(int tx, int ty, int day_time)
{
    ...
    for (position = 0; position < MAX_PLAYLIST_ENTRIES; position++)
    {
        ...
        {
            if (tx > playlist[position].min_x &&
                tx < playlist[position].max_x &&
                ty > playlist[position].min_y &&
                ty < playlist[position].max_y &&
                (playlist[position].time == 2 ||
                 playlist[position].time == day_time))
            {
