...
void lecture_rvb(const xmlNode * noeud, couleur_rvb * couleur)
{
	for (noeud = noeud->children; noeud; noeud = noeud->next)
	{
		...
		if (! xmlStrEqual(noeud->name, (xmlChar*)"rvb")) continue;
	...
	if (couleur->rouge > 1) couleur->rouge /= 255;
	if (couleur->vert  > 1) couleur->vert  /= 255;
	if (couleur->bleu  > 1) couleur->bleu  /= 255;
}
...
void lecture_rvba(const xmlNode * noeud, couleur_rvba * couleur)
{
	...
	{
		if (noeud->type != XML_ELEMENT_NODE) continue;
		if (! xmlStrEqual(noeud->name, (xmlChar*)"rvba")) continue;
	...
	if (couleur->vert  > 1) couleur->vert  /= 255;
	if (couleur->bleu  > 1) couleur->bleu  /= 255;
	if (couleur->alpha > 1) couleur->alpha /= 255;
}
...
void lecture_palette(const xmlNode * noeud, Uint8 *couleur)
{
	...
	if      (!strcmp(label, "green1"))  *couleur = c_green1;
	else if (!strcmp(label, "green2"))  *couleur = c_green2;
	else if (!strcmp(label, "green3"))  *couleur = c_green3;
	else if (!strcmp(label, "green4"))  *couleur = c_green4;
	else if (!strcmp(label, "yellow1")) *couleur = c_yellow1;
	else if (!strcmp(label, "yellow2")) *couleur = c_yellow2;
	else if (!strcmp(label, "yellow3")) *couleur = c_yellow3;
	else if (!strcmp(label, "yellow4")) *couleur = c_yellow4;
	else if (!strcmp(label, "orange1")) *couleur = c_orange1;
	else if (!strcmp(label, "orange2")) *couleur = c_orange2;
	else if (!strcmp(label, "orange3")) *couleur = c_orange3;
	else if (!strcmp(label, "orange4")) *couleur = c_orange4;
	else if (!strcmp(label, "purple1")) *couleur = c_purple1;
	else if (!strcmp(label, "purple2")) *couleur = c_purple2;
	else if (!strcmp(label, "purple3")) *couleur = c_purple3;
	else if (!strcmp(label, "purple4")) *couleur = c_purple4;
	else if (!strcmp(label, "blue1"))   *couleur = c_blue1;
	else if (!strcmp(label, "blue2"))   *couleur = c_blue2;
	else if (!strcmp(label, "blue3"))   *couleur = c_blue3;
	else if (!strcmp(label, "blue4"))   *couleur = c_blue4;
	else if (!strcmp(label, "grey1"))   *couleur = c_grey1;
	else if (!strcmp(label, "grey2"))   *couleur = c_grey2;
	else if (!strcmp(label, "grey3"))   *couleur = c_grey3;
	else if (!strcmp(label, "grey4"))   *couleur = c_grey4;
	else if (!strcmp(label, "rose1"))   *couleur = c_rose1;
	else LOG_ERROR("theme : couleurs - palette '%s' inconnue!\n", label);
	free(label);
}
...
void section_palette(const xmlNode * noeud)
{
	...
	for (noeud = noeud->children; noeud; noeud = noeud->next)
	{
		...
		if (xmlStrEqual(noeud->name, (xmlChar*)"couleur"))
		{
			...
			else LOG_ERROR("theme : Numéro de palette non valide : %i (accepté de 0 à 29)\n", numero);
		}
		else LOG_ERROR("theme : palette - balise '%s' inconnue !\n", noeud->name);
	}
}
...
void section_couleurs(const xmlNode * noeud)
{
	for (noeud = noeud->children; noeud; noeud = noeud->next)
	{
		...
		if      (xmlStrEqual(noeud->name, (xmlChar*)"couleur"))
		{
			...
			if      (!strcmp(prop, "titre_perso"))    lecture_rvb(noeud, &couleur_titre);
			else if (!strcmp(prop, "nom_dialogue"))   lecture_rvb(noeud, &couleur_nom_pnj_dialogue);
			else if (!strcmp(prop, "infobulle_texte"))lecture_rvb(noeud, &couleur_texte_aide);
			else if (!strcmp(prop, "infobulle_fond")) lecture_rvba(noeud, &couleur_fond_aide);
			else if (!strcmp(prop, "fond_fenetre"))   lecture_rvba(noeud, &couleur_fond_fenetre);
			else if (!strcmp(prop, "bord_fenetre"))   lecture_rvba(noeud, &couleur_bord_fenetre);
			else if (!strcmp(prop, "ligne_fenetre"))  lecture_rvba(noeud, &couleur_ligne_fenetre);
			else if (!strcmp(prop, "item_cooldown"))  lecture_rvba(noeud, &fr_quickitems_coolcolor);
			else if (!strcmp(prop, "item_zero_qte"))  lecture_rvba(noeud, &fr_quickitems_zerocolor);
		}
		else if (xmlStrEqual(noeud->name, (xmlChar*)"palette"))
		{
			const char * prop = get_string_property(noeud, "type");
			if      (!strcmp(prop, "defaut"))  lecture_palette(noeud, &couleur_canaux);
			else if (!strcmp(prop, "actif"))   lecture_palette(noeud, &couleur_canal_actif);
			else if (!strcmp(prop, "mp"))      lecture_palette(noeud, &couleur_mp);
			else if (!strcmp(prop, "mg"))      lecture_palette(noeud, &couleur_mg_canal);
			else if (!strcmp(prop, "ig"))      lecture_palette(noeud, &couleur_ig_canal);
			else if (!strcmp(prop, "mod"))     lecture_palette(noeud, &couleur_mod_canal);
			else if (!strcmp(prop, "coord"))   lecture_palette(noeud, &couleur_coord_canal);
			else if (!strcmp(prop, "dev"))     lecture_palette(noeud, &couleur_dev_canal);
		}
		else LOG_ERROR("theme : couleurs - balise '%s' inconnue !\n", noeud->name);
	}
...
void section_polices(const xmlNode * noeud)
{
	for (noeud = noeud->children; noeud; noeud = noeud->next)
	{
		...
		else LOG_ERROR("theme : polices - balise '%s' inconnue !\n", noeud->name);
	}
	...
	for (noeud = noeud->children; noeud; noeud = noeud->next)
	{
		...
		if (xmlStrEqual(noeud->name, (xmlChar*)"valeur"))
		{
			...
			if      (!strcmp(prop, "sorts_icone"))
			{
				...
				if (quickspells_ico < 12) quickspells_ico = 12;
				else if (quickspells_ico > 64) quickspells_ico = 64;
			}
			else if (!strcmp(prop, "sorts_case"))
			{
				strtol_securise((const char *)noeud->children->content, &quickspells_dim);
				if (quickspells_dim < 12) quickspells_dim = 12;
				else if (quickspells_dim > 64) quickspells_dim = 64;
			}
			else if (!strcmp(prop, "sorts_groupe"))
			{
				strtol_securise((const char *)noeud->children->content, &quickspells_div);
				if (quickspells_div < 1) quickspells_div = QUICKSPELLS_MAXSIZE;
			}
			else if (!strcmp(prop, "sorts_delta"))
			{
				strtol_securise((const char *)noeud->children->content, &quickspells_sep);
				if (quickspells_sep < -32) quickspells_sep = -32;
				else if (quickspells_sep > 32) quickspells_sep = 32;
			}
			else if (!strcmp(prop, "items_case"))
			{
				strtol_securise((const char *)noeud->children->content, &fr_quickitems_dim);
				if (fr_quickitems_dim < 12) fr_quickitems_dim = 12;
				else if (fr_quickitems_dim > 64) fr_quickitems_dim = 64;
			}
			else if (!strcmp(prop, "items_groupe"))
			{
				strtol_securise((const char *)noeud->children->content, &fr_quickitems_div);
				if (fr_quickitems_div < 1) fr_quickitems_div = FR_QUICKITEMS_MAXSIZE;
			}
			else if (!strcmp(prop, "items_delta"))
			{
				strtol_securise((const char *)noeud->children->content, &fr_quickitems_sep);
				if (fr_quickitems_sep < -32) fr_quickitems_sep = -32;
				else if (fr_quickitems_sep > 32) fr_quickitems_sep = 32;
			}
		}
		else LOG_ERROR("theme : quickitems - balise '%s' inconnue !\n", noeud->name);
	}
...
void section_infocombat(const xmlNode * noeud)
{
	for (noeud = noeud->children; noeud; noeud = noeud->next)
	{
		...
		{
			const char *prop = get_string_property(noeud, "type");
			if      (!strcmp(prop, "degat_feu"))       lecture_rvb(noeud, &couleur_degat_feu);
			else if (!strcmp(prop, "degat_froid"))     lecture_rvb(noeud, &couleur_degat_froid);
			else if (!strcmp(prop, "degat_magie"))     lecture_rvb(noeud, &couleur_degat_magie);
			else if (!strcmp(prop, "degat_lumiere"))   lecture_rvb(noeud, &couleur_degat_lumiere);
			else if (!strcmp(prop, "degat_poison"))    lecture_rvb(noeud, &couleur_degat_poison);
		}
		else if (xmlStrEqual(noeud->name, (xmlChar*)"console"))
		{
			const char *prop = get_string_property(noeud, "type");
			if      (!strcmp(prop, "coup_critique"))   console_coup_critique   = get_bool_value(noeud);
			else if (!strcmp(prop, "chance_critique")) console_chance_critique = get_bool_value(noeud);
                        else if (!strcmp(prop, "chance_esquive"))  console_chance_esquive  = get_bool_value(noeud);
			else if (!strcmp(prop, "degat_feu"))       console_degat_feu       = get_bool_value(noeud);
			else if (!strcmp(prop, "degat_froid"))     console_degat_froid     = get_bool_value(noeud);
			else if (!strcmp(prop, "degat_magie"))     console_degat_magie     = get_bool_value(noeud);
			else if (!strcmp(prop, "degat_lumiere"))   console_degat_lumiere   = get_bool_value(noeud);
			else if (!strcmp(prop, "degat_poison"))    console_degat_poison    = get_bool_value(noeud);
		}
		else if (xmlStrEqual(noeud->name, (xmlChar*)"flottant"))
		{
			const char *prop = get_string_property(noeud, "type");
			if      (!strcmp(prop, "coup_critique"))   flottant_coup_critique  = get_bool_value(noeud);
			else if (!strcmp(prop, "chance_critique")) flottant_chance_critique= get_bool_value(noeud);
                        else if (!strcmp(prop, "chance_esquive"))  flottant_chance_esquive = get_bool_value(noeud);
			else if (!strcmp(prop, "degat_feu"))       flottant_degat_feu      = get_bool_value(noeud);
			else if (!strcmp(prop, "degat_froid"))     flottant_degat_froid    = get_bool_value(noeud);
			else if (!strcmp(prop, "degat_magie"))     flottant_degat_magie    = get_bool_value(noeud);
			else if (!strcmp(prop, "degat_lumiere"))   flottant_degat_lumiere  = get_bool_value(noeud);
			else if (!strcmp(prop, "degat_poison"))    flottant_degat_poison   = get_bool_value(noeud);
		}
		else LOG_ERROR("theme : infocombat - balise '%s' inconnue !\n", noeud->name);
	}
...
void section_hudicons(const xmlNode * noeud)
{
	int icon_pos = 0;
	int i, icon_size = 32;
	for (i = 0; i < MAX_ICONES; i++) icon_posx_theme[i] = -99;
	...
	for (noeud = noeud->children; noeud; noeud = noeud->next)
	{
		...
		else if (xmlStrEqual(noeud->name, (xmlChar*)"icone"))
		{
			int icon_no = -1;
			const char *prop = get_string_property(noeud, "type");
			if (get_int_property(noeud, "masquer") == 1) continue;
			if      ((++icon_no < MAX_ICONES) && !strcmp(prop, "marcher"));
			else if ((++icon_no < MAX_ICONES) && !strcmp(prop, "asseoir"));
			else if ((++icon_no < MAX_ICONES) && !strcmp(prop, "regarder"));
			else if ((++icon_no < MAX_ICONES) && !strcmp(prop, "utiliser"));
			else if ((++icon_no < MAX_ICONES) && !strcmp(prop, "utiliser_avec"));
			else if ((++icon_no < MAX_ICONES) && !strcmp(prop, "echanger"));
			else if ((++icon_no < MAX_ICONES) && !strcmp(prop, "attaquer"));
			...
			else if ((++icon_no < MAX_ICONES) && !strcmp(prop, "stats"));
			else if ((++icon_no < MAX_ICONES) && !strcmp(prop, "console"));
...
{
	if (! xmlStrEqual(noeud->name, (xmlChar*)"Theme")) return 0;
    ...
    for (noeud = noeud->children; noeud; noeud = noeud->next)
    {
		...
		if (xmlStrEqual(noeud->name, (xmlChar*)"section"))
		{
			...
			if      (! strcmp(type, "palette"))      section_palette(noeud);
			else if (! strcmp(type, "couleurs"))     section_couleurs(noeud);
...
void init_theme(char * titre)
{
	...
	for (i = 0; i < liste_themes_nb; i++)
	{
		if (strcmp(liste_themes[i].titre, titre) == 0)
		{
			...
			if (doc != NULL) recherche_sections(xmlDocGetRootElement(doc));
			else LOG_ERROR("theme : echec à l'ouverture du fichier xml (%s) !\n", liste_themes[i].fichier);
...
int chargement_liste_themes(char * dossier_themes)
{
	...
	while ((ep = readdir(dp)) != NULL)
	{
		...
		if ((strlen(ep->d_name) <= 4) || (strlen(ep->d_name) > 24)) continue;
		/* et qui se terminent par le suffixe '.xml' */
		if (strcmp(ep->d_name + strlen(ep->d_name) - 4, ".xml") != 0) continue;
		...
		{
			if (strcmp(liste_themes[i].titre, liste_themes[liste_themes_nb].titre) == 0) deja = 1;
		}
		if (deja) continue;
		...
		if (chargement_theme_detail(liste_themes_nb)) liste_themes_nb++;
	}
	closedir(dp);
	return 1;
}
