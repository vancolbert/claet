...
void compress_fr_quickitems() {
	...
	for (i = 0; i < FR_QUICKITEMS_MAXSIZE; i++) {
		if ((empty_slot >= 0) && (fr_quickitem_list[i].quantity >= 0)) {
			...
			fr_quickitem_list[i].quantity = -1;
			empty_slot++;
		} else if ((empty_slot < 0) && (fr_quickitem_list[i].quantity < 0)) {
			empty_slot = i;
		}
	}
}
	...
	for (i = 0; i < FR_QUICKITEMS_MAXSIZE; i++) {
		...
		for (j = 0; j < ITEM_WEAR_START; j++) {
			...
			if (item_list[j].id != unset_item_uid) {
				if (item_list[j].id != fr_quickitem_list[i].id) {
					continue;
				}
			} else
			// On compare les images. Le soucis est que parfois la même image sert pour 2 objets différents.
			if (item_list[j].image_id != fr_quickitem_list[i].image_id) {
				continue;
			}
			...
			if (fr_quickitem_list[i].is_stackable == 1) {
				break;
			}
		}
	}
...
void build_fr_quickitems(int force_complete) {
	...
	if (fr_quickitems_autoremove) {
		...
		if (fr_quickitems_autocompress) {
			compress_fr_quickitems();
		}
	}
		...
		for (i = 0; i < ITEM_WEAR_START; i++) {
			...
			for (j = 0; j < FR_QUICKITEMS_MAXSIZE; j++) {
				...
				if (item_list[i].id != unset_item_uid) {
					if (fr_quickitem_list[j].id != item_list[i].id) {
						continue;
					}
				} else if (fr_quickitem_list[j].image_id != item_list[i].image_id) {
					continue;
				}
				already_here = 1;
				break;
...
int affect_fr_quickitems(int pos) {
	...
	for (i = 0; i < FR_QUICKITEMS_MAXSIZE; i++) {
		...
		if (item_list[pos].id != unset_item_uid) {
			if (fr_quickitem_list[i].id == item_list[pos].id) {
				break;
			}
		} else if (fr_quickitem_list[i].image_id == item_list[pos].image_id) {
			break;
		}
	}
...
int mouseover_fr_quickitems_handler(window_info *win, int mx, int my) {
	...
	if (fr_quickitem_list[id_quick].quantity > 0) {
		for (i = 0; i < ITEM_WEAR_START; i++) {
			...
			if (item_list[i].id != unset_item_uid) {
				if (item_list[i].id != fr_quickitem_list[id_quick].id) {
					continue;
				}
			} else if (item_list[i].image_id != fr_quickitem_list[id_quick].image_id) {
				continue;
			}
			id_item = i;
			break;
	...
	if (item_dragged >= 0) {
		...
		for (i = 0; i < FR_QUICKITEMS_MAXSIZE; i++) {
			...
			if (item_list[item_dragged].id != unset_item_uid) {
				if (fr_quickitem_list[i].id != item_list[item_dragged].id) {
					continue;
				}
			} else if (fr_quickitem_list[i].image_id != item_list[item_dragged].image_id) {
				continue;
			}
			// suppression du raccourci déjà existant dans une autre case
			if (i != id_quick) {
				if ((empty_slot < 0) && !fr_quickitems_insertmode) {
					empty_slot = i;
				}
				fr_quickitem_list[i].quantity = -1;
			}
		...
		if (fr_quickitem_list[id_quick].quantity >= 0) {
			...
			/* mode par insertion */
				...
				for (empty_slot = id_quick; empty_slot < FR_QUICKITEMS_MAXSIZE; empty_slot++) {
					if (fr_quickitem_list[empty_slot].quantity < 0) {
						break;
					}
				}
		...
		if (item_dragged < 0) {
			...
			if (!item_list[id_item].is_stackable) {
				...
				for (i = 0; i < ITEM_WEAR_START; i++) {
					...
					if (item_list[i].id != unset_item_uid) {
						if (item_list[i].id != item_list[id_item].id) {
							continue;
						}
					} else if ((item_list[i].image_id != item_list[id_item].image_id) || item_list[i].is_stackable) {
						continue;
					}
					item_dragged_max_quantity += item_list[i].quantity;
				}
			} else {
				item_dragged_max_quantity = item_list[id_item].quantity;
			}
