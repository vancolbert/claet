...
#ifdef FR_AFFICHE_NOM
		my_strncp(actors_list[i]->nom_acteur_affiche,&in_data[28],sizeof(actors_list[i]->nom_acteur_affiche));
        for (k = 0; k <= strlen(actors_list[i]->nom_acteur_affiche); k++)
        {
            if ((actors_list[i]->nom_acteur_affiche)[k] == '_')
            {
                (actors_list[i]->nom_acteur_affiche)[k] = ' ';
            }
        }
#endif //FR_AFFICHE_NOM
