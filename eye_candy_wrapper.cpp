...
extern "C" void ec_recall_effect(const ec_reference ref) {
	...
	if (ref != NULL) {
		...
		if (cast_reference != NULL && cast_reference->effect != NULL) {
...
extern "C" void ec_add_object_obstruction(object3d *obj3d, e3d_object *e3dobj, float force) {
	...
	if (!strncmp(obj3d->file_name + 2, "misc_ob", 7)) {
		if (!strncmp(obj3d->file_name + 15, "cook", 4) || !strncmp(obj3d->file_name + 15, "pot_", 4) || !strncmp(obj3d->file_name + 15, "cauld", 5)) {
			obstruction->fire_related = true;
		}
	} else if (!strncmp(obj3d->file_name + 2, "struc", 5)) {
		if (!strncmp(obj3d->file_name + 13, "chimn", 5) || !strncmp(obj3d->file_name + 13, "forge", 5)) {
			obstruction->fire_related = true;
		}
...
extern "C" void ec_remove_weapon(actor *_actor) {
	...
	for (int i = 0; i < (int)references.size();) {
		...
		i++;
		if (((*iter)->caster == _actor) && (((*iter)->effect->get_type() == ec::EC_SWORD) || (*iter)->effect->get_type() == ec::EC_STAFF)) {
			(*iter)->effect->recall = true;
			(*iter)->caster = NULL;
			(*iter)->target = NULL;
			continue;
...
extern "C" ec_reference ec_create_sword_emerald_claymore_feu(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::EMERALD_CLAYMORE_F, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_emerald_claymore_glace(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::EMERALD_CLAYMORE_G, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_emerald_claymore_arcane(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::EMERALD_CLAYMORE_A, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_emerald_claymore_lumiere(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::EMERALD_CLAYMORE_L, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_cutlass_feu(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::CUTLASS_F, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_cutlass_glace(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::CUTLASS_G, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_cutlass_arcane(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::CUTLASS_A, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_cutlass_lumiere(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::CUTLASS_L, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_sunbreaker_feu(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::SUNBREAKER_F, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_sunbreaker_glace(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::SUNBREAKER_G, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_sunbreaker_arcane(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::SUNBREAKER_A, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_sunbreaker_lumiere(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::SUNBREAKER_L, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_orc_slayer_feu(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::ORC_SLAYER_F, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_orc_slayer_glace(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::ORC_SLAYER_G, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_orc_slayer_arcane(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::ORC_SLAYER_A, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_orc_slayer_lumiere(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::ORC_SLAYER_L, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_eagle_wing_feu(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::EAGLE_WING_F, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_eagle_wing_glace(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::EAGLE_WING_G, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_eagle_wing_arcane(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::EAGLE_WING_A, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_eagle_wing_lumiere(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::EAGLE_WING_L, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_jagged_saber_feu(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::JAGGED_SABER_F, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_jagged_saber_glace(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::JAGGED_SABER_G, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_jagged_saber_arcane(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::JAGGED_SABER_A, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_jagged_saber_lumiere(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::JAGGED_SABER_L, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
extern "C" ec_reference ec_create_sword_of_light(actor *_actor, int LOD) {
	ec_internal_reference *ret = (ec_internal_reference *)ec_create_generic();
	ret->caster = _actor;
	get_sword_positions(_actor, ret->position, ret->position2);
	ret->effect = new ec::SwordEffect(&eye_candy, &ret->dead, &ret->position, &ret->position2, ec::SwordEffect::SWORD_OF_LIGHT, LOD);
	eye_candy.push_back_effect(ret->effect);
	return (ec_reference)ret;
}
