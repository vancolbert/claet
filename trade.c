...
int display_trade_handler(window_info *win) {
	...
	for (i = 16; i >= 0; --i) {
		if (your_trade_list[i].quantity) {
			...
			x_start = (i % 4) * 33 + 10;
			y_start = (i / 4) * 33 + 30;
			draw_item(your_trade_list[i].image_id, x_start, y_start, 33);
	...
	for (i = 16; i >= 0; --i) {
		if (others_trade_list[i].quantity) {
			...
			y_start = (i / 4) * 33 + 30;
			draw_item(others_trade_list[i].image_id, x_start, y_start, 33);
	...
	if (show_help_text && show_abort_help) {
		show_help(abort_str, win->len_x - (ELW_BOX_SIZE - 4) / 2 - strlen(abort_str) * 4, 4 + ELW_BOX_SIZE);
	}
