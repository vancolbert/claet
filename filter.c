...
int add_to_filter_list(const char *name, char local, char save_name) {
	...
	for (i = 0; i < MAX_FILTERS; i++) {
		if (filter_list[i].len <= 0) {
			...
			l = strlen(left) - 1;
			if (left[0] == '*' && left[l] != '*') {
				filter_list[i].wildcard_type = 1;
			}
			if (left[0] != '*' && left[l] == '*') {
				filter_list[i].wildcard_type = 2;
			}
			if (left[0] == '*' && left[l] == '*') {
				filter_list[i].wildcard_type = 3;
			}
...
int filter_text(char *buff, int len, int size) {
	...
	while (i < new_len) {
		...
		while (i < new_len && !isalpha((unsigned char)buff[i])) {
			i++;
		}
		if (i >= new_len) {
			break;
		...
		if (idx >= 0) {
			...
			/* skip this word */
			while (i < new_len && isalpha(buff[i])) {
				i++;
			}
		}
	}
