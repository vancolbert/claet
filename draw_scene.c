...
void draw_scene() {
	...
	if (!have_display) {
		...
		if (!quickbar_on_top && !quickbar_draggable && (quickbar_win >= 0)) {
			if (quickbar_dir == VERTICAL) {
				...
				if (quickbar_y + quickbar_y_len < window_height - HUD_MARGIN_Y) {
					hide_window(quickbar_win);
				}
			}
		}
		if (!quickspells_on_top && !quickspells_draggable && (quickspell_win >= 0)) {
			if (quickspells_dir == VERTICAL) {
				if (quickspell_x - quickspell_x_len > HUD_MARGIN_X) {
					hide_window(quickspell_win);
				}
			} else {
				if (quickspell_y + quickspell_y_len < window_height - HUD_MARGIN_Y) {
					hide_window(quickspell_win);
				}
			}
		}
	...
	Leave2DMode();
	if (elwin_mouse >= 0) {
		if (current_cursor != elwin_mouse) {
			change_cursor(elwin_mouse);
		}
		elwin_mouse = -1;
	}
	SDL_GL_SwapBuffers();
...
void update_camera() {
	...
	clamp_camera();
	if (ext_cam && !first_person && me && rx <= -min_tilt_angle && rx >= -max_tilt_angle) {
