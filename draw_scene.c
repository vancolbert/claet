...
void draw_scene()
{
	...
	if (!have_display)
	{
		...
		if (! quickbar_on_top && ! quickbar_draggable && (quickbar_win >= 0))
		{
			...
			{
				if (quickbar_x - quickbar_x_len > HUD_MARGIN_X) hide_window(quickbar_win);
			}
			else
			{
				if (quickbar_y + quickbar_y_len < window_height - HUD_MARGIN_Y) hide_window(quickbar_win);
			}
		}
		if (! quickspells_on_top && ! quickspells_draggable && (quickspell_win >= 0))
		{
			...
			{
				if (quickspell_x - quickspell_x_len > HUD_MARGIN_X) hide_window(quickspell_win);
			}
			else
			{
