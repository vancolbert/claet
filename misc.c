...
int click_line_bbox_intersection(const AABBOX bbox) {
	...
	if (fabs(T[X]) > (E[X] + click_line.length * fabs(click_line.direction[X]))) {
		return 0;
	}
	if (fabs(T[Y]) > (E[Y] + click_line.length * fabs(click_line.direction[Y]))) {
		return 0;
	}
	if (fabs(T[Z]) > (E[Z] + click_line.length * fabs(click_line.direction[Z]))) {
		return 0;
	}
	...
	if (fabs(T[Y] * click_line.direction[Z] - T[Z] * click_line.direction[Y]) > r) {
		return 0;
	}
	// line.cross(y-axis)?
	...
	if (fabs(T[X] * click_line.direction[Y] - T[Y] * click_line.direction[X]) > r) {
		return 0;
	}
	return 1;
}
