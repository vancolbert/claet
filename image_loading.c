...
static Uint32 get_sdl_image_information(el_file_ptr file, image_t *image) {
	...
	if (image_surface == 0) {
		LOG_ERROR("load_image() error: [%s] [%s]", el_file_name(file), IMG_GetError());
		return 0;
	}
	...
	if ((image_surface->format->BitsPerPixel == 8) && (image_surface->format->palette != 0)) {
		image->alpha = 0;
	} else {
...
static Uint32 load_image_SDL(el_file_ptr file, image_t *image) {
	...
	image->image = (GLubyte *)malloc_aligned(image_width * image_height * 4, 16);
	data = image->image;
	idx = 0;
	pixel = 0;
...
static Uint32 load_image_SDL_alpha(el_file_ptr file, image_t *image) {
	...
	image_surface = IMG_Load_RW(SDL_RWFromMem(el_get_pointer(file), el_get_size(file)), 1);
	if (image_surface == 0) {
		LOG_ERROR("load_image() error: [%s] [%s]", el_file_name(file), IMG_GetError());
		return 0;
	}
	...
	if (image->width != image_width) {
		LOG_ERROR("Alpha map '%s' had wrong width %i, expected width"
			  " is %i.", el_file_name(file), image_width, image->width);
		return 0;
	}
	if (image->height != image_height) {
		LOG_ERROR("Alpha map '%s' had wrong width %i, expected width"
			  " is %i.", el_file_name(file), image_height, image->height);
		return 0;
	}
	x_padding = image_width % 4;
	if (x_padding) {
...
Uint32 load_image_data_file(el_file_ptr file, const Uint32 compression, const Uint32 unpack, const Uint32 strip_mipmaps, const Uint32 base_level, image_t *image) {
	char buffer[128];
	...
	if (dds == 1) {
		result = load_dds(file, compression, unpack, strip_mipmaps, base_level, image);
	} else {
	...
	if ((dds != 1) && (check_alpha_image_name(el_file_name(file), sizeof(buffer), buffer) != 0)) {
...
Uint32 load_image_data(const char *file_name, const Uint32 compression, const Uint32 unpack, const Uint32 strip_mipmaps, const Uint32 base_level, image_t *image) {
	char buffer[128];
	el_file_ptr file;
	...
	return load_image_data_file(file, compression, unpack, strip_mipmaps, base_level, image);
}
