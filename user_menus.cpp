...
void Container::open_window(void) {
	...
	window_used = true;
	reload();
	win_id = create_window(um_window_title_str, -1, 0, win_x_pos, win_y_pos, win_width, get_height(), win_flags);
...
void Container::pre_show(window_info *win, int widget_id, int mx, int my, window_info *cm_win) {
	...
	if (win == NULL || cm_win == NULL || !(curr_menu < menus.size())) {
		std::cerr << __FUNCTION__ << ": Problem with input win=" << win << " widget=" << widget_id << " mx=" << mx << " my=" << my << " cmwin=" << cm_win << std::endl;
		return;
	}
...
int Container::context(window_info *win, int widget_id, int mx, int my, int option) {
	...
	case ELW_CM_MENU_LEN + 2:
		set_win_flag(&win->flags, ELW_USE_BORDER, border_on);
		break;
	case ELW_CM_MENU_LEN + 3:
		recalc_win_width();
		break;
	case ELW_CM_MENU_LEN + 4:
	case ELW_CM_MENU_LEN + 8:
		reload();
		break;
	case ELW_CM_MENU_LEN + 9:
		toggle_user_menus(&enable_user_menus);
		break;
	}
	return 1;
}
