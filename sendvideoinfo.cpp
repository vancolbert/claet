...
static inline void parse_extentions(const xmlNodePtr extensions_element, const char *extensions, bit_set_96 bits) {
	xmlNodePtr cur_node;
	...
	if (video_info_sent == 0) {
		...
		if ((document = xmlReadFile("extentions.xml", 0, 0)) != NULL) {
			...
			if (root_element != 0) {
				try
				{
					parse_extentions(root_element, reinterpret_cast<const char *>
