...
void    quick_use(int use_id) {
	...
	if ((use_id >= FR_QUICKITEMS_MAXSIZE) || (fr_quickitem_list[use_id].quantity < 1)) {
		return;
	}
	for (i = 0; i < ITEM_WEAR_START; i++) {
		...
		if (item_list[i].id != unset_item_uid) {
			if (item_list[i].id != fr_quickitem_list[use_id].id) {
				continue;
			}
		} else if (item_list[i].image_id != fr_quickitem_list[use_id].image_id) {
			continue;
		}
		use_id = i;
...
int HandleEvent(SDL_Event *event) {
	...
	if (mod_key_status & KMOD_LALT) {
		alt_on = 1;
	} else {
		alt_on = 0;
	}
	if (mod_key_status & KMOD_CTRL) {
		ctrl_on = 1;
	} else {
		ctrl_on = 0;
	}
	if (mod_key_status & KMOD_META) {
		meta_on = 1;
	} else {
		meta_on = 0;
	}
	...
	case SDL_KEYDOWN:
		...
		// Deuxième appuie de suite sur la touche "^", le symbol "^" est
		// affiché
