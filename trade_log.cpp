	...
	for (size_t i = 0; i < size; i++) {
		if (the_stuff[i].quantity > 0) {
			...
			if (get_item_count(the_stuff[i].id, the_stuff[i].image_id) == 1) {
				...
				out << "   " << prefix << " " << the_stuff[i].quantity << " " << get_item_description(the_stuff[i].id, the_stuff[i].image_id) << ((the_stuff[i].type != 1) ?" (dépôt)" :"") << std::endl;
			}
			// otherwise use the raw ids
			else {
				out << "   " << prefix << " " << the_stuff[i].quantity << " image_id=" << the_stuff[i].image_id << " id=" << the_stuff[i].id << ((the_stuff[i].type != 1) ?" (dépôt)" :"") << std::endl;
			}
		}
	}
}
...
void State::completed(void) {
	...
	message << "Echange (" << your_stuff->who() << " <-" << "-> " << their_stuff->who() << ") " << buf << std::endl;
	if (your_stuff) {
