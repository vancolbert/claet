...
float get_bag_rotation(float pos_x, float pos_y, int bag_id, int map_x, int map_y) {
	...
	Uint8 digest[16];
	memset(digest, 0, 16);
	safe_snprintf(str, 40, "%f%f%f%f%f", pos_x, pos_y, (float)bag_id, (float)map_x, (float)map_y);
	MD5Open(&md5);
	MD5Digest(&md5, str, strlen(str));
	MD5Close(&md5, digest);
	...
	return ((sinf(digest[2]) + sinf(digest[3]) + sinf(digest[4]) + sinf(digest[5]) + sinf(digest[6])) / 10.0f) * 360;
}
float get_bag_tilt(float pos_x, float pos_y, int bag_id, int map_x, int map_y) {
	char str[64];
	MD5 md5;
	...
	return ((cosf(digest[3]) + cosf(digest[4]) + cosf(digest[5]) + cosf(digest[6]) + cosf(digest[7])) / 5.0f) * 30;
}
...
void add_bags_from_list(const Uint8 *data) {
	...
	for (i = 0; i < bags_no; i++) {
		...
		y = y + 0.25f; // + get_bag_offset_y(bag_x, bag_y, bag_id, tile_map_size_x, tile_map_size_y);
		// DEBUG
		LOG_DEBUG_VERBOSE("bag <%i> (%f,%f) rot %f tilt %f\n", bag_id, x, y, get_bag_rotation(bag_x, bag_y, bag_id, tile_map_size_x, tile_map_size_y), get_bag_tilt(bag_x, bag_y, bag_id, tile_map_size_x, tile_map_size_y));
		if (use_eye_candy) {}
		// Now, find a place into the bags list, so we can destroy the bag properly
...
int display_ground_items_handler(window_info *win) {
	...
	for (i = ITEMS_PER_BAG - 1; i >= 0; --i) {
		if (ground_item_list[i].quantity > 0) {
			...
			y_start = GRIDSIZE * (i / ground_items_grid_cols);
			draw_item(ground_item_list[i].image_id, x_start, y_start, 33);
...
int click_ground_items_handler(window_info *win, int mx, int my, Uint32 flags) {
	...
	if (pos == -1 || pos >= ITEMS_PER_BAG) {} else if (!ground_item_list[pos].quantity) {
		if (item_dragged != -1) {
			str[0] = DROP_ITEM;
			str[1] = item_dragged;
		...
		if (quantity > item_quantity && !ctrl_on) {
...
void draw_pick_up_menu() {
	if (ground_items_win < 0) {
		...
		if (ground_items_menu_x_len < (min_grid_cols + 1) * GRIDSIZE || ground_items_menu_x_len >= window_width || ground_items_menu_y_len < min_grid_rows * GRIDSIZE || ground_items_menu_y_len >= window_height) {
			ground_items_menu_x_len = 6 * GRIDSIZE;
			ground_items_menu_y_len = 10 * GRIDSIZE;
		}
		ground_items_win = create_window(win_bag, our_root_win, 0, ground_items_menu_x, ground_items_menu_y, ground_items_menu_x_len, ground_items_menu_y_len, ELW_SCROLLABLE | ELW_RESIZEABLE | ELW_WIN_DEFAULT);
