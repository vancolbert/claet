...
void log_message(const std::string &type, const std::string &message, const std::string &file, const Uint32 line, ThreadData &thread) {
	char buffer[128];
	...
	std::strftime(buffer, sizeof(buffer), "%X", std::localtime(&raw_time));
	str << ", " << file << ":" << line << "] " << type;
	str << ": " << message;
	...
	ssize_t ret = write(thread.m_log_file, log_stream.str().c_str(), log_stream.str().length());
	if (ret != static_cast<ssize_t>(log_stream.str().length())) {
		std::cerr << "Failed to write the log file: " << log_stream.str(); // newline included
	}
}
void do_log_message(const LogLevelType log_level, const std::string &message, const std::string &file, const Uint32 line, ThreadData &thread_data) {
	Uint32 level;
	log_message(get_str(log_level), message, file, line, thread_data);
	if (log_level < llt_debug) {
		level = thread_data.m_debug_marks.size();
		thread_data.m_message_level = std::max(level, thread_data.m_message_level);
	}
}
void do_enter_debug_mark(const std::string &name, const std::string &file, const Uint32 line, ThreadData &thread_data) {
	DebugMark debug_mark;
	std::stringstream str;
	debug_mark.m_name = name;
	debug_mark.m_log_file_pos = lseek(thread_data.m_log_file, 0, SEEK_CUR);
	thread_data.m_debug_marks.push_back(debug_mark);
	str << "Enter debug mark '" << name << "'";
	do_log_message(llt_debug, str.str(), file, line, thread_data);
}
void do_leave_debug_mark(const std::string &name, const std::string &file, const Uint32 line, ThreadData &thread_data) {
	std::stringstream str;
	Uint64 size, pos;
	Uint32 level;
	if (thread_data.m_debug_marks.rbegin() == thread_data.m_debug_marks.rend()) {
		if (log_levels >= llt_debug) {
			str << "Can't leave debug mark '";
			str << name << "', because no debug ";
			str << "mark " << "entered.";
			do_log_message(llt_error, str.str(), file, line, thread_data);
		}
		return;
	}
	if (thread_data.m_debug_marks.rbegin()->m_name != name) {
		...
		str << " is '";
		str << thread_data.m_debug_marks.rbegin()->m_name << "'.";
		do_log_message(llt_error, str.str(), file, line, thread_data);
		thread_data.m_debug_marks.pop_back();
		return;
	}
	if (log_levels < llt_debug_verbose) {
		size = thread_data.m_debug_marks.rbegin()->m_log_file_pos;
		if (thread_data.m_message_level < thread_data.m_debug_marks.size()) {
			pos = lseek(thread_data.m_log_file, 0, SEEK_CUR);
			lseek(thread_data.m_log_file, size, SEEK_SET);
			if ((pos + 1024) < size) {
				if (ftruncate(thread_data.m_log_file, size) < 0) {
					std::cerr << "Failed to truncate log file: " << strerror(errno) << std::endl;
				}
			}
		}
	} else {
		str << "Leave debug mark '" << name << "'";
		do_log_message(llt_debug_verbose, str.str(), file, line, thread_data);
	}
	thread_data.m_debug_marks.pop_back();
	level = thread_data.m_debug_marks.size();
	thread_data.m_message_level = std::min(level, thread_data.m_message_level);
}
std::string get_local_time_string() {
	...
	std::time(&raw_time);
	memset(buffer, 0, sizeof(buffer));
	std::strftime(buffer, sizeof(buffer), "%c %Z", std::localtime(&raw_time));
	return buffer;
}
void init(const std::string &name) {
	...
	thread_datas[id].m_log_file = open(file_name.str().c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0644);
