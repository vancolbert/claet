...
static int save_aliases(void) {
	...
	if (fp == NULL) {
		LOG_ERROR("%s: %s \"%s\": %s\n", reg_error_str, cant_open_file, NUMERIC_ALIASES_FILENAME, strerror(errno));
		return -1;
	}
	for (i = 0; i < 100; i++) {
		if (NULL != numeric_aliases[i]) {
			fprintf(fp, "%d ", i);
			escape_octal_putstring((unsigned char *)numeric_aliases[i], numeric_alias_sizes[i], fp);
			fputs("\n", fp);
		}
	}
	return fclose(fp);
	...
	case CHAR_ESCAPED:
		if (isdigit(*text)) {
			...
			if (octptr == 3) {
				...
				if (*endp) {
					LOG_ERROR("%s: %s \\%s\n", reg_error_str, "Invalid alias escape sequence", octal_value);
					free(retval);
					return NULL;
				}
			...
			LOG_ERROR("%s: %s '%c'\n", reg_error_str, "Invalid alias escape sequence char", *text);
			free(retval);
			return NULL;
		}
	}
...
static int read_alias_line(int index, char *text, int len) {
	int newlen;
	char *realline = unescape_octal_string((unsigned char *)text, len, &newlen);
	if (NULL != realline) {
	...
	if (fp == NULL) {
		...
		while (fgets(line, 512, fp)) {
			...
			if (generic_numeric_process(line, strlen(line), read_alias_line) < 0) {
				LOG_ERROR("%s: %s \"%s\"\n", reg_error_str, "Invalid line read in file ", NUMERIC_ALIASES_FILENAME);
				error = -1;
				break;
			}
