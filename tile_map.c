...
static __inline__ void build_terrain_buffer() {
	...
	if (have_extension(arb_vertex_buffer_object)) {
		ELglBindBufferARB(GL_ARRAY_BUFFER_ARB, terrain_tile_buffer_object);
		ELglBufferDataARB(GL_ARRAY_BUFFER_ARB, terrain_buffer_usage * 4 * 2 * sizeof(GLfloat), terrain_tile_buffer, GL_DYNAMIC_DRAW_ARB);
		ELglBindBufferARB(GL_ARRAY_BUFFER_ARB, 0);
	}
}
void draw_quad_tiles(const unsigned int start, const unsigned int stop, unsigned int idx, const unsigned int zero_id) {
	Uint32 i, l, size;
	Sint32 x, y;
...
int get_tile_valid(const int x, const int y) {
	return (x >= 0) && (x < (tile_map_size_x * 6)) && (y >= 0) && (y < (tile_map_size_y * 6));
}
