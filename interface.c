...
void draw_game_map(int map, int mouse_mini) {
	...
	glVertex3i(150, 0, 0);
	glTexCoord2f(1.0f, 0.0f);
	glVertex3i(150, 600, 0);
	glTexCoord2f(0.0f, 0.0f);
	glVertex3i(750, 600, 0);
	glTexCoord2f(0.0f, 1.0f);
	glVertex3i(750, 0, 0);
	glEnd();
	if (mouse_mini) {
	...
	glBegin(GL_QUADS);
	glTexCoord2f(1.0f, 1.0f);
	glVertex3i(750, 450, 0);
	glTexCoord2f(1.0f, 0.0f);
	glVertex3i(750, 600, 0);
	glTexCoord2f(0.0f, 0.0f);
	glVertex3i(900, 600, 0);
	glTexCoord2f(0.0f, 1.0f);
	glVertex3i(900, 450, 0);
	glEnd();
	...
	glBegin(GL_QUADS);
	glTexCoord2f(1.0f, 1.0f);
	glVertex3i(750, 150, 0);
	glTexCoord2f(1.0f, 0.0f);
	glVertex3i(750, 450, 0);
	glTexCoord2f(0.0f, 0.0f);
	glVertex3i(900, 450, 0);
	glTexCoord2f(0.0f, 1.0f);
	glVertex3i(900, 150, 0);
	glEnd();
...
// because of the coordinate display - Lachesis
	if (map /*&&(adding_mark||max_mark>0)*/ ) {
		...
		if (adding_mark) {
			...
			if (inspect_map_text != 0) {
				if (temp_tile_map_size_x > 0) {
					screen_x = (151 + 600 * x / (temp_tile_map_size_x * 6));
				}
				if (temp_tile_map_size_y > 0) {
					screen_y = 601 - 600 * y / (temp_tile_map_size_y * 6);
				}
			}
			set_font(police_carte);
	...
	if (!map && show_continent_map_boundaries && cont_text != fallback_text) {
		...
		for (i = 0; continent_maps[i].name != NULL; i++) {
			if (continent_maps[i].cont == cur_cont_map) {
				if (!mouse_mini && mouse_over == -1 && m_px > continent_maps[i].x_start && m_px < continent_maps[i].x_end && m_py > continent_maps[i].y_start && m_py < continent_maps[i].y_end) {
					/* Mouse over this map */
					mouse_over = i;
				} else {
...
int put_mark_on_position(int map_x, int map_y, char *name) {
	...
	if (inspect_map_text) {
		if (map_x >= temp_tile_map_size_x * 6 || map_y >= temp_tile_map_size_y * 6) {
			return 0;
		}
	} else {
		if (map_x >= tile_map_size_x * 6 || map_y >= tile_map_size_y * 6) {
			return 0;
		}
	}
...
void delete_mark_on_map_on_mouse_position() {
	...
	// détermine les coordonnées en fonction de la carte inspectée
	if (!map_get_mouse_position(mouse_x, mouse_y, &mx, &my)) {
		return;
	}
...
void destroy_all_root_windows() {
	if (game_root_win >= 0) {
		destroy_window(game_root_win);
	}
	if (console_root_win >= 0) {
		destroy_window(console_root_win);
	}
	if (map_root_win >= 0) {
		destroy_window(map_root_win);
	}
	if (login_root_win >= 0) {
		destroy_window(login_root_win);
	}
	if (rules_root_win >= 0) {
	...
	if (update_root_win >= 0) {
		destroy_window(update_root_win);
	}
	if (langsel_rootwin >= 0) {
...
void hide_all_root_windows() {
	...
	if (rules_root_win >= 0) {
		hide_window(rules_root_win);
	}
	if (opening_root_win >= 0) {
		hide_window(opening_root_win);
	}
	if (newchar_root_win >= 0) {
	...
	if (langsel_rootwin >= 0) {
		hide_window(langsel_rootwin);
	}
}
void resize_all_root_windows(Uint32 w, Uint32 h) {
	if (game_root_win >= 0) {
		resize_window(game_root_win, w, h);
	}
	if (console_root_win >= 0) {
		resize_window(console_root_win, w, h);
	}
	if (map_root_win >= 0) {
		resize_window(map_root_win, w, h);
	}
	if (login_root_win >= 0) {
		resize_window(login_root_win, w, h);
	}
	if (rules_root_win >= 0) {
		resize_window(rules_root_win, w, h);
	}
	if (opening_root_win >= 0) {
		resize_window(opening_root_win, w, h);
	}
	if (newchar_root_win >= 0) {
		resize_window(newchar_root_win, w, h);
	}
	if (update_root_win >= 0) {
		resize_window(update_root_win, w, h);
	}
