...
static __inline__ void add_intersect_item_to_list(BBOX_TREE *bbox_tree, BBOX_ITEM *item, Uint32 idx) {
	if (item->cluster && item->cluster != current_cluster) {
		return;
	}
...
static __inline__ void add_intersect_items(BBOX_TREE *bbox_tree, Uint32 index, Uint32 count) {
	...
	for (i = 0; i < count; i++) {
		add_intersect_item(bbox_tree, index + i, bbox_tree->cur_intersect_type);
	}
}
...
static __inline__ void add_dyn_intersect_items(BBOX_TREE *bbox_tree, Uint32 node, Uint32 count) {
	...
	adapt_intersect_list_size(bbox_tree, count);
	for (i = 0; i < count; i++) {
		add_dyn_intersect_item(bbox_tree, node, i, bbox_tree->cur_intersect_type);
	}
}
	...
	if (sub_node != NO_INDEX) {
		...
		if (result == INSIDE) {
			...
			if (result == INTERSECT) {
				...
				if ((bbox_tree->nodes[sub_node].nodes[0] == NO_INDEX) && (bbox_tree->nodes[sub_node].nodes[1] == NO_INDEX)) {
					add_items(bbox_tree, sub_node, out_mask);
				} else {
	...
	if (sub_node != NO_INDEX) {
		...
		if (result == INSIDE) {
			...
			if (result == INTERSECT) {
				...
				if ((bbox_tree->nodes[sub_node].nodes[0] == NO_INDEX) && (bbox_tree->nodes[sub_node].nodes[1] == NO_INDEX)) {
					merge_items(bbox_tree, sub_node, out_mask, bbox);
				} else {
...
static __inline__ int light_depht_comp(BBOX_ITEM *a, BBOX_ITEM *b) {
	...
	ai = a->ID;
	bi = b->ID;
	if ((lights_list[ai] == NULL) || (lights_list[bi] == NULL)) {
		return 0;
	}
...
static __inline__ void delete_item_from_intersect_list(BBOX_TREE *bbox_tree, Uint32 ID, Uint32 type_mask) {
	...
	for (i = 0; i < MAX_INTERSECTION_TYPES; i++) {
		for (j = 0; j < TYPES_COUNT; j++) {
			if (type_mask != get_type_mask_from_type(j)) {
				continue;
			}
			start = bbox_tree->intersect[i].start[j];
			stop = bbox_tree->intersect[i].stop[j];
			for (k = start; k < stop; k++) {
				if ((type_mask == TYPE_MASK_3D_BLEND_SELF_LIT_OBJECT) || (type_mask == TYPE_MASK_3D_BLEND_NO_SELF_LIT_OBJECT) || (type_mask == TYPE_MASK_3D_NO_BLEND_SELF_LIT_OBJECT) || (type_mask == TYPE_MASK_3D_NO_BLEND_NO_SELF_LIT_OBJECT)) {
					id = get_3dobject_index(bbox_tree->intersect[i].items[k].ID);
				} else {
					id = bbox_tree->intersect[i].items[k].ID;
				}
				if (id == ID) {
					size = stop - k - 1;
					if (size > 0) {
...
static __inline__ void free_bbox_tree_data(BBOX_TREE *bbox_tree) {
	...
	if (bbox_tree->items != NULL) {
		...
		BBOX_TREE_LOG_INFO("bbox_tree->items");
	}
	...
	if (bbox_tree->nodes != NULL) {
		...
		BBOX_TREE_LOG_INFO("bbox_tree->nodes");
	}
...
static __inline__ void build_area_table(const BBOX_TREE *bbox_tree, Uint32 a, Uint32 b, float *areas) {
...
static __inline__ void find_axis_and_bbox(const BBOX_TREE *bbox_tree, Uint32 first, Uint32 last, Uint32 *ret, AABBOX *bbox) {
	Uint32 i, a1, a2, a3;
	VECTOR3 bmin, bmax;
	float d, e;
	VFill(bmin, BOUND_HUGE);
...
static __inline__ Uint32 get_3D_type(Uint32 blend, Uint32 ground, Uint32 alpha, Uint32 self_lit) {
	...
	switch (type) {
	case 0:
		return TYPE_3D_BLEND_GROUND_ALPHA_SELF_LIT_OBJECT;
	case 1:
		return TYPE_3D_BLEND_GROUND_ALPHA_NO_SELF_LIT_OBJECT;
	case 2:
		return TYPE_3D_BLEND_GROUND_NO_ALPHA_SELF_LIT_OBJECT;
	case 3:
		return TYPE_3D_BLEND_GROUND_NO_ALPHA_NO_SELF_LIT_OBJECT;
	case 4:
		return TYPE_3D_BLEND_NO_GROUND_ALPHA_SELF_LIT_OBJECT;
	case 5:
		return TYPE_3D_BLEND_NO_GROUND_ALPHA_NO_SELF_LIT_OBJECT;
	case 6:
		return TYPE_3D_BLEND_NO_GROUND_NO_ALPHA_SELF_LIT_OBJECT;
	case 7:
		return TYPE_3D_BLEND_NO_GROUND_NO_ALPHA_NO_SELF_LIT_OBJECT;
	case 8:
		return TYPE_3D_NO_BLEND_GROUND_ALPHA_SELF_LIT_OBJECT;
	case 9:
		return TYPE_3D_NO_BLEND_GROUND_ALPHA_NO_SELF_LIT_OBJECT;
	case 10:
		return TYPE_3D_NO_BLEND_GROUND_NO_ALPHA_SELF_LIT_OBJECT;
	case 11:
		return TYPE_3D_NO_BLEND_GROUND_NO_ALPHA_NO_SELF_LIT_OBJECT;
	case 12:
		return TYPE_3D_NO_BLEND_NO_GROUND_ALPHA_SELF_LIT_OBJECT;
	case 13:
		return TYPE_3D_NO_BLEND_NO_GROUND_ALPHA_NO_SELF_LIT_OBJECT;
	case 14:
		return TYPE_3D_NO_BLEND_NO_GROUND_NO_ALPHA_SELF_LIT_OBJECT;
	case 15:
		return TYPE_3D_NO_BLEND_NO_GROUND_NO_ALPHA_NO_SELF_LIT_OBJECT;
	default:
		return 0xFF;
	}
}
...
static __inline__ Uint32 get_2D_type(Uint32 alpha) {
	if (alpha == 0) {
		return TYPE_2D_NO_ALPHA_OBJECT;
	} else {
		return TYPE_2D_ALPHA_OBJECT;
	}
}
...
static __inline__ Uint32 get_water_type(Uint32 reflectiv) {
	if (reflectiv) {
		return TYPE_REFLECTIV_WATER;
	} else {
		return TYPE_NO_REFLECTIV_WATER;
	}
}
...
static __inline__ void add_aabb_to_abt(BBOX_TREE *bbox_tree, const AABBOX bbox, Uint32 ID, Uint32 type, Uint32 texture_id, Uint32 dynamic) {
	...
	if (bbox_tree != NULL) {
		...
		if (result == 0) {
			add_dynamic_item_to_node(bbox_tree, 0, bbox, ID, type, texture_id, 0);
		}
		set_all_intersect_update_needed(bbox_tree);
	} else {
		BBOX_TREE_LOG_INFO("bbox_tree");
...
static __inline__ void delete_dynamic_item_from_node(BBOX_TREE *bbox_tree, Uint32 node, Uint32 idx, Uint32 ID, Uint32 type_mask) {
	...
	if (is_extra_first_sub_node(bbox_tree->nodes[node].dynamic_objects.items[idx].extra)) {
		...
		delete_dynamic_aabb_from_node(bbox_tree, bbox_tree->nodes[node].nodes[1], ID, type_mask);
	}
	if (index <= 1) {
		size = 0;
		index = 0;
		...
		count = index - idx - 1;
		if (count > 0) {
			memmove(&bbox_tree->nodes[node].dynamic_objects.items[idx], &bbox_tree->nodes[node].dynamic_objects.items[idx + 1], count * sizeof(BBOX_ITEM));
		}
		index--;
	}
...
static __inline__ Uint32 dynamic_aabb_is_in_node(BBOX_TREE *bbox_tree, Uint32 node, Uint32 ID, Uint32 type_mask) {
	...
	for (i = 0; i < bbox_tree->nodes[node].dynamic_objects.index; i++) {
		if ((type_mask == TYPE_MASK_3D_BLEND_SELF_LIT_OBJECT) || (type_mask == TYPE_MASK_3D_BLEND_NO_SELF_LIT_OBJECT) || (type_mask == TYPE_MASK_3D_NO_BLEND_SELF_LIT_OBJECT) || (type_mask == TYPE_MASK_3D_NO_BLEND_NO_SELF_LIT_OBJECT)) {
			...
			id = bbox_tree->nodes[node].dynamic_objects.items[i].ID;
		}
		if ((id == ID) && (get_type_mask_from_type(bbox_tree->nodes[node].dynamic_objects.items[i].type) == type_mask)) {
...
void free_bbox_items(BBOX_ITEMS *bbox_items) {
	if (bbox_items != NULL) {
		if (bbox_items->items != NULL) {
			...
			BBOX_TREE_LOG_INFO("bbox_items->items");
		}
		free(bbox_items);
		bbox_items = NULL;
	} else {
...
static __inline__ int check_shadow_line_walk_outside(const AABBOX bbox, const FRUSTUM frustum, const VECTOR3 light_dir, Uint32 mask, const float *hit) {
	...
	VECTOR3 walk;
	int i, k;
	if (check_aabb_outside_frustum(bbox, frustum, mask) != OUTSIDE) {
		return INTERSECT;
	}
	for (i = 0, k = 1; k <= mask; i++, k += k) {
		if ((k & mask) && (hit[i] > 0.0f)) {
			...
			VAdd(tmp_bbox.bbmin, bbox.bbmin, walk);
			VAdd(tmp_bbox.bbmax, bbox.bbmax, walk);
			if (check_aabb_outside_frustum(tmp_bbox, frustum, mask) != OUTSIDE) {
				return INTERSECT;
			}
		}
	}
...
static __inline__ void add_dyn_items_shadow(BBOX_TREE *bbox_tree, Uint32 sub_node, const FRUSTUM frustum, Uint32 in_mask, const FRUSTUM view_frustum, const FRUSTUM_DATA data, const VECTOR3 light_dir, Uint32 mask, Uint32 point_mask) {
	Uint32 idx, size, i;
	idx = bbox_tree->cur_intersect_type;
	size = bbox_tree->nodes[sub_node].dynamic_objects.index;
	for (i = 0; i < size; i++) {
		if (check_aabb_outside_frustum(bbox_tree->nodes[sub_node].dynamic_objects.items[i].bbox, frustum, in_mask) != OUTSIDE) {
			if (check_shadow_lines_outside(bbox_tree->nodes[sub_node].dynamic_objects.items[i].bbox, view_frustum, data, light_dir, mask, point_mask) != OUTSIDE) {
				add_dyn_intersect_item(bbox_tree, sub_node, i, idx);
			}
		}
	}
}
static __inline__ void add_items_shadow(BBOX_TREE *bbox_tree, Uint32 sub_node, const FRUSTUM frustum, Uint32 in_mask, const FRUSTUM view_frustum, const FRUSTUM_DATA data, const VECTOR3 light_dir, Uint32 mask, Uint32 point_mask) {
	Uint32 idx1, idx2, size, i;
	idx1 = bbox_tree->cur_intersect_type;
	idx2 = bbox_tree->nodes[sub_node].items_index;
	size = bbox_tree->nodes[sub_node].items_count;
...
static __inline__ void check_sub_nodes_shadow(BBOX_TREE *bbox_tree, Uint32 sub_node, const FRUSTUM frustum, Uint32 in_mask, const FRUSTUM view_frustum, const FRUSTUM_DATA data, const VECTOR3 light_dir, Uint32 mask, Uint32 point_mask) {
		...
		if (result == INSIDE) {
			...
			if (result == INTERSECT) {
				...
				if ((bbox_tree->nodes[sub_node].nodes[0] == NO_INDEX) && (bbox_tree->nodes[sub_node].nodes[1] == NO_INDEX)) {
					...
					check_sub_nodes_shadow(bbox_tree, bbox_tree->nodes[sub_node].nodes[0], frustum, out_mask, view_frustum, data, light_dir, mask, point_mask);
					check_sub_nodes_shadow(bbox_tree, bbox_tree->nodes[sub_node].nodes[1], frustum, out_mask, view_frustum, data, light_dir, mask, point_mask);
				}
			}
		}
	}
...
static __inline__ int calculate_point_mask(const VECTOR3 light_dir) {
	int mask, idx, i, k;
	idx = 0;
	if (VExtract(light_dir, X) >= 0.0f) {
		idx += 1;
	}
	if (VExtract(light_dir, Y) >= 0.0f) {
		idx += 2;
	}
	if (VExtract(light_dir, Z) >= 0.0f) {
		idx += 4;
	}
	mask = 0;
...
void check_bbox_tree_shadow(BBOX_TREE *bbox_tree, const FRUSTUM frustum, Uint32 mask, const FRUSTUM view_frustum, Uint32 view_mask, const VECTOR3 light_dir) {
	Uint32 idx, point_mask;
	FRUSTUM_DATA data;
	if (bbox_tree != NULL) {
		idx = bbox_tree->cur_intersect_type;
		if (idx != INTERSECTION_TYPE_SHADOW) {
			return;
		}
		if (bbox_tree->intersect[idx].intersect_update_needed > 0) {
