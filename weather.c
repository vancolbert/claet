...
void weather_set_area(int area, float x, float y, float radius, int type, float intensity, int change_duration) {
	if (weather_areas[area].type == 0) {
		weather_areas[area].intensity = 0.0;
	}
...
void update_weather_type(int type, float x, float y, float z, int ticks) {
	...
	if (num_drops > MAX_RAIN_DROPS) {
		num_drops = MAX_RAIN_DROPS;
	}
...
void weather_render() {
	...
	if (light_level[0] > 1.0) {
		light_level[0] = 1.0;
	}
	if (light_level[1] > 1.0) {
		light_level[1] = 1.0;
	}
	if (light_level[2] > 1.0) {
		light_level[2] = 1.0;
	}
	...
	for (type = 1; type < MAX_WEATHER_TYPES; ++type) {
		if (!weather_defs[type].use_sprites && weather_drops_count[type] > 0) {
			...
			glColor4f(weather_defs[type].color[0] * light_level[0], weather_defs[type].color[1] * light_level[1], weather_defs[type].color[2] * light_level[2], weather_defs[type].color[3]);
...
void weather_add_lightning(int type, float x, float y) {
	if (thunders_count < MAX_THUNDERS && lightnings_defs_count > 0) {
		...
		lightning_falling = 1;
		skybox_coords_from_ground_coords(lightning_sky_position, lightning_position[0] + camera_x, lightning_position[1] + camera_y);
...
void weather_sound_control() {
	...
	int i;
	if (cur_time < last_sound_update + 200) {
		return;
	} else {
		last_sound_update = cur_time;
	}
	if (!sound_on) {
		rain_sound = 0;
	} else {
	...
	for (i = 0; i < thunders_count; ) {
		...
		if (cur_time >= thunders[i].time + dist / SOUND_SPEED) {
			if (sound_on) {
				...
				case 0:
					snd_thunder = snd_thndr_1;
					break;
				case 1:
					snd_thunder = snd_thndr_2;
					break;
				case 2:
					snd_thunder = snd_thndr_3;
					break;
				case 3:
					snd_thunder = snd_thndr_4;
					break;
				case 4:
					snd_thunder = snd_thndr_5;
					break;
				default:
					snd_thunder = 0;
				}
